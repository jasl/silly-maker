...
一台复古电脑——不是游戏。开机直达桌面：重叠可拖拽窗口、任务栏、开始菜单，以及一组应用——确定性扫雷（雷区来自事务 RNG，绝不泄漏给 UI）、文件存进权威状态的记事本、年代感浏览器、显示属性与控制面板。

它存在的意义：证明引擎在视觉小说之外也站得住。

- **流体视口**——桌面 1:1 平铺任意浏览器区域（含手机竖屏），无黑边。
- **窗口管理是 UI 瞬态**——约 180 行的 Story 侧 store 搞定 z 序、焦点、最小化/最大化、拖拽与边界钳制。
- **存档即硬盘**——完全没有存档 UI：引擎持续自动保存，SillyOS 在开机画面期间恢复 `auto.current`。关掉标签页再开机，文件都在。
- **自绘系统外观**——引擎默认系统菜单、标题屏、设置对话框全部隐藏（`hideSystemMenu`、无 `titleScreen`）；Win98 风格的按下反馈与内凹输入框是纯 CSS。
