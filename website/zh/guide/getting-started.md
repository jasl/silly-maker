...
Player 构建产物是自包含的静态包（相对路径、存档在每位访客自己的浏览器里——
不需要服务器），`dist-web/` 可以直接交给静态托管。模板和 examples 自带
Cloudflare Workers 的 `deploy:cf` 任务：
...
3. 确认产品能通过授权页、伴随文件或稳定公开链接，向接收者提供 SillyMaker
   MIT 文本和实际随 Player 打包材料要求的 notice。
4. 在应用目录里运行 `deno task deploy:cf`（自动构建 dist-web/ 并部署），
   再把 URL 给我打开。
```
