...
- Delivery: `deno task build:web` for the web and `site:build` for static hosting (GitHub Pages / Cloudflare Workers). `deno task build:desktop` produces desktop previews with an icon and file-backed saves — the host `.app` by default, and cross-compiled `.app`/`.msi`/`.AppImage` packages via `--target`; signing/notarization and durable-store promotion remain explicit release gates.
