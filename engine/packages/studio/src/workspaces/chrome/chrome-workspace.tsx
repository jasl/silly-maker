...
import { createChromeLayoutDocumentSessionV1 } from "../../core/chrome-layout-session.ts";
...
export function ChromeWorkspaceSectionV1(props: ChromeWorkspaceSectionPropsV1): ReactElement {
  ...
  const session = useMemo(() => createChromeLayoutDocumentSessionV1(io), [io]);
  ...
  const save = useCallback((): void => {
    ...
    if (current === null) return;
    setNote(null);
    void session.save({ document: graduateChromeLayoutDocumentV1(current) }).then((result) => {
      if (result.kind === "ok") setNote("已保存；运行中的游戏会热更新。");
      else if (result.kind === "error") setNote(chromeSaveNoteV1(result.code));
    });
  }, [session]);
  ...
  const confirmSaveAndOpen = useCallback((): void => {
    ...
    void session.save({ document: graduateChromeLayoutDocumentV1(current) }).then((result) => {
      if (result.kind === "ok") {
        ...
        setNote(chromeSaveNoteV1(result.code));
      }
    });
