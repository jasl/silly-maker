...
interface FakeChromeIoV1 extends ChromeLayoutSourceIoV1 {
  ...
  replaceSaved(path: string, document: ChromeLayoutDocumentV1, digest: string): void;
}
...
function fakeChromeIoV1(
  ...
  return {
    ...
    replaceSaved(path, document, digest) {
      documents.set(path, { digest, doc: document });
    },
    list: () =>
      Promise.resolve({
        kind: "ok" as const,
...
describe("ChromeWorkspaceSectionV1", () => {
  ...
  it("refreshes a conflicted Chrome digest while preserving the dirty draft", async () => {
    const io = fakeChromeIoV1([{ path: hudLayoutPathV1, doc: hudLayoutDocumentV1() }]);
    ...
    io.replaceSaved(hudLayoutPathV1, hudLayoutDocumentV1(), "sha256:external");
    ...
    // The draft survives the conflict and the retry uses the refreshed CAS baseline.
    expect(screen.getByLabelText("X")).toHaveValue(280);
    await userEvent.setup().click(
      container.querySelector("[data-studio-chrome-save]") as HTMLElement,
    );
