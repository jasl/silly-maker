...
describe("CommandLog", () => {
  it("admits public commands and evidence before mutating the log", () => {
    ...
    expect(() => log.append(invalidCommand, finalizedAttempt(replayBase, 1))).toThrow(
      CanonicalJsonError,
    );
    ...
    expect(() => log.append(parsedCommand(1), invalidEvidence)).toThrow(CanonicalJsonError);
    expect(log.entries()).toEqual([]);
    ...
    const entry = log.append(rawCommand, finalizedAttempt(replayBase, 1));
    expect(entry.command).toEqual(rawCommand.command);
    expect(entry.command).not.toBe(rawCommand.command);
    expect(entry.logOrdinal).toBe(1);
  });

  it("preserves admitted public metadata and rejects engine-field collisions", () => {
    type MetadataCommandV1 = FixtureLoggedCommandV1 & Readonly<Record<string, unknown>>;
    const replayBase = snapshotAtSequence(0);
    const log = createCommandLogV1<
      ...
      FixtureFaultV1
    >({ replayBase, limit: 200 });
    const firstAttempt = finalizedAttempt(replayBase, 1);
    const entry = log.append(
      { ...parsedCommand(1), label: "first" },
      firstAttempt,
    );

    expect(entry.label).toBe("first");

    const protoMetadata = log.append(
      Object.fromEntries([
        ["source", "game"],
        ["command", { kind: "fixture.command", ordinal: parsePositiveSafeInteger(2) }],
        ["__proto__", "metadata"],
      ]) as MetadataCommandV1,
      finalizedAttempt(firstAttempt.result.snapshot, 2),
    );
    expect(Object.hasOwn(protoMetadata, "__proto__")).toBe(true);
    expect(protoMetadata["__proto__"]).toBe("metadata");

    expect(() =>
      createFixtureLog(replayBase).append(
        { ...parsedCommand(1), candidateRngAfter: "forged" } as never,
  ...
  it("keeps public entries separate while retaining replay-base identity", () => {
    const attempts = mixedAttempts(201);
    const log = createFixtureLog(attempts[0]!.finalizedAttempt.preSnapshot);
    for (const fixture of attempts) {
      log.append(fixture.parsedCommand, fixture.finalizedAttempt);
