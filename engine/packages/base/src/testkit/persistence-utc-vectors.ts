...
export const persistenceUtcAcceptedCorpusV1 = [
...
export const persistenceUtcRejectedCorpusV1 = [
...
function expectedObservationV1(
  ...
  return ({ input, outcome, ...(outcome === "accepted" ? { value: input } : {}) });
}

export const persistenceUtcAdmissionExpectedV1 = {
  accepted: persistenceUtcAcceptedCorpusV1.map((input) => expectedObservationV1(input, "accepted")),
  rejected: persistenceUtcRejectedCorpusV1.map((input) => expectedObservationV1(input, "rejected")),
};

function observeAdmissionV1(input: string): PersistenceUtcAdmissionObservationV1 {
  try {
    return ({ input, outcome: "accepted", value: parseIsoUtcInstantV1(input) });
  } catch {
    return ({ input, outcome: "rejected" });
  }
}
...
export function evaluatePersistenceUtcAdmissionVectorsV1() {
  return ({
    accepted: persistenceUtcAcceptedCorpusV1.map(observeAdmissionV1),
    rejected: persistenceUtcRejectedCorpusV1.map(observeAdmissionV1),
  });
}
