...
`trusting`。成为家人和建立信任阶段的其余宽度仍属于 M3 及之后，不能由这一条旅程代替。
...
截至当前 M3 切片，已实现的内容宽度是 8/16 个自主行为、脸/颈/背 3/8 类直接互动、背部梳理、逗猫棒 1/3
种玩具，以及
...
已接受结果、关系事实和有界记忆可以恢复。现有猫 GLB 和房间细节仍不是最终美术验收，音频也尚未加入。M3 将继续
