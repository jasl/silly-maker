...
HMR or Desktop production promotion. The private adapter remains default-off on
ordinary paths; only the separately qualified, explicit `app desktop-dev`
workflow selects it.
...
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R0    | Inspector Authoring Scene read, refresh, and CAS admission update one authoring document session; Motion metadata feeds the optional Visual ambient binding and referenced definitions feed read-only scrub. A saved Authoring Scene also recompiles its configured virtual module and follows the product's ordinary R2/R3 update boundary. Low-level Scene/Regions/Chrome ports remain tooling paths, not current Studio workspaces. | The static Player shell has no author/source update path.                                                                                                            |
...
[Deno 2.9.6](https://github.com/denoland/deno/releases/tag/v2.9.6) is the first
stable release whose source and behavior closed SillyMaker's accepted
`deno desktop --hmr` revalidation gate. Launch a configured Web application in
the maintained native development workflow from the repository root:

```sh
deno task app desktop-dev <application-id>
```

The command runs the official in-runtime Vite server from that application's
root, explicitly selects the package-private Desktop adapter, and keeps preview
records/downloads under the application's ignored
`tmp/sillymaker-desktop-dev/<application-id>/` directory. Ordinary `app dev`,
`app build`, and static `app desktop` remain separate and do not select or
activate this adapter. The launcher does not inspect a version string, use Deno's undocumented
framework marker, or create an external Vite proxy/companion. Current native
qualification is macOS arm64; this command does not promote Desktop persistence,
packaging, signing, crash durability, or multi-platform release support.
The exact bounded evidence and non-goals are recorded in the
[stable activation plan](plans/2026-08-28-deno-desktop-hmr-stable-activation.md).
...
deno task app desktop-dev <app>                       # Deno Desktop + official Vite HMR
...
`app desktop-dev` is the explicit maintained native HMR workflow described above
and remains separate from static packaging.
...
local numbers do not promote Desktop behavior. The AR5 mainline is delivered. The independent
Desktop HMR candidate subsequently passed stable Deno 2.9.6 source-and-behavior revalidation and
now has the explicit `app desktop-dev` workflow above; this did not reopen AR5 or promote Desktop
production behavior.
