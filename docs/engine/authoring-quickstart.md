...
- Start the application with `deno task author <application-id>` or `deno task dev`. The Vite dev page exposes the embedded author launcher; the same-origin standalone surface is `/__sillymaker/inspector/`. Both shells use the same Inspector/Authoring Host implementation and dirty/undo/save/conflict rules, but each shell mount owns its own memory.
...
- Authoring documents register nowhere: the Project Authoring Index scans both `*.authoring-scene.json` and `*.scene.json` plus Motion, Regions, and Chrome sources, admits them once into metadata/named skips, and records each scene's source kind. One lazy owner per Vite dev server shares the cached snapshot and refreshes one changed path. Inspector lists/reads/writes only the Authoring Scene authority through its port; other family ports do not imply current workspaces. `story check` is a one-shot consumer, not the dev server's in-memory owner.
...
For clickable zones, Inspector shows the selected object's projected region rows and real polygon/rectangle overlay read-only. Edit the Regions JSON/code path directly, or bootstrap from legacy bitmap judgment art with `deno task story regions trace <image.png> --out src/regions/<name>.regions.json`; then import the JSON, run `parseRegionsDocumentV1`, hand `regions` to `resolveContent`, and dispatch semantic invocations from `onHitRegionActivate`. A future writable Regions tool needs its own accepted consumer contract.
