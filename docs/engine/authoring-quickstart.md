...
Ordinary scene work — moving an actor, resizing it, changing which motion plays on an entrance — does not start in a TypeScript tree. Start the game (`deno task author <application-id>` or `deno task dev`). The Vite dev page exposes a lightweight **打开内嵌创作** launcher; **调试 → 工具 → Studio** still opens the same Authoring Host implementation in a same-origin standalone tab. Both shells use the same workspace/session/source-IO factories and dirty/undo/save/conflict rules; each mounted shell owns its in-memory Host lifetime, so separate tabs do not share an unsignaled draft instance. Use the **创作工作区** rail to switch Scene, Motion, Regions, Chrome, and Flow: only one workspace is visible, while a visited workspace keeps its local draft/undo state and shows **未保存** on the rail when dirty. The live game keeps running while saves apply through its admitted update path. The starter (`template`) and the Cat Cafe opening are live scene-managed references; Engine Lab is the maintained embedded/R2 conformance fixture.
...
- In either shell: drag geometry-declared actors on the Scene canvas (snap to edges/centers, corner handle scales), edit exact numbers in the inspector, rebind cue motions in the cue table, then select **Motion 工坊** to tune keyframes. When the Studio binding supplies an asset registry, the canvas preloads and draws the real backgrounds/poses (Cat Cafe does; asset-free stories keep code-native placeholders). Saving writes the scene/motion document through the dev-only CAS port; `git diff` should show only those JSON files. A digest conflict refreshes the saved baseline while retaining your dirty draft and undo history, so review and retry instead of reopening the document.
...
- Scene construction needs no code either (Authoring Architecture S4): the navigator's 新建场景 form writes a blank `src/scenes/<name>/<name>.scene.json`; the Content browser (the binding's `contents` manifest) adds declared backgrounds/characters as entries with derived stable tags and default placements; the inspector removes entries (dependent cues included) and edits appearance through structured selects (`appearanceFields`) — or, without any manifest registration, through free-text rows plus an add-key row (values gated by the stage appearance admission grammar); a read-only resolution panel shows the catalog's actual result for the selected entry (rendererId, geometry, assets in resolution order), and the 试穿 toggle previews appearance changes on the canvas only, never dirtying the draft; the cue table appends/removes show/hide cues and creates-or-clones a `*.motion.json` next to the scene, bound to that cue. Placeable content without declared geometry is flagged — it cannot be dragged, only numerically placed. In the Flow workspace, summaries and choice labels that reference shared `text.*` ids resolve through the binding's optional `resolveText` port.
...
- Authoring documents register nowhere: the Project Authoring Index scans the story tree by convention (`*.scene.json`, `*.motion.json`, `*.regions.json`, `*.chrome-layout.json`). One lazy owner per Vite dev server admits all four families on the first list, retains only navigator metadata/named skips, and refreshes one path after an add/change/unlink; the four Studio list views share that snapshot while an opened document is still read directly through its CAS port. `story check` is a separate one-shot consumer of the same enumeration/admission implementation, not a shared in-memory server. A new motion is the document itself plus the scene package's explicit import for runtime playback — the Studio binding lists no source paths, and inadmissible files surface as named warnings in Studio's diagnostics panel.
...
Scene-managed stories get the dev-only Authoring Host in both embedded and standalone form. The Scene workspace is a navigator over `*.scene.json` (plus 新建场景), a Content browser that adds declared backgrounds/characters as entries, a real-renderer canvas without playing there, a placement inspector (structured appearance + entry removal), and a cue table with motion bindings and create-or-clone. The separate **Motion 工坊** workspace edits the selected source/case. Actors with declared geometry are draggable right on the Scene canvas (snapping to canvas edges/centers, corner handle for scale), and in Motion clicking a timeline dot selects that keyframe so the ghost can be dragged to write its offsets — numeric inputs stay as the precision entry for both. Saves go through the CAS scene port and the admitted Vite update path refreshes the running game where that Story owns one; the Scene document stays the single authoring authority. Rail focus does not yet carry a typed scene/cue/motion/region/chrome/flow target into another workspace, so select the target within the destination workspace. Ordinary production builds exclude the Host, Studio binding, source-write endpoints, and real dev-source client — the embedded launcher is not a Player feature.
