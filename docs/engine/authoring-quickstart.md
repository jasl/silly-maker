...
- Use scene search to open an Authoring Scene, then use the virtualized layer/object tree or real Stage preview to select an object. Off-canvas, transparent, and group objects remain selectable through dashed ghost bounds. The Inspector can edit local transform, visual content, existing appearance values, sibling object order, and layer order. Saving writes through Authoring Scene CAS; a digest conflict refreshes the saved baseline while retaining the dirty draft and undo history for explicit retry.
- Hit regions, Motion, Timeline, interaction/GUI intent, compiled-layer and JSON-pointer source provenance are inspection facets. Region shapes render as a read-only preview overlay; Motion/Timeline scrub is detached presentation sampling. The Inspector does not create components, convert groups to visuals, edit standalone Regions/Chrome documents, edit Blueprint/code, or become a gameplay writer.
