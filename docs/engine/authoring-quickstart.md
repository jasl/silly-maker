...
- Start the application with `deno run dev` from its directory or `deno task app dev <application-id>` from the repository root. The Vite dev page exposes an “打开内嵌制作” action; when the application selects the reference DevDock it shares that movable development panel, otherwise it uses an authoring-only fallback. The same-origin standalone surface is `/__sillymaker/inspector/`. Both shells use the same Inspector/Authoring Host implementation and dirty/undo/save/conflict rules, but each shell mount owns its own memory.
- In `sillymaker.config.ts`, opt in with `inspector: { module, exportName }`. Export an `InspectorBindingV1` containing the content catalog, real renderers, and only the optional assets/Motion/Timeline catalogs the preview needs. If the binding constructs application-owned dev-only resources such as an asset registry, release them through its optional synchronous or asynchronous `dispose()`; the Inspector composition waits for it on rollback, replacement, and final close. Do not add source paths, a Session writer, or a second document store to the binding.
- Use scene search to open an Authoring Scene, then use the virtualized layer/object tree or real Stage preview to select an object. Off-canvas, transparent, and group objects remain selectable through dashed ghost bounds. The Inspector can edit local transform, visual content, existing appearance values, one Visual ambient Motion binding/phase, sibling object order, and layer order. Ambient choices come from the project's Motion index; changing the binding does not edit the Motion document. Saving writes through Authoring Scene CAS; a digest conflict refreshes the saved baseline while retaining the dirty draft and undo history for explicit retry.
- Hit regions, Motion definitions/cue transitions/other bindings, Timeline, interaction/GUI intent, compiled-layer and JSON-pointer source provenance remain inspection facets. Region shapes render as a read-only preview overlay; Motion/Timeline scrub is detached presentation sampling. The Inspector does not create components, convert groups to visuals, edit standalone Regions/Chrome documents, edit Blueprint/code, or become a gameplay writer.
...
- Closing the embedded shell with a dirty Authoring Scene asks **保存并关闭 / 放弃并关闭 / 取消**. Focused text/numeric editing owns keyboard input and does not trigger gameplay shortcuts.
- Low-level Scene, standalone Regions/Chrome documents and full Motion keyframe editing remain direct data/code or focused-tool workflows; the old Studio five-workspace shell is gone. `MotionWorkbenchV1` remains reusable outside Inspector when a focused writable Motion tool is needed. Narrative Flow remains a derived read-only graph contract with no current Inspector workspace.
- Authoring documents register nowhere: the Project Authoring Index scans both `*.authoring-scene.json` and `*.scene.json` plus Motion, Regions, and Chrome sources, admits them once into metadata/named skips, and records each scene's source kind. One lazy owner per Vite dev server shares the cached snapshot and refreshes one changed path. Inspector lists/reads/writes only the Authoring Scene authority through its port; other family ports do not imply current workspaces. `app check` is a one-shot consumer, not the dev server's in-memory owner.
...
For a real game-specific Scene authoring tool, add one cohesive React module and select it through
`InspectorBindingV1.sceneInspector.properties`. A contribution receives the current admitted Scene, read-only
facets and selection plus `selectObject` and `execute`. `execute` accepts the public
`SceneAuthoringOperationV1` vocabulary and binds the call to the render's exact document identity/draft revision;
stale callbacks reject without changing the newer draft. The contribution does not receive the Authoring Host,
document Session, source IO, Save, or gameplay authority. Use a separate focused tool contract for another
document family instead of turning this slot into a generic editor registry. The Template intentionally contributes
no tool until a product needs one.
...
deno task app simulate e2e --scenario calibration   # play the full narrative without a browser, JSON output
...
One application = one `WebGameApplicationV1` declaration + one `startWebGameApplicationV1` call. Start from `template` (copy the directory + global rename; the copy is a complete project — `sillymaker.config.ts`, `vite.config.ts`, `tools/app.mts` — and inside this repository you additionally add its directory to the root `project.config.ts` list); the full reference is `e2e/src/application/`. A curated repository example also completes the product contract in [`examples/AGENTS.md`](../../examples/AGENTS.md#reference-application-product-contract); copying an application skeleton is not that acceptance. Application-directory conventions: `composition.tsx` (projector/slots/the `*GameApplicationV1` declaration), `ui.tsx` or `shell-ui.tsx` (runtime exports are PascalCase React components only, in a separate file from the application declaration and non-component registries for Vite Fast Refresh), `core-application.ts` (headless instance factory), `entry.tsx` (boots from composition). Builds are application tasks; the application CLI carries diagnostics (app-locally via `deno task app <verb> .`, or at the repository root once the directory is listed in `project.config.ts`):
...
input with `selectNarrative`, `dispatchResolution`, nullable `dispatchTime`, a
dialogue `renderer`, nullable `history`, `resolveText`, nullable
`replayCurrentVoice`, and the optional current-voice playback query; use
...
return the definition as `ui.narrative`. The dialogue renderer and optional
separate History renderer use only their immutable public props and bounded
callbacks; never add `slots.narrative`, import the
...
game state owns the primary, or
...
deno task preview
deno task app inspect <app>    # resolved identity and program report
deno task app check <app>      # structured Story diagnostics
deno task app simulate <app> [--scenario s] [--seed n]
deno task app dev <app> --smoke
...
| `ui.narrative_surface_definition_invalid`                                                                          | the definition input is not the admitted public shape, omits the required nullable History feature port, or contains an invalid callable; copy the template construction and choose either the full VN preset or the focused core-only preset                                      |
| `ui.whole_canvas_surface_definition_invalid`                                                                       | the WholeCanvas definition/catalog does not satisfy its admitted public shape; use the query-gated Engine Lab construction as the focused reference                                                                                                                                |
...
| `motion.document_invalid` / `motion.document_json_invalid` (`app check`)                                           | a `src/**/*.motion.json` fails strict Motion admission or is not JSON; fix the file the diagnostic points at                                                                                                                                                                       |
| `motion.id_duplicate` / `motion.id_filename_mismatch` (`app check`)                                                | two motion files claim one id / the filename stem is not the id's final segment; rename so click-to-locate and the write port stay stable                                                                                                                                          |
| `scene.document_invalid` / `scene.document_json_invalid` (`app check`)                                             | a `src/**/*.scene.json` fails strict Scene admission or is not JSON; fix the file the diagnostic points at                                                                                                                                                                         |
| `scene.id_duplicate` / `scene.id_filename_mismatch` / `scene.cue_motion_missing` (`app check`)                     | two scene files claim one id / the filename stem drifts from the id / a cue references a motion no `*.motion.json` declares                                                                                                                                                        |
...
| `scene.ambient_motion_missing` (`app check`)                                                                       | an entry's `ambient` loop references a motion no `*.motion.json` in this source tree declares — the loop would silently never play                                                                                                                                                 |
| `regions.document_json_invalid` / `document_invalid` / `id_duplicate` / `id_filename_mismatch` (`app check`)       | a `src/**/*.regions.json` is not valid JSON, fails strict admission, two files claim one regionsId, or the filename stem is not the id's final segment; fix the file the diagnostic points at                                                                                      |
| `stage.hit_region_polygon_invalid` / `stage.hit_region_hover_invalid` (projection diagnostics)                     | a region's `polygonPoints` break the shape rules (3–64 integer vertices inside the box, non-zero area) or its `hoverAssetId` is invalid; the region degrades to its rectangle / drops the hover — repair the regions document (Inspector shows the projected real shape read-only) |
| `regions.trace_image_invalid` / `regions.trace_budget_unreachable` (`app regions trace`)                           | the input is not a supported PNG (needs RGBA, gray+alpha, or palette+tRNS; no interlacing — `details.reason` names the problem; re-export the image) / the silhouette cannot simplify into `--max-vertices`; raise the budget                                                      |
| `chrome_layout.document_json_invalid` / `document_invalid` / `id_duplicate` / `id_filename_mismatch` (`app check`) | a `src/**/*.chrome-layout.json` is not valid JSON, fails strict admission, two files claim one layoutId, or the filename stem is not the id's final segment; fix the file the diagnostic points at                                                                                 |
...
| `scene.cue_binding_scope_collision` (`app check`)                                                                  | a declared presentation (motion or explicit cut) shares a stage edge with a **bare** cue; without dispatch context the fallback matches the edge, not the cue — declare the bare cue explicitly (`cut: true` or the same motion) instead of forking the stage identity             |
...
The current Inspector can select/remove one Visual ambient Motion reference, tune its phase, and offer a detached scrub; it does not write keyframes, cue transitions, other bindings, or Timeline references. For focused human Motion tuning, use the retained reusable `MotionWorkbenchV1` and shared Motion CAS/session path: scrub/play, edit duration/delay/keyframes, A/B against saved, then save. DevDock provenance remains read-only under the Game root. Saving is compare-and-swap against the file digest, rewrites only that motion file deterministically, and marks it `authoring.status: "human_tuned"`. Diagnostic panels still declare their stage behavior through `DevDockPanelV1.stage`: click-to-inspect stays `live`, while a panel that must inspect a transient frame sets `stage: "frozen"`.

The current embedded and standalone Authoring Host presents only Inspector. It opens `*.authoring-scene.json`, not low-level Scene, and saves through the Authoring Scene CAS port. Ordinary production builds exclude the Host, Inspector binding, source-write endpoints, and real dev-source client. For an `authoring_scene` release build, Vite additionally excludes its source JSON, admission/compiler, inspection facets, and non-Vite fallback, retaining only the virtual runtime plan.

Stories may still emit the read-only `NarrativeFlowGraphV1` projection for checks or a future focused view, but `InspectorBindingV1` has no Flow field and current Inspector does not render it. Do not recreate the old rail/workspace wrapper; any future Flow UI must consume the derived graph without becoming a second author authority.

For clickable zones, Inspector shows the selected object's projected region rows and real polygon/rectangle overlay read-only. Edit the Regions JSON/code path directly, or bootstrap from legacy bitmap judgment art with `deno task app regions trace <image.png> --out src/regions/<name>.regions.json`; then import the JSON, run `parseRegionsDocumentV1`, hand `regions` to `resolveContent`, and dispatch semantic invocations from `onHitRegionActivate`. A future writable Regions tool needs its own accepted consumer contract.
...
writable Motion work in the focused Workbench; DevDock provenance and Inspector
scrub may locate/sample source but must not create a second editor session under the Game root.
...
- Skin and layout use only the published tokens: colors/spacing/radii/sizes are `--silly-color-*`, `--silly-space-*`, `--silly-radius-*`, `--silly-control-min-size` (control chrome), `--silly-target-min-size` (touch-target floor) — defined in `theme/tokens.css` of `@sillymaker/ui`. The default palette is professional-neutral dark with no brand hue; Stories theme by overriding the tokens under their `[data-application-id]` scope. Product fonts, palette, control recipes, and third-party component CSS must also stay under that root: never put them on `:root`, `html`, `body`, bare controls, or an unscoped universal selector. An application-owned Host may still set document geometry (`html`/`body`/`#root` size, margin, overflow) and whole-document `color-scheme`, without carrying product styling into sibling tools. A product `@font-face` uses a namespaced family and is activated only by the product root. Inspector, DevDock, Embedded Authoring, and game-specific tool extensions inherit the independent `--silly-tool-*` Tool Theme; do not restyle them through product tokens. Tailwind-style product builds disable document-global Preflight/component reset.
