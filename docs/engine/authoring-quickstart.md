...
- In `sillymaker.config.ts`, opt in with `inspector: { module, exportName }`. Export an `InspectorBindingV1` containing the content catalog, real renderers, and only the optional assets/Motion/Timeline catalogs the preview needs. Do not add source paths, a Session writer, or a second document store to the binding.
