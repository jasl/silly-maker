...
- Start the application with `deno run dev` from its directory or `deno task app dev <application-id>` from the repository root. The Vite dev page exposes the embedded author launcher; the same-origin standalone surface is `/__sillymaker/inspector/`. Both shells use the same Inspector/Authoring Host implementation and dirty/undo/save/conflict rules, but each shell mount owns its own memory.
