...
状态：S4.3.1b 已完成生产 Narrative cutover。Story 以
...
内置包、Story、Hotfix 和外部资源最终都解析为同一类 manifest entry，但图片、音频、视频和字体保持不同的类型合同，不能伪装成 image。媒体 provider 声明逻辑路径、类型及必要的尺寸/拓扑；作者不维护精确 byte size 或 digest receipt。仓库 build/check 校验适用的图片类型与尺寸，runtime Host 负责 fetch/decode 并在失败时进入 fallback。若未来的分发、签名或 updater 边界需要内容哈希，应从实际构建或 transport 输出派生，而不是要求内容作者同步维护。
...
## 12. Migration from the first V1

下列是首个 PoC 偶然形成、现已替换的历史 exact-key contract：
...
替换时应明确 state-contract identity 变化或一次性迁移；不得静默向 exact schema 增字段。（route/variant 平行权威已随 PoC 删除。）
...
- typewriter、advance、choice、history、seen、auto、skip 和 voice replay；
