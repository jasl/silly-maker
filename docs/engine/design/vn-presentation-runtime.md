...
- `pause`：明确 duration/skip policy 或 semantic resume——正被 `hold`
  归并（时间轴与 Host 决定轴不再横跨），随 hold 车道 M1 显式迁移并删除；
