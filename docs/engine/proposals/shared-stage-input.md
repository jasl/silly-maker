...
写），focused Host 矩阵 + 浏览器真实指针 e2e（原生 inert 强制、点区决
...
4. **pending 由 Story 铸造。** base 不铸 pending（narrative runner
   是 Story 侧：Lab `e2e/src/gameplay/narrative-runtime.ts` 与
   `e2e/src/gameplay/narrative-units/drill.ts`、实验仓
...
- `presentation_barrier` 不收：Barrier 是引擎自动确认的呈现结算边
  ...
  常拒）。
- 该成员作为 pending 普通数据自然进入 Snapshot、Save、canonical
  bytes、digest 与 replay 完整性；它不参与权威算术、决议合法性、
  `when` 臂求值，也不建立独立的 digest/replay 分支。它是 Host 提示，
  与 `pace` 同一信任级。
...
- **ui host（focused DOM test）**：shared choice → gameplay 层无 `inert`，下层
