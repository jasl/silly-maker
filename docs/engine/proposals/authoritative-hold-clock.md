...
外部实验仓（私有）在引擎零改动下收完可绑该句的 STORY。其活清单把剩余
1:1 分成：

- **E1**：两句台词之间画面停着，原作计时事件按帧走，期间可换帧、画
  `■□` 条、问警戒/剩余时间，到点再续。
- **E2**：持有期滴权威（若干「每 N 秒 `+=1`」的计量：好感/警戒/进度
  族）。是 E1 的姐妹命令，不是第二套钟。
...
- 入院复用既有 duration 入院（safe integer、≥1；`pause` 现行家族顶
  600,000ms 一并沿用，实测真实内容最长 470 帧 ≈ 7.8s）。**不发明
...
| 钟                                   | 拥有                                               | 不做             |
| ------------------------------------ | -------------------------------------------------- | ---------------- |
| 权威 `remainingMs`                   | Save、回放、六格条 / HUD ％、到期与条件去向、E2 滴 | 读 `Date.now()`  |
...
与 hold 长到一起；六格条与进度％读 hold 的剩余毫秒（或同一 commit 写
下的权威域），不开第二口权威钟。
...
Engine Lab `cal-hold` 滴 rapport 与实验仓一条每 5s 权威滴（含 5s 阈值
换帧）是首两条活路径。其余滴表仍是 Story 刀。
...
中段换帧（进度帧序列、双帧交替）是后切片：按经过毫秒的阈值穿越应用已
声明的 stage 批，与滴同一结算法。现有 6 本墙钟 flipbook 不在
