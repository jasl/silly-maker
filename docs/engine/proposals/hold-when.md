...
720 例全绿）。第二条候选真中止路径（警戒抓包）于 2026-08-22 由实
验仓刀 #339 证伪：CE249 是窗尽 / WAIT 100 后再问，CE277 不写警戒；
挂 `when` 会跳过 `V354++` 或 100f，比现有窗后 branch 更不忠实。不
是未接活路径，也不再是引擎 defer。q1（hold 中热区，输入轴）已由
[mid-hold-input](mid-hold-input.md) 车道认领并同日交付关闭
（2026-08-22：Lab conformance + 实验仓 CE18 嘴区活路径）。
...
CE251 素股第一票后 SHOW `bg_001_01`）。开门时写在活清单上的两条候
选里，只有一条是 hold `when` 真中止：

- **深夜房条中途觉醒 / 厌恶**（已接）：CE281 8×60f 持有期滴睡眠深
  度与嫌恶，中途越线必须离开条、进觉醒/厌恶，不能等到期。
- **警戒抓包**（2026-08-22 证伪，实验仓刀 #339）：原假设是「偷窥 /
  警戒条持有期间警戒升到门槛 → 改道抓包，不能把 WAIT 走完再问」。
  解码 CE249 / CE277 后不是：窗尽 / WAIT 100 后再问；CE277 只画
  50f 条、不写警戒。挂 `when` 会跳过 `V354++`（抓包图错档）或
  100f。已有窗后 branch 就是原作。

真中止是「同一口钟上的状态谓词」，不是第二套叙事 VM，也不是 Host
按钮。hold 中热区（条还在走时点身体）是**输入轴**，见 open
question q1——已由 mid-hold-input 关闭，V1 不靠它开门。
...
果驱动的臂」承诺——开门时写的两条证据路径里，睡眠/嫌恶滴是这种，
警戒抓包后来证伪、并不滴警戒；监视器驱动的臂是「下一结算」粒度，
与监视器系统自身已接受的陈旧度一致。V1 不为合并时间线重写监视器
结算。
...
- 双消费者：Engine Lab 锁两种粒度（自身 tick 同瞬间截断 + 监视器
  下一结算）；实验仓接一条真中止活路径（深夜房条中途觉醒/厌恶已
  接；警戒抓包后证伪，见交付记录）。

## Open questions（建议随 admission 裁决）

1. **hold 中热区**（条还在走时激活身体区）是输入，不是时间派生谓
   词。V1 当时**显式 defer**；2026-08-22 已由
   [mid-hold-input](mid-hold-input.md) 关闭。觉醒条已接，警戒抓包
   证伪。不把 regionId 塞进 hold resolution。
