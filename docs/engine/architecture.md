...
| Application packages      | `.` per package                                                                                                                                                                                                                                                                                         | `template/` (the minimal game starter, MIT) and `examples/*` (bookshop, cards, cat-cafe, silly-os) each compose one self-contained application project (`sillymaker.config.ts` + `vite.config.ts`); Cards is the maintained neutral GUI-only product and carries no Story authority. The root `project.config.ts` only lists their directories for repository-level aggregation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
Cards is the first maintained consumer;
its final receipt excludes the Base/UI/Web root barrels as well as all omitted
Game and outer-capability owners.
