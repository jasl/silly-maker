...
VN Reference Tour 只完成 M0 外壳，不能在完整产品门关闭前视为旗舰或可玩产品参考。
...
| Application packages      | `.` per package                                                                                                                                                                                                                                                                                         | `template/` (the minimal game starter, MIT), `examples/bookshop` (focused Narrative teaching example), and GUI-only `examples/silly-os` each compose one self-contained application project (`sillymaker.config.ts` + `vite.config.ts`). The independent `examples/vn-reference-tour` WIP has delivered its M0 package, frozen denominator, supported-export shell, and starter-domain deletion; its M1 complete script/two-Scene author data is not implemented yet. The discontinued Electronic Pet and retired Cat Cafe packages are absent from the active workspace. The root `project.config.ts` only lists application directories for repository-level aggregation. A small tooling-owned application fixture—not a published example—keeps GUI-only startup and final-graph conformance independent of product content.                                       |
