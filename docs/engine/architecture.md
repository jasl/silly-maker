...
Settings 也已关闭。M4 已开始作者接手与产品证据，Template 与该产品显式选择同一 preset。完整产品门关闭前，
VN Reference Tour 仍不能视为旗舰或完整产品参考。
...
| Application packages      | `.` per package                                                                                                                                                                                                                                                                                                                                                                                                                                | `template/` (the minimal game starter, MIT), `examples/bookshop` (focused low-level Narrative renderer example), and GUI-only `examples/silly-os` each compose one self-contained application project (`sillymaker.config.ts` + `vite.config.ts`). The `examples/vn-reference-tour` WIP has delivered M0–M3 and started M4 with Agent plus owner-authorized Computer Use-assisted participant handoffs over the same ambient-binding Inspector/CAS path; it and Template select the full default VN Player, while current-low-end qualification and remaining product evidence stay open. Bookshop's later teaching role is reviewed only after VN completion. The discontinued Electronic Pet and retired Cat Cafe packages are absent from the active workspace. The root `project.config.ts` only lists application directories for repository-level aggregation. A small tooling-owned application fixture—not a published example—keeps GUI-only startup and final-graph conformance independent of product content.                                                                                                                                                                                                                                                                                                                                                                                                            |
