...
enters its existing BuildIdentity/simulation digest. A separate private selection
controller can replace that complete immutable set inside one application
generation: it stages the candidate through the same Direct lifecycle, waits for
the application-owned consumer publication, and retires the predecessor only
after acknowledgement. This controller and the separate DevDock publication seam
currently prove a private hot-swap substrate in neutral contract tests; no product
connects them end to end yet, and the VN does not fabricate such a consumer.
Authoritative gameplay changes still require an R2 Application/Game successor and
the existing exact Save/lease handoff.

Neither entry discovers packages, resolves a public Mod graph, executes arbitrary
post-release code, owns State/Save/digest, or sandboxes trusted same-realm
JavaScript. Code loaders and extension-point compilers are cold, resource-free
staging functions by contract; any reversible resource belongs in the existing
Direct lifecycle. Products that do not select this private entry remain complete
and structurally exclude the Mod and Direct implementation modules.
