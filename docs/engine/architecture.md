...
conformance 与自动化。仓库内的 Story/application packages 是使用它开发的具体产品；当前没有旗舰。
VN Reference Tour 已交付 M0–M3；引擎维护的 focused default VN Player 已提供贴底对话层、
...
代表性真实 current-low-end qualification 是唯一开放门槛。Template 与该产品显式选择同一 preset；
M4 尚未关闭、M5 尚未开始，VN Reference Tour 仍不能视为旗舰或完整产品参考。
...
| `@sillymaker/ui`          | `.`, `./assets`, `./code-surface`, `./debug`, `./debug/dev-source-client`, `./diagnostics`, `./input`, `./native-behavior`, `./narrative-player`, `./narrative-player/core`, `./reference`, `./reference/dev-dock`, `./reference/settings`, `./styles.css`, `./viewport`; workspace-only `./internal`, `./internal/dev-dock-host`, `./internal/development-tool-launcher`, `./internal/embedded-authoring-launcher`, `./code-surface/internal` | React shell, the single-owner GameViewport (fit/fluid/single-axis expansion plus container layout variants), UI composition and default GameRoot, stage, characters, assets, interaction/input, overlays, Narrative/WholeCanvas presentation, generic System/settings hosts, semantic/presentation bridges, recovery UI, the development-conditional source client, and the published global theme stylesheet. The focused `./narrative-player` entry is the explicitly selected, engine-maintained full VN preset; `./narrative-player/core` omits its optional History presentation while retaining dialogue, playback and input policy. Both stay outside the generic root and non-VN final graphs. The focused `./code-surface`, `./input`, `./native-behavior`, and `./viewport` entries let neutral GUI products select only their actual presentation owners; `./code-surface/internal` owns GUI-composition unit residency. The public reference DevDock load handle plus package-private lightweight DevDock Host and development/embedded-authoring launcher bridges provide explicit lazy ownership and one movable development panel without loading Studio or the full DevDock implementation in the resident graph. The root entry does not re-export optional Code Surface/reference implementations. The focused `./reference/*` entries form the explicit copy/eject-friendly DevDock plus preset settings surface. |
...
| Application packages      | `.` per package                                                                                                                                                                                                                                                                                                                                                                                                                                | `template/` (the minimal game starter, MIT), `examples/bookshop` (focused low-level Narrative renderer example), and GUI-only `examples/silly-os` each compose one self-contained application project (`sillymaker.config.ts` + `vite.config.ts`). The `examples/vn-reference-tour` WIP has delivered M0–M3 and is executing M4: Agent plus owner-authorized Computer Use-assisted participant handoffs share the ambient-binding Inspector/CAS path, and independent review corrected the historical 59/44 script to the current 110/82 denominator. It and Template select the full default VN Player. Engine review and Starter classification are complete with no additional Template change, and independent product re-review passed without a product-integrity blocker. Representative real current-low-end qualification is the only open gate; M4 is not closed and M5 has not started. Bookshop's later teaching role is reviewed only after VN completion. The discontinued Electronic Pet and retired Cat Cafe packages are absent from the active workspace. The root `project.config.ts` only lists application directories for repository-level aggregation. A small tooling-owned application fixture—not a published example—keeps GUI-only startup and final-graph conformance independent of product content.                                                                                                   |
