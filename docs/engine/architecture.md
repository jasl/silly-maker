...
conformance 与自动化。仓库内的 Story/application packages 是使用它开发的具体产品；
`examples/vn-last-sound-check`（《最后一次试音》）是维护中的旗舰产品，并已完成 M0–M5。引擎维护的 focused default VN Player 已提供贴底对话层、
全画布推进、History/播放控制、Ctrl/Tab/H/鼠标中键、最终媒体与产品矩阵，产品入口、Save/recovery 与
Settings 也已关闭。M4 已完成 Agent 与所有者授权 participant 作者接手；独立审查把历史 59 unique /
44 per route 修正为当前 110 unique / 82 per route。独立 engine review 与 Starter feedback classification
已完成且不需要追加 Template 改动，独立 product re-review 也已通过且没有 product-integrity blocker。
2026-08-29 所有者因没有合适设备，取消代表性真实 current-low-end qualification 作为本产品完成门槛；
该资格未执行、未通过，也不形成低端设备支持声明。M4 按缩减后的接受范围关闭，M5 完成旗舰与发布接线
切换，但不声称远程 live deployment。Template 与该产品显式选择同一 preset。
...
private extension runtime 只由产品显式选择的 lazy contribution 或 public Mod graph 间接引入，既可服务
first-party，也可服务应用在 build-time 明确信任并安装的仓外 package。ordinary no-extension application
会从最终依赖图完全排除它。`@sillymaker/agent` 只依赖 Base 和 React peer；focused public
`./session` 提供中立 Session/Run client/connector，workspace-private `./internal` 保留 Agent Host、
`UiArtifact` 与 deterministic fake。Inspector core publication 与 embedded
...
| `@sillymaker/agent`       | public `./session`; workspace-only `./internal`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | The focused public entry owns the transport/provider-neutral Session/Run client, semantic connector/connection ports, bounded response/event admission, observable status, run currentness, ordered stream, cancellation/reconnection, diagnostics, and awaited disposal. The private entry retains the deterministic fake, observable Agent GUI/session Host, retained `UiArtifact` revisions, admitted `UiIntent`, and the closed React renderer. Neither entry owns a real backend/provider wire, persistence, tool execution, source/Game writer, OpenUI/A2UI adapter, or external effect.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `@sillymaker/composition` | `.`, `./legacy`, `./state`, focused public `./mod`; workspace-only `./internal/extension-runtime`                                                                                                                                                                                                                                                                                                                                                                                                                              | Maintained cold-path façade for typed plugins, profiles, services, registries, direct-plan compilation, authoritative registration sealing, live reload, reversible staging-safe in-process lifecycle effects, and the neutral State-module registry bridge. The public `./mod` entry admits trusted build-time metadata once, resolves an explicitly selected immutable set, cold-compiles application-owned typed extension points, and replaces complete selections through acknowledged successors. It reuses the private Direct lifecycle but exposes no Context, service locator, Session, State, Save, Host, source IO, package discovery, or arbitrary-code installer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `@sillymaker/studio`      | `.`, `./composition`; workspace-only `./internal/authoring-companion`, `./internal/agent`                                                                                                                                                                                                                                                                                                                                                                                                                                      | The dev-only Inspector, Authoring Host, and standalone/embedded shells. It owns virtualized Authoring Scene and layer/object navigation, real-renderer preview plus authoring ghost overlays, bounded transform/appearance/order and Visual ambient-binding edits over the shared document session/CAS/history, read-only facets and Motion/Timeline scrub, persistent R1 publication, and a package-private neutral single-companion bridge. The root exports one focused build-known `sceneInspector.properties` contribution surface plus the existing Scene operation vocabulary/result; tools receive only current Scene/facets/selection and a revision-fenced execution port, never Host/Session/source IO/Save or the private lifecycle backend. Motion definitions, cue transitions, and other binding references remain read-only. `./internal/authoring-companion` lets one product decorate its binding without exposing companion lookup or a registry; `./internal/agent` explicitly selects Engine Lab's experimental Agent and is absent from the core Authoring graph. Vite serves the standalone Inspector route and a lazy embedded launcher; Player and ordinary runtime never import Inspector/source-write implementation. The old Studio workspace shell is not retained.                                                                                                                                                                                                                                                                                                                                                                    |
| `@sillymaker/ui`          | `.`, `./assets`, `./chrome`, `./code-surface`, `./debug`, `./debug/dev-source-client`, `./diagnostics`, `./input`, `./native-behavior`, `./narrative-player/core`, `./narrative-player/history`, `./presentation-run`, `./reference`, `./reference/dev-dock`, `./reference/settings`, `./styles.css`, `./viewport`; workspace-only `./internal`, `./internal/dev-dock-host`, `./internal/development-tool-launcher`, `./internal/embedded-authoring-launcher`, `./internal/narrative-player-preset`, `./code-surface/internal` | React shell, the single-owner GameViewport (fit/fluid/single-axis expansion plus container layout variants), UI composition and default GameRoot, stage, characters, assets, interaction/input, overlays, Narrative/WholeCanvas presentation, hosted chrome widgets, presentation-run ownership, generic System/settings hosts, semantic/presentation bridges, recovery UI, the development-conditional source client, and the published global theme stylesheet. The focused `./narrative-player/core` and `./narrative-player/history` entries are lower-level VN presentation primitives; the workspace-private `./internal/narrative-player-preset` retains the cohesive implementation used to prove them, but UI no longer owns a public full VN preset. Official genre composition belongs to `@sillymaker/vn`, and all VN entries stay outside the generic UI root and non-VN final graphs. The focused `./chrome`, `./code-surface`, `./input`, `./native-behavior`, `./presentation-run`, and `./viewport` entries let neutral GUI products select only their actual presentation owners; `./code-surface/internal` owns GUI-composition unit residency. The public reference DevDock load handle plus package-private lightweight DevDock Host and development/embedded-authoring launcher bridges provide explicit lazy ownership and one movable development panel without loading Studio or the full DevDock implementation in the resident graph. The root entry does not re-export optional Code Surface/reference implementations. The focused `./reference/*` entries form the explicit copy/eject-friendly DevDock plus preset settings surface. |
| `@sillymaker/vn`          | `./interaction`, `./ui`, `./ui/core`, `./history`, `./preset`                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Official first-party VN genre composition. `./interaction` owns the generic interaction document/compiler/runtime policy and its Base NarrativeGraph lint/prediction projection while accepting product-owned typed predicates, effects, asset dependencies and source references. `./ui/core` exposes only the focused core Player; `./ui` is its umbrella plus the cold-path History presentation bridge, without selecting the History implementation. `./history` supplies the optional default History renderer/open control, and `./preset` is the conventional static Core + History composition. The package composes Base and lower-level UI contracts; it does not own a second Narrative, State, Save, replay, Session, text/content, source-writer, service-container, or public Mod authority. Products keep their content, theme, special surfaces and authoritative game rules.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Application packages      | `.` per package                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `template/` (the minimal game starter, MIT), GUI-only `examples/silly-os`, and the maintained flagship `examples/vn-last-sound-check` each compose one self-contained application project (`sillymaker.config.ts` + `vite.config.ts`). One Last Sound Check and Template select the full default VN Player. The flagship's Agent plus owner-authorized participant handoffs share the ambient-binding Inspector/CAS path, and independent review corrected the historical 59/44 script to the current 110/82 denominator. Engine, Starter, and product reviews are complete. The owner cancelled representative real current-low-end qualification as a completion gate because no suitable device was available; it was not run or passed and creates no low-end claim. M4/M5 are closed, with static publish wiring but no claimed remote live deployment. Bookshop retired in the explicit post-completion review. The discontinued Electronic Pet and retired Cat Cafe packages are absent from the active workspace. The root `project.config.ts` only lists application directories for repository-level aggregation. A small tooling-owned application fixture—not a published example—keeps GUI-only startup and final-graph conformance independent of product content.                                                                                                                                                                                                                                                                                                                                                                                    |
...
The focused public `@sillymaker/composition/mod` entry is a cold application
composer over that same private Direct lifecycle. `defineSillyModMetadataV1`
performs the one metadata admission; the runtime then trusts that typed result.
Its application-provided catalog contains admitted data contributions or a
literal trusted-code loader. Only the explicitly selected immutable set is
loaded, dependency-ordered, checked against application-owned typed extension
points, and compiled before any lifecycle child mounts. Unknown targets,
kind/collision errors, missing/cyclic dependencies, incompatible API ranges,
load errors, and compile errors reject the candidate before publication. A
lifecycle setup failure uses Direct's existing rollback. Explicit async resource
handles are awaited during rollback, predecessor retirement, and application
close.

The selection controller replaces a complete immutable set inside one
application generation: it stages the candidate through the same Direct
lifecycle, waits for application-owned consumer acknowledgement, and retires the
predecessor only after acknowledgement. One Last Sound Check uses this public
entry for its optional History presentation Mod in development. This is an R1
presentation consumer, not permission to rewrite a live Simulation graph.
Authoritative gameplay changes still require an R2 Application/Game successor
and the existing exact Save/lease handoff.

The public entry does not discover installed packages, scan directories, resolve
npm at runtime, execute arbitrary post-release code, own State/Save/digest, or
sandbox trusted same-realm JavaScript. Package selection and literal loaders are
ordinary application build inputs. Products that do not import `./mod` remain
complete and structurally exclude it and the private Direct implementation.
One Last Sound Check separately proves a product-specific post-release
declarative text/asset override path: bounded same-origin bytes target named
product slots, and a complete Web application successor applies the admitted
selection. That Artifact format is not a general executable Mod container or a
public authoritative-gameplay R2 API.
...
typed Inspector binding. The workspace-private
`@sillymaker/studio/internal/authoring-companion` entry exposes the decorator,
owner/definition/render-input types, and the existing conflict-refresh helper;
lookup, retention, and replacement remain
Studio-owned, so it is not a registry or discovery API. Core publication sees
only a neutral compatibility ID, content signature, owner factory, renderer, and
async disposer; Agent Host/UiArtifact types stay behind the explicit private Agent entry, while
the neutral Session client contract is imported from public `@sillymaker/agent/session`.
Compatible R1 candidates reuse the same companion owner, while a changed
compatibility ID or content signature rejects before owner replacement. If
candidate and visible rollback rendering both fail, the terminal publication
poisons once and retires the companion and Authoring owners once. This is a
single selected sibling seam, not a registry, public plugin surface, or Mod ABI.

The embedded shell is activated from a dev-only action. That action joins the
reference DevDock's movable development panel when selected and otherwise uses
a small authoring-only fallback; both paths address the same mounted shell. The
shell stays mounted while merely hidden. Its independent application-focus owner keeps
...
the atomic Visual ambient Motion binding/phase and other structural/reference
edits remain individual steps. The local adapter
...
### Public Agent Session/Run and private Host/UiArtifact seam

Focused public `@sillymaker/agent/session` exposes the transport/provider-neutral client plus
semantic connector/connection ports. The client owns observable
...
JSON `output_data`, `run_completed`, and `run_failed`; connection generation remains an internal
fence rather than public state. The client keys each submitted run by `(sessionId, runId)` and
accepts only that tuple's next contiguous positive sequence; a
...
fenced before a Host observer sees it. A connect replacing an operation-failed connection closes
...
and a bounded local history of retained Artifact revisions. Local cancel first marks the Host's
presentation run `cancel_requested`, so deliberately late output cannot publish, while the public
Session client retains the tuple until a remote terminal event. A malformed or
...
Engine Lab's explicit private Agent companion decorates its typed Inspector binding with one public
Session client factory, one private fake connector, and pre-admitted AR2 Scene operations. The
private Agent Host interprets `output_data` as its admitted Artifact successor; the Session contract
does not own that meaning. The embedded surface keeps Artifact actions
...
Its four checked-in canonical records form the explicit supported release
corpus; they do not claim capture from a historical public release or imply
support for any unlisted identity. Cat Cafe revision 1 left the corpus when the
product retired; no successor product adopts or migrates that identity.
...
| R2    | Engine Lab owns a literal composition self-accept boundary with real owner-injected `BuildIdentity`; the Web composer transfers an exact Save + lease pair and replaces Game/Session on the same Host/root. Engine Lab's sibling Authoring Host remains mounted. The retired Cat Cafe supplied historical product evidence but is no longer a maintained consumer.                                                                                                       | Not wired.                                                                                                                                                                      |
...
Save/fence pair. Focused Core/Web tests cover those semantics. The retired Cat
Cafe supplied forward/reverse Chromium/WebKit product evidence over real Player
Save exports; Engine Lab retains the maintained neutral contract. Decoded State/RNG/sequence/integrity/digest and pending
...
than a parallel System or Root writer. Engine Lab's
`whole_canvas_conformance=1` route is the maintained neutral consumer; Cat
Cafe's ending was the retired first product consumer. The GUI-only SillyOS uses the neutral Web GUI entry rather than a
