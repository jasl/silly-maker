...
VN Reference Tour 已交付 M0–M2；引擎维护的 focused default VN Player 已提供贴底对话层、
全画布推进、History/播放控制、Ctrl/Tab/H/鼠标中键、最终媒体与产品矩阵，Template 与该产品显式选择同一 preset。完整产品门关闭前，
...
| Application packages      | `.` per package                                                                                                                                                                                                                                                                                                               | `template/` (the minimal game starter, MIT), `examples/bookshop` (focused low-level Narrative renderer example), and GUI-only `examples/silly-os` each compose one self-contained application project (`sillymaker.config.ts` + `vite.config.ts`). The `examples/vn-reference-tour` WIP has delivered M0–M2 and started M3 with boot-time autosave resume plus return-to-title/Continue continuity; it and Template select the focused default VN Player, while the remaining M3 Save/recovery/settings and later product evidence stay open. Bookshop's later teaching role is reviewed only after VN completion. The discontinued Electronic Pet and retired Cat Cafe packages are absent from the active workspace. The root `project.config.ts` only lists application directories for repository-level aggregation. A small tooling-owned application fixture—not a published example—keeps GUI-only startup and final-graph conformance independent of product content.                                                                         |
