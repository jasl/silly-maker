...
conformance 与自动化。仓库内的 Story/application packages 是使用它开发的具体产品；VN Reference Tour
是维护中的旗舰 Reference Product，并已完成 M0–M5。引擎维护的 focused default VN Player 已提供贴底对话层、
...
| Application packages      | `.` per package                                                                                                                                                                                                                                                                                                                                                                                                                                | `template/` (the minimal game starter, MIT), `examples/bookshop` (maintained focused low-level Narrative renderer example), GUI-only `examples/silly-os`, and the maintained flagship `examples/vn-reference-tour` each compose one self-contained application project (`sillymaker.config.ts` + `vite.config.ts`). VN and Template select the full default VN Player. VN's Agent plus owner-authorized participant handoffs share the ambient-binding Inspector/CAS path, and independent review corrected the historical 59/44 script to the current 110/82 denominator. Engine, Starter, and product reviews are complete. The owner cancelled representative real current-low-end qualification as a completion gate because no suitable device was available; it was not run or passed and creates no low-end claim. M4/M5 are closed, with static publish wiring but no claimed remote live deployment. Bookshop remains maintained pending a separate teaching-role review. The discontinued Electronic Pet and retired Cat Cafe packages are absent from the active workspace. The root `project.config.ts` only lists application directories for repository-level aggregation. A small tooling-owned application fixture—not a published example—keeps GUI-only startup and final-graph conformance independent of product content.                                                                                        |
