...
| Application packages      | `.` per package                                                                                                                                                                                                                                                                                         | `template/` (the minimal game starter, MIT) and `examples/*` (the focused Bookshop narrative example and GUI-only SillyOS) each compose one self-contained application project (`sillymaker.config.ts` + `vite.config.ts`). The accepted VN Reference Tour will be a new package copied from `template/`; it is not implemented yet. The discontinued Electronic Pet and retired Cat Cafe packages are absent from the active workspace. The root `project.config.ts` only lists application directories for repository-level aggregation. A small tooling-owned application fixture—not a published example—keeps GUI-only startup and final-graph conformance independent of product content.                                                                                                                                                                        |
