...
| Application packages      | `.` per package                                                                                                                                                                                                                                                                                         | `template/` (the minimal game starter, MIT) and `examples/*` (bookshop, cat-cafe, silly-os, electronic-pet) each compose one self-contained application project (`sillymaker.config.ts` + `vite.config.ts`). Electronic Pet owns its product-local Three runtime, authored 3D scene document/compiler, and dev-only Inspector replacement while its ordinary Player graph excludes the Inspector/source writer. The root `project.config.ts` only lists application directories for repository-level aggregation. A small tooling-owned application fixture—not a published example—keeps GUI-only startup and final-graph conformance independent of product content.                                                                                                                                                                                                 |
