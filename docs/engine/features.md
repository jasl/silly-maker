...
- Audio presentation: the Host-neutral `AudioHostV1` surface with a deterministic `createFakeAudioHostV1` for tests, and `createAudioPresenterV1`, which reconciles the Story's continuous audio intent per committed publication (idempotent replace/loop/fade, voice tied to say occurrences with stop/sustain and replay), plays each commit-only SFX at most once per presentation epoch through an instance-local consumed watermark, restores continuous intent across epoch changes without replaying history, and suspends/resumes with page visibility.
