...
- **M2（Lab conformance + 文档，主仓 `da74ef86`）**：Lab 演习决策菜
  ...
  下不派发）。focused Host test 钉 shared/isolated/history/混合窗口与
  焦点政策；浏览器 e2e `shared-stage-input.spec.ts` 在真实应用中补真
...
- **普通持久化语义。** 声明值随 pending 自然进入 Snapshot、Save、
  canonical bytes、digest 与 replay 完整性，但没有专门的算术、决议、
  `when` 或 digest/replay 分支；未声明形状仍与旧 pending 字节恒等。
...
isolation 开关 → 停；`stageInput` 被任何权威算术 / `when` 臂读取，或
为它新增独立 digest/replay 语义 → 停。
