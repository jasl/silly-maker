...
historical chain；原`S4.2` broad checkpoint也已由completed docs-only `S4.2.0`重切，`S4.2.1`
History atomic cross-axis preparation现也已关闭；原`S4.2.2` broad checkpoint又由completed docs-only
`S4.2.2.0`细分，`S4.2.2.1` DOM-free Narrative session/readiness attachment floor现也已关闭；
原`S4.2.2.2` broad dormant Host又由completed docs-only `S4.2.2.2.0`重切；linear core
`.2.2.2.1` DOM-free generic Host-commit atomic substrate与`.2.2.2.2` dormant Narrative React Host现也已关闭并转为
historical。原S4.2.3 History close/dismiss/input/focus broad checkpoint已由completed docs-only `S4.2.3.0`重切；
`.3.1` DOM-free generic exact History-child lifecycle substrate与`.3.2` dormant Narrative close/input/root + History focus Host lifecycle
现也已关闭并转为historical。原`S4.2.4` controller timers与History suspension/resume broad checkpoint现由completed
docs-only `S4.2.4.0` exact Dialogue player timing/suspension entry重切；`.4.1` generic prepared state-install participant
substrate、`.4.2` DOM-free Narrative DialoguePlayerController core与`.4.3` dormant Host player-view integration现也已关闭并转为
...
现也已交付并转为historical。PF5/M3.0–M3.6d 与 PF5 closeout 已完成并转为 historical。
Current/next、core slice与direct gate均为 **PF6/S5 docs-only exact-entry/adjudication（当前）**；
S5 broad checkpoint 尚无 exact allowlist/mergeable entry，不得直接开始 runtime implementation。
冻结线性顺序为 **S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.0–M3.6d（已完成；historical） → PF6/S5 docs-only exact-entry/adjudication（当前）**。本文后续 historical delivery records 中残留的 “PF5/M3.6c（当前）” 或 “PF5/M3.6d（当前）” 只记录当时顺序，均由本最终 pointer supersede，不是 live gate。
...
historical chain；原S4.2 broad checkpoint也已由completed docs-only S4.2.0重切，S4.2.1 History atomic
cross-axis preparation与S4.2.2.1 DOM-free Narrative session/readiness attachment floor现也已完成；原S4.2.2 broad
checkpoint又由completed docs-only S4.2.2.0细分；原S4.2.2.2 broad dormant Host现也由completed docs-only
S4.2.2.2.0重切，S4.2.2.2.1 DOM-free generic Host-commit atomic substrate与S4.2.2.2.2 dormant Narrative React
Host现也已关闭；原S4.2.3 broad checkpoint现又由completed docs-only S4.2.3.0重切，S4.2.3.1 DOM-free generic
exact History-child lifecycle substrate与S4.2.3.2 dormant Narrative close/input/root + History focus Host lifecycle现也已关闭。
原S4.2.4 broad checkpoint现由completed docs-only S4.2.4.0 exact Dialogue player timing/suspension entry重切，S4.2.4.1 generic
prepared state-install participant substrate、S4.2.4.2 DOM-free Narrative DialoguePlayerController core与S4.2.4.3 dormant Host
...
ingress corrective现也已关闭。PF5/M3.0–M3.6d 与 PF5 closeout 已完成；下一独立切片、
current/next、core slice与direct gate均为 **PF6/S5 docs-only exact-entry/adjudication（当前）**。
冻结线性顺序为 **S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.0–M3.6d（已完成；historical） → PF6/S5 docs-only exact-entry/adjudication（当前）**。PF6/S5 broad checkpoint 尚无可实施 exact entry；当前 gate 只负责 docs-only adjudication。
旧 promotion 数字保留为
...
承诺；当时 linear core下一切片为 `PF4/S3 System dialogs`，Save PF5/M3 只有在S4b.1c
atomic cutover/promotion及closeout完成后才允许开始；该前置现已关闭，current pointer见PF5。
...
29. S4.2：dormant Narrative Host、Host-commit readiness、timer scheduling与History exact-child lifecycle（superseded broad checkpoint）；
30. S4.2.0：Narrative/History dormant execution split（docs-only，已完成）；
31. S4.2.1：History atomic cross-axis preparation（已完成）；
32. S4.2.2：Narrative session与Host-commit readiness（superseded broad checkpoint）；
33. S4.2.2.0：Narrative session/readiness exact entry（docs-only，已完成）；
34. S4.2.2.1：DOM-free Narrative session/readiness attachment floor（已完成）；
35. S4.2.2.2：dormant React Narrative Host（superseded broad checkpoint）；
36. S4.2.2.2.0：guarded readiness/input与Narrative Host exact entry（docs-only，已完成）；
37. S4.2.2.2.1：DOM-free generic Host-commit atomic substrate（已完成）；
38. S4.2.2.2.2：dormant Narrative React Host（已完成）；
39. S4.2.3：History close/dismiss/input/focus lifecycle（superseded broad checkpoint）；
40. S4.2.3.0：History lifecycle exact entry与execution split（docs-only，已完成）；
41. S4.2.3.1：DOM-free generic exact History-child lifecycle substrate（已完成）；
42. S4.2.3.2：dormant Narrative close/input/root + History focus Host lifecycle（已完成）；
43. S4.2.4：controller timers与History suspension/resume（superseded broad checkpoint）；
44. S4.2.4.0：exact Dialogue player timing/suspension entry（docs-only，已完成）；
45. S4.2.4.1：generic prepared state-install participant substrate（已完成）；
46. S4.2.4.2：DOM-free Narrative DialoguePlayerController core（已完成）；
47. S4.2.4.3：dormant Host player-view integration（已完成）；
48. S4.2.5：dormant Engine Lab conformance（superseded broad checkpoint）；
49. S4.2.5.0：dormant Engine Lab Narrative conformance exact entry（docs-only，已完成）；
50. S4.2.5.1：broad implementation checkpoint（historical；已由`.5.1a`–`.5.1c`细分）；
51. S4.2.5.1a：managed InputRouter facade corrective（已完成）；
52. S4.2.5.1b：Host physical ingress corrective（已完成）；
...
57. S4.3.1b：tracked-consumer atomic cutover and promotion（已完成；historical）；
58. S4b broad checkpoint：whole-canvas primary/detail方向（historical）；
59. S4b.0：whole-canvas exact entry（docs-only，已完成）；
60. S4b.1a：whole-canvas family + routed-input substrate（已完成；historical）；
61. S4b.1b：dormant production composition/Host/GameStage substrate（已完成；historical）；
62. S4b.1c：tracked-consumer atomic cutover and promotion（已完成；historical）；
63. input/gesture reset（pointercancel、focus loss、visibility change）与 Browser
    Agent observation。
...
finding。S3 promotion与 S1-R.0–R3.0（含R1a corrective）完成；在该交付检查点，linear core
节点与下一独立切片均为S1-R.3a；S4仍受 S1-R aggregate gate约束。
...
因此顺序现为 **S3 → S1-R.0–S1-R.5 → S4 → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） →
S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**；若更早的 family 后续选择 external stable
...
**2026-08-09 S1-R.2c delivery：** UI新增source-relative、dormant stable-vector admission
authority与tests。Finite definition/slot catalog、captured schema callable/receiver、same-factory
baseline/reservation/proposal provenance、R1a proof-time occurrence classification、opaque defensive
canonical bytes、scope-local identity/order与10-stage exact admission均已闭合。Reservation authority
只封装R3提供的normalized foreign root-slot ID set，不读取Coordinator或枚举runtime phase；exact
proposal只携带captured lease/baseline/reservation和same-factory next baseline，不写accepted/runtime
state，也不产生notification/allocation。UI public/internal barrels、transient contracts、live family、
Save/Persistence/canonical bytes均不变；architecture只更新实际internal consumer graph。验证通过
Base bounded + R0/R1/R2c/public API focused `5 / 89`、UI `72 / 792`、全量`246 / 3720`与完整
`deno task check`；三路adversarial review无finding。下一独立切片为R3.0，只闭合
apply-time CAS stale taxonomy、zero delta与stable-readiness envelope，不提前实现R3 mutation或R4
settlement。

**2026-08-09 S1-R.3.0 delivery：** R0 pure contract新增apply-only precondition-stale code、
exact-zero delta row、proposal provenance → lease → baseline → reservation generation ordered evidence，
以及stable-only readiness envelope与epoch → candidate → lease → source ordered fence。R2c admission stale
...
本段是计划裁决，不是delivery记录；裁决时的唯一入口为S1-R.3a。

**2026-08-09 S1-R.3a delivery：** live transient Coordinator现为一个薄facade，复用
composition-owned runtime authority与generic kernel；exact reducer state、mutation/reentry fence、
captured-listener delivery与instance/routing identity cursor均只有一个owner。source-relative dormant
stable composite在同一kernel上绑定exact R1 registry与R2 admission configuration provenance，解析
same-factory admitted definition，并闭合stable runtime binding、root contributor vector与fresh generation
seam；它不apply stable proposal、不写accepted baseline、不settle stable readiness，也不连接任何
live stable family。UI root与`./internal` barrels、package export map、transient public contract均未改变。
...
S1-R.4b.0 terminal composite disposition；该checkpoint现由下述R4b.0 delivery取代。随后依次为R4b.1
source-bound readiness + global cascade settlement、R5 neutral harness/bounded churn/dead-path audit，再进入仍受
S1-R aggregate gate约束的S4。

**2026-08-10 S1-R.4b.0 delivery：** live generic single runtime kernel新增construction-captured、optional、
default-exact的first-terminal prepare/gate seam；只有reducer返回exact
`applied / surface.coordinator_disposed`才进入该seam，普通operation、already-disposed repeat与未提供callback的adapter
保持既有state/receipt。完整successor、install record与notification vector均在gate前captured，gate成功后只以已捕获
local完成assignment/finalization；callback、output getter、validation或gate throw都保持old state、zero notification并
释放transition fence。R4a nonterminal finalizer仍旁路两种terminal receipt，terminal分类不预读caller operation。

Source-relative dormant stable composite consumer以existing transient terminal reducer successor为oracle，pure-build完整
terminal composite successor：清空accepted baselines、stable runtime bindings、root contributors与private
attempt/contributor provenance，按exact contributor-vector change复用或fresh generation token，并保留transient
identity high-water/recipe。Registry-wide disposal通过构造期捕获的exact registry receiver/callable作为同一prepared
install gate；首次dispose、raw external already-disposed convergence、listener reentry与capacity边界均由确定性测试锁定。
新增private provenance comparator只返回冻结的identity comparison与bounded size，仍为source-relative audit seam。
本批不settle readiness、不启动global child cascade、不接live stable family，也不扩张UI root、`./internal` barrel、
package exports、transient public receipt/evidence或Story API。

验证通过focused `7 files / 154 tests`、UI package `76 files / 919 tests`、全量`250 files / 3847 tests`与
`deno task check` green、Engine browser `101 / 101`、examples browser
`45 passed / 2 skipped`与prebuilt Player `38 / 38`。该checkpoint当时的linear core current/next均为
S1-R.4b.1 source-bound readiness + global cascade settlement，现由下述R4b.1 delivery取代；随后为R5
neutral harness/bounded churn/dead-path audit，再进入仍受S1-R aggregate gate约束的S4。

**2026-08-10 S1-R.4b.1 delivery：** dormant stable composite runtime现以source-bound readiness
envelope完成ready/failure terminal settlement，并严格执行application epoch → exact candidate instance →
publisher lease → source revision fence；stale receipt保持exact zero，shared phase或capacity fault返回existing
`faulted / surface.stable_reconcile_faulted`并保留old composite state、identity high-water、reservation token与
两类notification exact zero。Ready/failure、stable proposal apply、greater-empty、effective publisher dispose及
nonterminal transient transition均经唯一shared topology policy派生active/suspended phase；所有本次解除阻塞的
same/other-owner direct children与proposal fresh root/eligible child先合并为一个topology-preorder canonical batch，
再执行一次shared safe-integer capacity check与bulk identity allocation。Parent-unavailable gap不会伪分配，
readiness-failed仍只由显式newer publication重试；retained predecessor继续使用authenticated exact subtree，
terminal Coordinator dispose保持R4b.0优先收口路径。

Stable proposal、readiness、dispose与transient post-reducer reflow均先pure-build完整successor，再经existing
single runtime kernel原子安装；phase/cascade改变transient-facing publication时仍保持transient-before-state的
exact notification ordering。Canonical allocation不使用raw cross-scope interleaving、runtime sequence或
allocation order发明authority，多个publisher合计超过64个合法direct children仍只受composition shared cursor
约束。本批没有接入live stable family，也没有扩张UI root、`./internal` barrel、package exports、transient
public receipt/evidence或Story API。
...
当前HEAD的验证结果。该checkpoint当时的linear core current/next为S4.1b authenticated action admission，现由下述
S4.1b.0 delivery取代。

**2026-08-10 S4.1b.0 delivery：** Managed action route已抽取source-relative contract-bound shared gate，同时保持既有
Coordinator adapter与public action envelope/receipt语义不变；composition-owned stable composite从同一次whole-composite
topology projection选择唯一input owner并签发opaque direct-target proof，不让Narrative重算phase、layer或owner。Exact active
direct target可捕获frozen binding contract与source-bound proof；retained predecessor只能保留用于消费旧binding的contract而没有
direct target/source/proof，preparing、gap、suspended或transient-selected状态均不能取得stable direct proof。Proof在source
advance、replace、empty、dispose或successor变化后稳定stale，route admission保持既有epoch → topology → instance → input owner →
routing lease → action catalog precedence，并保持zero topology/state mutation与zero notification。

本批没有新增UI root、`./internal` barrel或package export，没有接Narrative semantic port、physical/automatic resolution、React
Host、History child或live Story ingress；特别是它只建立shared route gate与composite owner proof，不能被描述为live Narrative
semantic dispatch。验证通过focused `7 files / 172 tests`、UI package `79 files / 999 tests`、全量
`253 files / 3927 tests`与完整`deno task check` green；Engine browser `101 / 101`、examples browser
`45 passed / 2 skipped`及prebuilt Player `38 / 38`也通过。该checkpoint当时的linear core current/next为
S4.1b.1 semantic-port callable/action mapping与authenticated admission，现由下述S4.1b.1a delivery细分并取代。

**2026-08-10 S4.1b.1a delivery：** Claimed route continuation现只在Surface route、input publication与physical gesture
全部认证成功后接收frozen exact `{ actionId, attempt }`。Action ID来自authenticated envelope，opaque attempt保持exact identity且
不能用自身字段覆盖该action ID；stale/rejected/faulted route、untagged input、dispose/reentry与throw仍保持既有零continuation、
fallthrough或fence-release语义。

本corrective没有定义Narrative action mapping、semantic callable、mapped proof、automatic controller attempt或non-test claimant，
...
authority。Manual pause现由S4.1b.1b.1a关闭；automatic pause-expiry由S4.1b.1b.1b.1拥有，Say/custom/
presentation-barrier与player controls的remaining mapping及其裁决由S4.1b.1b.1b.2拥有。验证通过focused `7 files / 187 tests`、UI package
...
不冒充本HEAD的新验证。该checkpoint当时的linear core current/next为S4.1b.1b.1，现由下述
S4.1b.1b.1a delivery继续细分。

**2026-08-10 S4.1b.1b.1a delivery：** Existing Narrative physical admission现同时支持ready+active direct Pause
target，但只有captured pending的`skippable`为`true`时才签发frozen zero-key、same-admission one-shot pause-resume attempt；
non-skippable Pause可以保留exact physical binding，却不能取得manual semantic capability。只有完整通过既有
Surface/Input/publication/gesture gate的authenticated `narrative.resume`与exact pause-resume attempt才能从immutable admitted
frame派生frozen `{ expectedOccurrenceId, resolution: { kind: "resume" } }`并调用已捕获的semantic receiver/callable。
`ui.confirm`与`narrative.advance`仍返回family-local `unmapped`且不读取或消耗attempt；choice/pause cross-kind、clone、foreign、
repeat、stale gesture/source/phase/replacement/dispose保持semantic dispatch为零，semantic callable同步throw继续规范化为rejected
completion而不回滚Surface/composite state。

本批没有实现automatic timer或controller-attempt lease，也没有裁决Say的`ui.confirm`/`narrative.advance`、reveal-first与
`advancePolicy`，没有建立custom payload proof、presentation-barrier acknowledgment、player-control policy或non-test claimant。
Base PendingInteraction/resolution、semantic receipt、Save/Persistence/replay均无变化；UI root、`./internal` barrel、package export、
generic stable/action result table、generic receipt/code、Narrative Host/History/React/Web ingress与live Story graph均无扩张。
验证通过focused `7 files / 192 tests`、UI package `79 files / 1010 tests`、full `253 files / 3938 tests`与
`deno task check` green。本批没有改变live browser/build graph，因此未重跑browser/examples/prebuilt；最近已有的Engine browser
`101 / 101`、examples browser `45 passed / 2 skipped`与prebuilt Player `38 / 38`仅作先前证据，不冒充本HEAD的新验证。
该checkpoint当时的linear core current为S4.1b.1b.1b.1、next为S4.1b.1b.1b.2，现由下述
S4.1b.1b.1b.1 delivery取代。

**2026-08-10 S4.1b.1b.1b.1/.2 execution-order amendment：** 该amendment把当时的mergeable floor限定为automatic pause-expiry
controller-attempt admission/dispatch：`skippable: true | false` Pause都允许automatic `resume`，manual capability仍只由
既有`skippable`合同控制。Attempt绑定exact epoch、runtime instance、topology revision、publisher lease、target
occurrence/source revision与controller generation；topology变化使旧attempt stale，同一target仍ready+active时current
generation可fresh issue，suspend后恢复必须由S4.2建立fresh generation，replacement/empty/dispose直接取消。该floor不创建
timer、不读取clock，也不拥有deadline、remaining duration/cursor。Say的`ui.confirm`/`narrative.advance`、reveal-first与
`advancePolicy`、custom payload、presentation-barrier acknowledgment及player-control policy继续在下一
S4.1b.1b.1b.2保持stop；执行顺序仍禁止提前进入S4.2。本amendment不声明runtime delivery或验证结果。

**2026-08-10 S4.1b.1b.1b.1 delivery：** Composition-owned stable action authority新增source-relative、read-only
ready-active direct-target capture/proof：proof绑定exact application epoch、runtime instance、topology revision、publisher lease、
target identity与source revision，并由whole-composite topology而非Narrative重算phase。Raw publisher-registry与composite
baseline/runtime divergence现在使全部stable action capture/proof路径fail closed，保持state、topology与notification exact zero。

Narrative bridge以private claim-once controller generation与frozen zero-key attempt承载automatic pause expiry；exact current
ready+active Pause无论`skippable`为`true`或`false`都只提交captured `resume` semantic request。任意topology change使旧attempt stale；同一
target仍ready+active时同一generation可fresh issue，suspend会撤销generation且恢复必须由S4.2建立fresh generation，replacement、
empty、publisher/Coordinator dispose与successor则取消旧controller/token。该切片没有读取clock、创建timer或保存deadline/
remaining cursor，也没有接Narrative Host、History、React/Web、live Story或non-test claimant；UI root、`./internal` barrel、package
export、generic stable/action result/delta table与generic receipt/code均无扩张。
...
S4.1b.1b.1b.2a → S4.1b.1b.1b.2b → S4.2 → S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）。该checkpoint现由下述
...
loss撤销；admission只借用exact controller，old admission/token不能影响fresh generation。Issuance不读取phase；route完成
generic fence/mapping后取得controller gate、验证currentness、spend并只调用一次`capturePhase`。Callback后currentness重验的
stale优先于invalid/throw fault；incomplete调用`revealAll`后再重验，success才`revealed/null`，complete才把gate转为
semantic in-flight。所有negative/reveal-only路径semantic dispatch zero。
...
S4.1b.1b.1b.2b.3 → S4.2 → S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）。
...
S4.1b.1b.1b.2b.2 → S4.1b.1b.1b.2b.3 → S4.2 → S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）。
...
`38 / 38`。该delivery完成时linear core current/next均推进为
S4.1b.1b.1b.2b.2b settle/replay recovery，现由下述docs-only checkpoint进一步拆分。

**PF4/S4.1b.1b.1b.2b.2b.0 recovery generation admission contract checkpoint
（docs-only，已完成）：** Recovery generation继续复用`.2a`的同一个claim-exclusive Stage authority与同一个
Narrative Barrier controller/bridge authority，不新增第二个cursor、anchor token或writer。Stage current proof只由既有
authority认证，并相对bridge保留的prior proof判定initial/equal/higher，lower直接映射stale；bridge只保留一个O(1) current generation及
generation建立时的exact preexisting Barrier target-or-null。Equal复用exact generation与原target-or-null，保持zero
resnapshot；同application bridge上的fresh reconciler proof为foreign并fail closed，不把React remount、StrictMode或callback/
effect identity提升为freshness。Claim后presentation epoch只经existing controller委托同一Stage authority的generation-retarget
入口推进，same epoch与reentry fail closed，禁止raw public writer或test hook。Initial/higher安装还绑定descriptor-captured exact activation gate：安装时必须closed，后续
recovery issuance/dispatch时必须是同一gate且open。Application successor只能以fresh bridge/authority建立initial generation。
...
delivery推进。后续顺序保持S4.1b.1b.1b.2b.2b.1 → S4.1b.1b.1b.2b.3 → S4.2 → S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）。
...
S4.1b.1b.1b.2b.3d.1 History exact-parent open intent implementation → S4.2 → S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）；
...
remaining duration/cursor。原`.3d` broad checkpoint同样不再直接实施，现由`.3d.0` exact entry与`.3d.1`
implementation取代；accepted boundary仍只在authenticated current ready-active Dialogue route读取read-only
availability并mint opaque exact-parent History open intent。`.3d.1`不调用Coordinator打开/关闭child，不分配
transient occurrence/instance，也不产生topology、readiness、focus或dismiss mutation。S4.2继续独占
...
`.3b.1`的acceptance要求先跑Narrative family与public-boundary focused tests，再跑UI package、full unit与完整
...
evidence，不记录上述命令结果；只验证本文件`deno fmt --check`与`git diff --check`。该checkpoint当时的linear core
current/next均为 **S4.1b.1b.1b.2b.3b.1 voice replay physical route implementation**，现由下述delivery推进。

**PF4/S4.1b.1b.1b.2b.3b.1 voice replay physical route implementation delivery（completed）：**
Candidate preflight现把exact optional raw voice port descriptor-capture为frozen zero-key handle；既有
`NarrativeStablePhysicalActionAdmissionInternalV1`仍是唯一route/admission authority，并只签发same-admission、one-shot
voice attempt。Exact `player.replay_voice` consumer在generic gate之后spend authentic attempt，复用bridge-wide
`sayCallbackClaim`与既有manual/content-auto形成single first-win callback boundary；callback后的admission/proof/frame/handle
漂移先归stale，own claim只以exact CAS释放。Absent/true/false/throw或nonboolean分别收口为ignored/handled/ignored/faulted，
全部保持Input consumed、semantic dispatch与authoritative/composite state、topology、notification为zero。

本delivery只修改source-relative Narrative family implementation、同目录mutation-sensitive test与public-boundary negative
guards；没有新增generic result、UI root/`./internal` barrel或package export，也没有接Host/React/Web/live Narrative
claimant、audio presenter、Auto/Skip、History或第二callback/route authority。验证通过focused `2 files / 137 tests`、UI
...
本HEAD验证。该delivery当时把linear core current/next推进为
**S4.1b.1b.1b.2b.3d History exact-parent open intent floor** broad checkpoint；该checkpoint现由下述`.3d.0`
exact entry与`.3d.1` implementation细分并取代。

**PF4/S4.1b.1b.1b.2b.3d.0 History exact-parent open-intent exact entry contract（docs-only，completed）：**
原`.3d` broad checkpoint标记为superseded，不再直接领取；实际source/test delivery由
S4.1b.1b.1b.2b.3d.1独占。Candidate preflight新增required raw
`NarrativeStableHistoryAvailabilityPortInternalV1`：它只能是prototype exact为`Object.prototype`、non-array且own key
exact only `readHistoryAvailabilityInternalV1`的ordinary object，该key必须是own data descriptor callable
`() => boolean`。Preflight一次descriptor-capture exact receiver与callable，private `WeakMap`保存binding，admitted candidate的新增
`historyAvailabilityPort` field只保存frozen zero-own-key branded
`NarrativeStableCapturedHistoryAvailabilityPortInternalV1`。Required-port closed set新增
`narrative.history_availability`；missing resolver port继续返回`narrative.required_port_missing`，present但malformed、accessor、extra/
symbol key、foreign prototype或reflection trap/throw继续归一化为`narrative.candidate_preflight_faulted`与pre-allocation exact zero delta。

既有opaque `historyObservationPort` field、`narrative.history_observation` required-port ID及其S4.2 content-observation ownership保持
exact不变；`.3d.1`不得把它收窄、调用、替换成availability callable或让availability handle承载History content。新增closed
source-relative名称只有`NarrativeStableHistoryOpenActionAttemptInternalV1`、
`NarrativeStableHistoryOpenIntentInternalV1`、`NarrativeStableHistoryOpenDispatchResultInternalV1`与既有
`NarrativeStablePhysicalActionAdmissionInternalV1.issueHistoryOpenAttemptInternalV1()`。Attempt与intent均为frozen zero-own-key
branded capability。Result exact union是fresh
`{ kind: "requested", intent, completion: null }`与canonical
`{ kind: "ignored" | "stale" | "faulted", completion: null }`；requested row own keys exact为
`kind | intent | completion`，其他row exact为`kind | completion`。History不建立第二binding/claimed route或新route method；existing
`routeInternalV1`只新增exact `player.toggle_history` mapping，result仅进入Narrative-local physical union与existing authenticated
consumer result，不扩generic Surface/Input result、receipt或code。

`issueHistoryOpenAttemptInternalV1()`只为exact current ready-active `say | choice | pause | custom |
presentation_barrier` Dialogue root签发attempt；preparing、retained replacement、suspended、exiting、empty或terminal不签发。Issue要求
physical admission仍current，existing bridge-wide `sayCallbackClaim`与`saySemanticInFlightClaim`都为空，并把issuing admission、
current stable input/direct-target proof、exact parent target/source/frame与captured availability handle放入private WeakMap provenance；它不调用
availability，也不mint open intent。Attempt进入exact mapped consumer前仍先经过existing generic
application/terminal/coherence、Surface topology/instance/input-owner/routing-lease/action-catalog、Input publication与gesture fence；consumer未被
调用时`consumerResult`保持`null`且attempt不spent。Action mapping先于spend：same-admission unspent History token探测non-History
mapped action，或same-admission unspent other-kind token探测`player.toggle_history`，均返回existing physical `unmapped`且不spend；
unrecognized clone、foreign/predecessor或repeat token在exact History mapping下closed为History `stale`。只有same-admission、
correct-kind、unspent authentic winner可继续，并在任一family check/callback前one-shot spend。

Spend后route重验physical admission、direct-target proof、stable input、exact parent target/source/frame与captured handle；identity/
currentness漂移归`stale`，authority capture/inspection throw或captured binding内部不合法归`faulted`。仍current时必须再次要求
`saySemanticInFlightClaim === null`与`sayCallbackClaim === null`，以fresh zero-key token exact-CAS
安装existing bridge-wide`sayCallbackClaim`，pre-call重验own token、semantic-inflight仍空及上述完整currentness后，才以zero arguments、
exact captured receiver调用`readHistoryAvailabilityInternalV1`。History availability永不安装、转移、释放或清理
`saySemanticInFlightClaim`；semantic-inflight pending期间issue为`null`，preissued mapped attempt为spent `stale`。Same-binding nested route继续由
generic in-progress fence拒绝；shared callback claim只约束既有已claim-aware的Say/manual、content-auto、voice、mode与nested History
路径，`.3d.1`不顺带改造choice/pause/custom/barrier claim policy。Callback内其他既有authorized operation只保留其自身exact delta，
outer route必须在解释return/throw前先重验own callback token、semantic-inflight、admission、proof、parent/source/frame/handle；任一nested
replacement、preparation、source retry、suspend、empty、terminal或claim drift均以`stale`覆盖callback outcome。仍current时exact
`false`为`ignored`，exact `true`才mint fresh intent并返回`requested`，throw或non-boolean为`faulted`；finally只以own-token CAS释放
`sayCallbackClaim`，不得清除successor claim。

`NarrativeStableHistoryOpenIntentInternalV1`的private provenance绑定exact Narrative bridge/stable action authority与mint时ready-active parent
proof、target、source及frame，但不绑定physical-admission lifetime，也不暴露raw Coordinator、parent handle/
occurrence/instance、source revision、frame或copyable target spelling。Physical admission单独dispose/recreate不使已mint intent失效；parent
replacement/preparation、source retry、suspend（即使之后resume）、empty、application/coordinator/bridge terminal则使它不可再兑换。
`.3d.1`只mint该future one-use handoff，不新增redeem/consume/inspect/claim API，也不在本切片预先spend intent。S4.2必须在同一个
composition-owned Coordinator child-open transition内原子重验intent provenance/current exact parent并把该intent标记spent；不得先做独立
check/spend再打开child形成TOCTOU。Actual one-use/repeat/clone/foreign/stale redemption、History occurrence/instance分配、readiness、
input/focus/dismiss、root suspension与close仍全部由S4.2的atomic transition及其tests拥有。

`.3d.1` RED matrix必须覆盖raw/captured availability descriptor与hostile reflection、original callable/receiver retention、exact
type/API/result keys、五种Dialogue kind及全部unavailable phase、generic-before-spend、attempt authenticity/cross-kind、false/true/throw/
non-boolean outcome、same-binding与cross-admission callback reentry、semantic-inflight conflict、post-callback stale precedence，以及
10,000次attempt/intent/source/readiness churn下production strong state O(1)。Ordinary outcomes的semantic/gameplay、stable/composite
state、source/runtime、topology、notification及History identity allocation必须exact zero；caller-retained attempts/intents只能作为
private `WeakMap` key保留旧provenance。`.3d.1`只证明intent frozen/opaque/fresh、每个successful physical attempt最多mint一个及其
mint-time provenance；post-mint admission-independence、parent/source/suspension/terminal invalidation与one-use redemption必须由S4.2
在真实atomic Coordinator transition中验证，不得以本批不存在的consume/inspect seam冒充证据。

Implementation scope只允许source-relative Narrative family implementation、同目录mutation-sensitive tests与
`engine/packages/ui/src/public-api.test.ts` negative guards。Dialogue revision `2`与History revision `1` catalogs、generic
`playerInputActionIdsV1.toggleHistory`、existing opaque observation port、UI root、`./internal` barrel/package export、generic Managed
Surface/Input route/result、Base/Web/React Host、legacy DialoguePanel/Engine Lab/Story writer及live graph均保持zero diff。若实现要求调用
Coordinator或修改topology、提前分配History occurrence/instance、暴露raw parent identity、建立standalone intent spend/check API、收窄
opaque observation port、新增History-specific callback slot而不复用existing shared claim、写入/清理semantic-inflight、扩大generic/public
contract、承担History close/focus/dismiss/root suspension/timer coordination或改造无关choice/pause/custom/barrier claim policy，立即停止并修订
owning design或留给S4.2。
...
由下述delivery取代。该History intent delivery checkpoint完成时，linear core current/next与implementation gate曾推进为
**S4.2** broad checkpoint；该pointer随后又由下述S4.2.0 split历史化。
...
该delivery当时把linear core current/next与implementation gate推进为 **S4.2** broad checkpoint；该checkpoint现由
下述`.0` amendment细分并取代。

**PF4/S4.2.0 Narrative/History dormant execution split（docs-only，completed）：** 原S4.2 broad
checkpoint不再作为一个合并切片。唯一有效线性顺序为 **S4.2.1 History atomic cross-axis
preparation（现已完成）→ S4.2.2 Narrative session与Host-commit readiness（现又由`.2.2.0`及`.2.2.1`–`.2.2.2`
细分）→ S4.2.3 History close/dismiss/input/focus lifecycle（现又由`.3.0`及`.3.1`–`.3.2`细分）→
S4.2.4 controller timers与History suspension/resume（现由completed docs-only `.4.0`及`.4.1`–`.4.3`细分）→
S4.2.4.1 generic prepared state-install participant substrate（已完成）→
S4.2.4.2 DOM-free Narrative DialoguePlayerController core（已完成）→ S4.2.4.3 dormant Host player-view integration（已完成）→
...
S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。后项只能
...
root是stable composite runtime node，不在transient publication中，因此不存在可由public/transient
Coordinator取得的authentic parent handle；实现不得伪造`ManagedSurfaceHandleV1`、parent evidence或调用public
Coordinator API。唯一允许的cross-axis入口是从exact composite kernel claim的source-relative、composition-owned
authority；它在composite state内认证current ready-active stable parent，并委托一个只接受该已认证stable parent与
History candidate的narrow reducer helper建立transient child。该helper不是新
`ManagedSurfaceOperationV1` row，也不进入Coordinator、generic contracts或任何barrel。

该cross-axis mutation authority必须按`(kernel, exactClaimant)` claim-exclusive：composition-owned Narrative-family
record为每个exact composite kernel只mint并strong-hold一个frozen zero-own-key claimant；same kernel + exact same
claimant重复claim复用同一authority。该package-private claimant可由同一composition/kernel中的bridge successor复用，但每个
bridge lifecycle必须另绑定exact current active bridge，使old bridge/intent保持stale而successor正常使用同一composite
authority。Foreign kernel、clone/foreign claimant或borrowed/wrong-receiver调用全部fail closed；没有release、transfer、
reclaim、foreign/caller mint seam，也不得把mutation实现做成按kernel全局缓存、可由任意caller取得的callable。Composition/kernel
terminal后该claim永久不可复活。

Family result精确为`NarrativeStableHistoryChildPreparationResultInternalV1`：fresh frozen
`{ kind: "preparing", preparation, completion: null }`携带zero-own-key
`NarrativeStableHistoryChildPreparationInternalV1`，canonical frozen
`{ kind: "stale" | "faulted", completion: null }`不携带preparation；own keys分别exact为
`kind | preparation | completion`与`kind | completion`。Preparation的private provenance绑定exact composite kernel、
Narrative bridge、fresh History occurrence/instance及preparing child，留给`.2`接管session/readiness，且不暴露raw
parent/target/occurrence/instance、Coordinator handle/evidence或generic receipt/code。Composite authority内部可以用
`installed`描述prepared-state install，Narrative family只能把它映射为`preparing`，不得泄漏generic/internal result。
Clone、foreign、repeat、spent intent，parent proof/target/source/frame漂移，root已preparing/suspended/retained/terminal，或exact single child
已占用均收敛为`stale`；coherence、definition、identity/capacity或planning fault收敛为`faulted`。Availability只采用
`.3d.1`已经认证的route-time `true`，`.1`不得重调availability或读取opaque History observation content。

Intent provenance/currentness、child definition/slot/cardinality与fresh identity先对exact current composite state完成
pure plan，再用既有one-shot prepared-state install gate提交。只有gate确认same intent仍authentic且unspent，并将该
intent one-way标记spent后，才安装preparing successor；因此只有family `preparing`消费intent。Internal prepared-state
token stale/invalid、gate未调用或返回false、commit abort及所有`stale | faulted`路径都保持intent unspent以及state、identity high-water、
topology与notification exact zero；不得先提供standalone check/spend/consume/redeem seam。两个pre-minted intent的
first winner建立preparing child并spent，loser因root已suspended/slot已占用返回unspent `stale`；winner repeat也为
`stale`。Loser虽未spend，其topology-bound proof已经永久stale；child后来close也不能复活该old intent，fresh vacancy必须
重新mint fresh intent。

Applied successor只新增一个`child_open` blocking-fallback History candidate，`parentInstanceId`绑定exact stable
Dialogue runtime instance，使root在同一composite install中进入suspended；stable accepted source/vector、semantic/gameplay
state与History content保持zero diff，并只发布一次完整successor notification。Root replacement preparation与failure必须
structurally retain exact root+History subtree及其identity；replacement ready cutover、greater-empty、publisher或composite
terminal必须在同一install cascade-retire整棵History subtree，不能reparent给successor root。`.1`不settle readiness、
不关闭child，也不安装Host/input/focus/timer authority；这些分别由`.2`、`.3`与`.4`领取。

`.1`的implementation scope exact只有下列七个existing files：
...
Hard stop：`.1`只允许在上述seven-file scope内新增source-relative internal cross-axis authority；若需要修改
`managed-surface-runtime-kernel.ts`、扩大generic/public runtime-kernel API/contract、Coordinator、`managed-surface-contracts.ts`、generic action/result/receipt/code、
UI root/`./internal`/package exports，或需要React/Web/Base/Save/wire、Host/session、History observation/content、
close/dismiss/input/focus、controller/reveal/timer、Engine Lab/Story/live claimant或legacy writer，立即停止并修订owning
contract。若narrow helper不能在不新增`ManagedSurfaceOperationV1`或public Coordinator parent handle的前提下表达
cross-axis child，prepared install gate不能保证applied-only spend，或retain/cascade不能与root在一个composite successor内
闭合，也必须在RED前停止；不得以第二topology authority、split check/spend或generic/public ABI绕过。

`.2`只消费`.1` family preparation，并在其private record上建立dormant Narrative session、single Host lease、root/History candidate attachment、
React Host-commit readiness/candidate failure及ready前必需的initial-focus与input/action registration；它不实现这些binding的
close/dismiss route lifecycle。该lifecycle随后又由`.3.0`细分：`.3.1`独占DOM-free generic exact-child close/dismiss substrate，`.3.2`
独占History preparing/active close、四类dismiss、exact opener restore及pointer/keyboard/gamepad managed input与root + History focus
lifecycle；`.4`再独占bridge-owned mode的controller observation、reveal/auto/skip
clock、deadline/remaining/generation及History/higher-blocker suspend/resume。`.5.1`只接dormant Engine Lab conformance，
不切换tracked live writer；只有S4.3可原子接入live composition/Stories并删除legacy owners/exports。

本`.0` amendment严格只修改本cross-plan文件；没有source、test、runtime、architecture、Host/React/Web/live
claimant、public API或delivery evidence。验证只要求本文件`deno fmt --check`与`git diff --check`。`.0`与后续
`.1`现均为completed historical entry；原S4.2.2又由下述`.2.2.0`细分，`.2.2.1`也已由下述delivery关闭；唯一
原`.2.2.2` broad dormant Host也已由下述completed `.2.2.2.0` exact entry重切；该checkpoint当时的current/next、core slice与
implementation gate为 **S4.2.2.2.1 DOM-free generic Host-commit atomic substrate**，随后才是`.2.2.2.2`
dormant Narrative React Host；该`.1` gate现已由下述delivery关闭。

**PF4/S4.2.1 History atomic cross-axis preparation delivery（completed）：** implementation精确落在上述
seven-file scope。Stable Dialogue root继续只存在于composite stable runtime而不镜像进transient publication；reducer只新增
source-relative narrow cross-axis child-preparation helper，由composite authority提供已认证stable parent projection并建立
`parentInstanceId` link，没有新增`ManagedSurfaceOperationV1`、public/transient Coordinator parent handle、generic action/result/
receipt或第二topology store。Exact `(kernel, exactClaimant)` authority保持claim-exclusive；Narrative-family按kernel strong-hold
frozen zero-key claimant并供same-kernel bridge successor复用，authority的frozen exact input/guard/candidate/result只在source file间
流动。Pure plan完成definition/slot/cardinality/shared identity及exact ready-active parent认证后，existing prepared-state install gate在
state assignment前调用Narrative no-throw guard；只有guard的`spent: false -> true` CAS与exact candidate capture成功才安装，generic
candidate provenance在assignment/同步notification前完成认证，`stale | faulted | abort`全部保持intent、state、identity high-water、
topology与notification zero delta。

Whole-composite construction/reflow现结构化认证stable-parent/transient-child link：root replacement preparation/failure retain exact
root + History subtree与identity；successful cutover、stable empty、publisher/bridge disposal及composite terminal在同一successor内
cascade-retire整棵subtree，preparing child使用readiness-failure structural retirement，ready child使用exact close，均不reparent给
successor root。成功History redemption只安装一个`preparing / child_open` blocking-fallback child、原子suspend exact parent并发布一次
完整successor notification；两个pre-minted intent only-first-wins，loser保持unspent但proof永久stale，child retire后也必须mint fresh
intent。

Narrative boundary新增且只新增source-relative
`createNarrativeStableHistoryChildLifecycleInternalV1({ bridge })`、exact bridge-bound lifecycle、zero-key preparation与
`preparing | stale | faulted` family result。Factory从private bridge record推导kernel；per-kernel claimant/authority与per-lifecycle bridge
identity分离，使disposed predecessor永远stale而successor复用同一authority。Redemption重验intent/bridge/parent/source/frame/proof及
shared callback/semantic claims，不依赖physical admission存活或重调History availability/observation/semantic port；所有可能抛出的
preparation/result/guard/input construction均在authority调用前完成，prepared gate内只读module-owned plain identity/active/claim fields并
执行candidate capture + spent CAS，generic `installed`返回后只用pre-captured no-throw WeakMap setter绑定private provenance并映射为
family `preparing`。Root与`./internal` type/runtime negative guards同时锁定Narrative、generic authority/guard/candidate/result及reducer
helper spelling；UI barrels、package exports、public runtime-kernel/Coordinator contract与React/Web/Base/Save/wire、Host/session、
close/dismiss/input/focus、timer/controller、Engine Lab/live claimant均保持zero diff。

验证通过focused `4 files / 297 tests`、UI package `79 files / 1211 tests`、full `253 files / 4141 tests`，以及完整
`deno task typecheck`、`deno task lint`、`deno fmt --check`、`git diff --check`与`deno task check`。首次full run只出现一个与本slice无关的Engine Lab
timer时序flake；该case isolated rerun green，随后同一full matrix完整rerun green，因此不把首轮flake伪装成产品回归。该delivery
未重跑browser、examples或prebuilt；先前Engine browser `101 / 101`、examples browser `45 passed / 2 skipped`与prebuilt Player
`38 / 38`只作prior evidence，不冒充本HEAD验证。S4.2.1现为completed historical entry；该checkpoint当时把
implementation gate推进到 **S4.2.2.1 DOM-free Narrative session/readiness attachment floor**，现已由下述`.2.2.1`
delivery继续推进。

**PF4/S4.2.2.0 Narrative session/readiness exact entry（docs-only，completed）：** 原S4.2.2把DOM-free
session/readiness observation、React Host、ready/fail、focus与ready前action binding混在一个checkpoint；现有checkout的opaque
`rendererComponent`/`historyObservationPort`也不是这些合同，candidate renderer props、Narrative portal与Host commit seam均尚未实现。
本entry不把System Host、legacy `DialoguePanelV1`/`VnLayerV1`或live GameStage当作隐含合同，而将唯一线性顺序冻结为：

1. **S4.2.2.0（本entry，completed）**：只冻结下述source-relative name table、atomic boundary、RED、file scope与stop；
2. **S4.2.2.1 DOM-free Narrative session/readiness attachment floor（completed）**：只交付DOM-free exact
   Narrative session、root/History readiness preparation observation与single logical Host lease；不settle ready/fail，不准备action/input/
   focus，不接React/DOM/portal/renderer props；
3. **S4.2.2.2（superseded broad checkpoint）**：已由`.2.2.2.0–.2`重切，不能再作为implementation gate；
4. **S4.2.2.2.0（docs-only，completed）→ .2.2.2.1（completed）→ .2.2.2.2（completed）**：先冻结exact generic/
   Narrative/React boundary，再分别交付DOM-free generic Host-commit atomic substrate与dormant Narrative React Host；
...
`.2.2.1`的session/factory entry精确命名为frozen `NarrativeStableSessionInternalV1`及
`createNarrativeStableSessionInternalV1(input: Readonly<{ readonly bridge: NarrativeStablePublisherBridgeInternalV1 }>): NarrativeStableSessionInternalV1`；
factory从exact active bridge private record取kernel，per bridge只缓存一个exact session并采用该bridge已
缓存的exact `NarrativeStableHistoryChildLifecycleInternalV1`，不得另建claimant、lifecycle或topology owner。
Repeated exact session/lifecycle factory调用返回same identity；same-kernel successor取得fresh session/lifecycle并复用既有per-kernel
family claimant，disposed predecessor session、lease、preparation与lifecycle永久stale。
Factory及bridge-private cache/binding必须留在`narrative-managed-surface-family.ts`；new
`narrative-managed-surface-session.ts`严格只拥有上述session/lease/root/readiness exported types，不得新增helper export/name、runtime
logic、raw bridge accessor或binding/authority claim；session test只能通过family factory验证runtime behavior。

Readiness observation只使用zero-own-key frozen `NarrativeStableRootPreparationInternalV1`、existing
`NarrativeStableHistoryChildPreparationInternalV1`、closed union
`NarrativeStableReadinessEntryInternalV1 = Readonly<{ kind: "root"; preparation: NarrativeStableRootPreparationInternalV1 }> |
Readonly<{ kind: "history"; preparation: NarrativeStableHistoryChildPreparationInternalV1 }>`及
`NarrativeStableReadinessSnapshotInternalV1 = Readonly<{ entries: readonly NarrativeStableReadinessEntryInternalV1[] }>`。
Root brand只作TypeScript nominal key，runtime不得有symbol/string/extra/accessor field。Snapshot是cached frozen exact `{ entries }`，
entry/vector也frozen、root-first、最多两项；entry exact identities/order不变时repeated read
必须返回same snapshot identity，only entry vector change才mint fresh frozen snapshot/vector。Root preparation private record绑定bridge-private
exact root target/frame与stable preparing evidence，History preparation继续由S4.2.1 installed candidate private provenance认证。
History redemption的prepared-install guard必须在state assignment与同步notification前把exact preparation登记进bridge-private
eventual-session current-preparation slot；该slot在session尚未创建时也必须存在，later session只能adopt同一个preparation而不能重建。
Listener首见installed child时必须已能读到同一个entry，而stale/faulted/guard-abort/reentry不能登记、通知或消费额外authority。
Observation只发布opaque preparation，不含React element、Story renderer/props、portal/container、History content、raw observation port、
input、focus、Coordinator handle、generic candidate/evidence或instance/occurrence ID；snapshot只枚举current exact preparing root/History，
ready、failed、retired、foreign或ABA successor均不出现。

`NarrativeStableSessionInternalV1` exact keys固定为
`getReadinessSnapshotInternalV1(): NarrativeStableReadinessSnapshotInternalV1`、
`subscribeInternalV1(listener: () => void): () => void`、
`getHistoryChildLifecycleInternalV1(): NarrativeStableHistoryChildLifecycleInternalV1`及
`attachHostInternalV1(input: Readonly<{ readonly hostIdentity: object }>): NarrativeStableHostLeaseInternalV1`；
`NarrativeStableHostLeaseInternalV1` exact keys固定为`isCurrentInternalV1(): boolean`与`releaseInternalV1(): void`。Attach严格捕获exact
own-data `{ hostIdentity }`；invalid/stale session在任何read/mutation前抛`ui.narrative_stable_host_attachment_invalid`，distinct/parallel
identity抛`ui.narrative_stable_host_lease_conflict`。Same exact identity reattach返回fresh generation并立即fence predecessor；release
idempotently先关闭本generation，microtask只在仍为same released generation且无successor时清空logical lease，且`.1` release不fail/close
candidate、不改topology。Bridge terminal先fence session/lease/lifecycle、使cached snapshot empty并退订，再走既有dispose；session没有独立
dispose member。

`.2.2.1` exact implementation scope只有以下五个文件：

1. `engine/packages/ui/src/narrative/narrative-managed-surface-family.ts`；
2. `engine/packages/ui/src/narrative/narrative-managed-surface-family.test.ts`；
3. new `engine/packages/ui/src/narrative/narrative-managed-surface-session.ts`；
4. new `engine/packages/ui/src/narrative/narrative-managed-surface-session.test.ts`；
5. `engine/packages/ui/src/public-api.test.ts`。

`.2.2.1` RED必须锁unchanged repeated read same exact snapshot identity、entry vector change才fresh frozen、same-attempt preparation identity reuse、
`[] | [root] | [history] | [root, history]` exact order/cardinality、subscribe reentry/unsubscribe、installed-only History
registration、preexisting lifecycle adoption、admission recreation independence、same-kernel bridge successor、10k bounded state，以及
invalid/distinct/same-identity lease generation与bridge-terminal fencing；并锁住settlement、action route、React/DOM/portal/focus/renderer/
History content均为zero delta。

以下`.2.2.2`段落只保留S4.2.2.0当时的provisional reservation；其中no-arg guard、unguarded failed、single prepared
slot与十一文件broad scope均已由下述S4.2.2.2.0 exact entry supersede，不能再作为RED或implementation authority。
该历史reservation当时允许`.2.2.2`新增root/History terminal settlement。Narrative provisional boundary为
`NarrativeStableHostAttachmentInternalV1`与closed
`NarrativeStableReadinessSettlementResultInternalV1 = Readonly<{ kind: "settled"; completion: null }> |
Readonly<{ kind: "stale"; completion: null }> | Readonly<{ kind: "faulted"; completion: null }>`；attachment exact methods为
...
`settleHistoryReadinessFailedInternalV1(historyPreparation)`与`releaseInternalV1()`。Only-current exact attachment可terminal-once
settle；clone/foreign/old generation/retired preparation均`stale`，planning/internal fault为`faulted`，accepted ready后late failure不得倒写。

Root ready复用generic stable source-bound envelope，但kernel只新增source-relative
`settleStableReadinessReadyWithCommitGuardInternalV1(envelope: ManagedSurfaceStableReadinessEnvelopeInternalV1,
commitGuard: ManagedSurfaceStableReadinessCommitGuardInternalV1): ManagedSurfaceStableReadinessResultInternalV1`；root failed exact复用
`settleStableReadinessFailedInternalV1(envelope)`与既有result。Shared
`ManagedSurfaceStableReadinessCommitGuardInternalV1`只有exact one key
`readonly commitInternalV1: () => boolean`。History不得扩S4.2.1 exact-one-method preparation authority；必须新增独立same-claimant
`ManagedSurfaceStableExactParentTransientChildReadinessAuthorityInternalV1`，其exact methods为
`settleExactParentTransientChildReadinessReadyInternalV1(candidate: unknown, commitGuard:
ManagedSurfaceStableReadinessCommitGuardInternalV1): ManagedSurfaceStableExactParentTransientChildReadinessResultInternalV1`与
`settleExactParentTransientChildReadinessFailedInternalV1(candidate: unknown):
ManagedSurfaceStableExactParentTransientChildReadinessResultInternalV1`。Exact claim function为
`claimManagedSurfaceStableExactParentTransientChildReadinessAuthorityInternalV1(kernel: ManagedSurfaceStableCompositeRuntimeKernelInternalV1, exactClaimant: object)`；
claim必须命中existing
preparation claim record的same exact claimant identity，same claimant重复claim返回same authority；missing/different claimant、foreign
kernel、clone或borrowed receiver在candidate/state read前抛
`ui.managed_surface_stable_exact_parent_transient_child_readiness_claim_invalid`，无release/transfer/second claimant。Result exact为
`Readonly<{ kind: "applied" }> | Readonly<{ kind: "stale" }> | Readonly<{ kind: "faulted" }>`。

Shared guard必须是frozen exact one-method own-data object。Root/History ready都先pure-plan successor + whole-composite reflow，再以prepared
state install提交；ready gate在assignment前exact CAS
candidate phase并调用shared guard，History failed只CAS candidate phase，root failed复用existing gate。Malformed/accessor/extra-key guard为
`faulted` zero；stale epoch/candidate/lease/source、guard `false`或prepared token drift为existing `surface.stale_readiness` zero；guard throw/
non-boolean为`faulted` zero。任何zero/reentry/state drift都不得assignment、notification或terminalize；only installed generic `applied`才map
Narrative `settled`，同步listener首见ready/failed时candidate provenance、Surface、parent suspension/resume与binding必须已经一致；ordinary
Coordinator/runtime readiness对structurally cross-axis History必须继续existing stale zero，不能绕过claimed authority。

现有action binding会立即replace router current，不能用于preparing candidate；因此`.2.2.2`另增
`ManagedSurfacePreparedContractBoundActionBindingInternalV1`与
`prepareManagedSurfaceContractBoundActionBindingInternalV1(input)`。Prepared handle exact one-shot methods为
`commitInternalV1(): boolean`、`abortInternalV1(): void`及
`getBindingInternalV1(): ManagedSurfaceActionBindingV1 | null`。Prepare只可注册inert handler并capture expected router/current contract，
不得推进input publication或替换current binding；commit one-shot/no-throw地CAS exact expected current + reserved revision，abort
idempotently撤销inert registration，get在installed commit前与abort后均为`null`。Candidate gate在preflight closed，不夺取retained/current
owner；`NarrativeStableHostReadyCommitInternalV1`只在exact focus registration与prepared binding均已完成后mint，session prepared-state
install gate
在assignment前调用其no-throw CAS；它不是caller可伪造zero-key token，private record必须绑定exact Host lease generation、candidate、portal
shell、focus registration与prepared binding。Commit/guard只能plain-read module-owned identity/current-generation/bool并做CAS/pointer swap，不能在
kernel transition fence内调用router、Host、bridge、Coordinator或Story callable；abort/stale/fault保持predecessor binding current。
Generic `applied`/Narrative `settled`同步listener首见ready state时focus/binding必须已同批激活，不得形成ready-then-effect或
active-but-invisible窗口。
History prepared binding可认证并拥有child input，但本切片没有close consumer；`ui.cancel`/`player.toggle_history`保持stable consumed且
topology zero，active/preparing exact lifecycle substrate归`.3.1`，Narrative close/input与opener/root + History focus restore归`.3.2`。

`.2.2.2`的expected implementation scope严格只有以下十一文件：

1. `engine/packages/ui/src/managed-surfaces/managed-surface-action-route.ts`及同名`.test.ts`；
2. `engine/packages/ui/src/managed-surfaces/managed-surface-stable-composite-state.ts`及同名`.test.ts`；
3. `engine/packages/ui/src/narrative/narrative-managed-surface-family.ts`及同名`.test.ts`；
4. `engine/packages/ui/src/narrative/narrative-managed-surface-session.ts`及同名`.test.ts`；
5. new `engine/packages/ui/src/narrative/narrative-surface-host.tsx`及`narrative-surface-host.test.tsx`；
6. `engine/packages/ui/src/public-api.test.ts`。

React Host必须从explicit Narrative portal container挂载，以instance-stable key保留retained root + History subtree；preparing shell在
ready前必须正确portal/fallback/inert/`aria-hidden`，render/layout成功后于microtask重验same session/lease/generation/preparation/portal/
shell才settle ready，pre-ready render/layout/effect fault才settle failed。Accepted ready后的error必须交回outer diagnostics/error owner，
不得倒写readiness failure；StrictMode fake unmount、same-identity reattach、portal detach、replacement retain/failure、History preparation/
failure与listener retirement均须terminal-once。Candidate renderer/history-observation/Host component/factory/props的exact spellings尚未由
现有source证明；`.2.2.2`进入RED前必须先以自己的docs-only entry冻结，不能由implementation自行发明或把opaque raw port直接暴露给
React/Story。同一entry还必须冻结ready History provenance对generic Coordinator close/dismiss/owner mutation的existing-code zero-delta
negative guard，并在`.2.2.2`自己的entry里再冻结具体code；`.2` RED必须证明该zero delta，不能让`.3`未来authority在交付前已有旁路，
否则停止RED并修订entry。

两批仍全部package-internal/source-relative；每个新增top-level name、session/lease/attachment/authority/guard/prepared-binding member与
runtime spelling均须在UI root、`./internal`及package export inventory有type/runtime negative guard。不得修改UI barrel、package
exports、existing S4.2.1 preparation authority shape、public runtime-kernel/Coordinator/Surface contract、Base/Web、input-router source、
reducer、stable-contract、CSS、GameStage/composer/`DefaultGameRootV1`、Engine Lab/template/examples、legacy
`DialoguePanelV1`/`VnLayerV1`或live architecture/features/development/website。

Hard stop：若`.1`无法只靠bridge/composite notification发布opaque readiness preparations，History registration不能在existing redemption
prepared gate内assignment前闭合，或single Host lease需要React/DOM/settlement，停止并修订`.1`；不得越过five-file scope。若`.2`不能用
root guarded-ready + 独立same-claimant History readiness authority闭合，prepared action/focus commit必须调用external callable，History
observation必须暴露mutable/raw Story state，renderer必须取得Coordinator/Surface evidence或direct semantic authority，或Host-commit需要
新的async renderer promise/stream contract，或ready History无法对generic Coordinator close/dismiss/owner mutation保持existing-code
zero delta且将具体code留给`.2.2.2` own entry冻结，先落`.2.2.2` docs-only entry并停止RED；不得以ready-then-effect、raw slot lookup、
second session/topology/input writer、
generic/public API或live cutover绕过。Generic exact History-child lifecycle留`.3.1`，Narrative close/input/focus/opener留`.3.2`，
clock/reveal/Auto/Skip timer/suspension留`.4`，Engine Lab
wiring留`.5.1`，GameStage Narrative portal/composer/Story migration与legacy writer/export deletion留S4.3。

本entry严格只修改三份owning planning/design文档中的合同文本；本cross-plan change本身只修改本文件，没有source、test、runtime、
architecture、React Host、live claimant或public API diff。验证只要求相关文档`deno fmt --check`与`git diff --check`，不得引用S4.2.1的
focused/UI/full/check矩阵冒充本docs-only checkpoint的新runtime evidence。S4.2.2 broad entry现只作superseded historical checkpoint；唯一
`.2.2.0`与下述`.2.2.1`现均作completed historical checkpoint；该checkpoint当时的linear core/current/next与implementation gate为
**S4.2.2.2.1 DOM-free generic Host-commit atomic substrate**；该`.1` gate现已由下述delivery关闭。

**PF4/S4.2.2.1 DOM-free Narrative session/readiness attachment implementation delivery（已完成的历史
checkpoint）：** implementation严格落在上述five-file scope：family source/test、new session source/test与
`public-api.test.ts`。`narrative-managed-surface-session.ts`只有session/lease/root/readiness type declaration与type-only
import；factory、private records、bridge/lifecycle WeakMap access、subscription与全部runtime mutation都留在
`narrative-managed-surface-family.ts`，没有新增helper export、raw bridge accessor、binding claim或second topology owner。

每个exact active bridge只缓存one session与one History lifecycle，repeated factory返回same identity，lifecycle-before-session被
later session exact adopt；same-kernel bridge successor继续复用S4.2.1 per-kernel claimant/preparation authority，但取得fresh
session/lifecycle并使old objects永久stale。Root以fresh frozen zero-key token绑定current stable preparing attempt；History复用
S4.2.1 exact preparation。Redemption guard在composite assignment与同步notification前已登记preparation record、candidate与
bridge-private eventual-session slot，因而raw listener在session创建前读取或later session adopt都只看到same token；
stale/faulted/guard-abort保持zero registration/spend/notification。

Session只投影current root/History preparations。Cached frozen snapshot exact为`{ entries }`，root-first、cardinality `0..2`；
entry vector的exact identity/order未变时repeated read返回same snapshot，只有vector变化才mint fresh frozen
entry/vector/snapshot并通知active subscribers。Listener throw被隔离；早listener同步terminalize bridge会停止captured
listener vector，external Coordinator terminal即使先被raw listener观察也会在factory/attach/lease currentness ingress立即fence，
不留assignment-before-session-observer窗口或terminal后notification。

Host lease只保留single logical identity与current generation：same identity reattach立即fence predecessor，distinct identity在
kernel/snapshot read前conflict，release先幂等关闭own generation；每session只有one scheduled microtask flag，只在无
same-host successor时清理released generation。Opaque provenance使用WeakMap；bridge strong slots只保留current lifecycle/
History preparation/session，session只保留current root preparation/entries/snapshots/Host lease、active-subscriber Set、one unsubscribe与
scalar booleans。Unsubscribe/terminal会删除active listener，`10k` snapshot/attach/release/subscription churn不累积attempt、preparation、
generation、listener、timer或tombstone history，强保留保持O(1)。

UI root、`./internal`与package inventory已对全部new top-level type/factory与session/lease member同时加type/runtime
negative guard。UI barrels、package exports、stable-composite/reducer/action-route、public/generic Coordinator/runtime-kernel contract、
ready/fail、action/input/focus、React/DOM/portal/renderer、close/dismiss、timer/controller、Base/Web、Engine Lab/Stories/live graph均为
zero expansion。验证通过focused **3 files / 209 tests**、UI package **80 files / 1222 tests**、full
**254 files / 4152 tests**，以及完整`deno task check`、`deno task typecheck`、`deno task lint`、相关文件
`deno fmt --check`与`git diff --check`。本source-relative dormant delivery未重跑browser/examples/prebuilt；先前Engine browser
`101 / 101`、examples browser `45 passed / 2 skipped`与prebuilt Player `38 / 38`只作prior evidence，不冒充本delivery
HEAD新验证。该delivery当时把current推进到原 **S4.2.2.2 dormant React Narrative Host own docs-only exact entry
before RED**；该entry随后由下述`.2.2.2.0`关闭并重切，当时的唯一implementation gate为`.2.2.2.1`；该`.1`与
`.2.2.2.2` gate现亦已由下述delivery关闭；原S4.2.3 exact-entry gate现由下述completed `.3.0`关闭并重切，
...
当前唯一direct RED/implementation gate为 **PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**；唯一有效顺序为
**S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。
...
StrictMode lease与real-detach cleanup混在一个十一文件checkpoint；现有immediate action binding与只枚举preparing token的
S4.2.2.1 readiness snapshot都不能冒充ready前atomic handoff或ready/retained render source。本entry只冻结合同并把唯一顺序重切为：

1. **S4.2.2.2.0（本entry，completed）**：只冻结下述exact names/shapes、atomic protocol、RED、scope与stop；
2. **S4.2.2.2.1 DOM-free generic Host-commit atomic substrate（已完成）**：只改action-route pair、stable-composite pair与
   `public-api.test.ts`共五文件，交付prepared action binding、guarded root/History readiness、cross-axis child action authority与generic
   bypass fence；不接Narrative/React/DOM/portal/renderer；
3. **S4.2.2.2.2 dormant Narrative React Host（completed）**：只改family pair、session pair、new Host pair与`public-api.test.ts`共七文件，
   交付immutable renderer/History observation、Host runtime/component、explicit portal、layout acknowledgment、prepared binding与
   Host-local focus registration、StrictMode generation、pre-ready error boundary及real-detach terminal cleanup；
...
`.2.2.2.1`新增source-relative
`PrepareManagedSurfaceContractBoundActionBindingInputInternalV1`，exact own-data fields只有`authority`、`inputContextId`、
`inputRouter`、`isGestureCurrent`与optional `registerManagedInputHandler`。Factory exact为
...
handle exact methods只有
`commitInternalV1(contract: ManagedSurfacePreparedInputBindingContractInternalV1): boolean`、`abortInternalV1(): void`与
`getBindingInternalV1(): ManagedSurfaceActionBindingV1 | null`。Prepare只descriptor-capture并建立inert dispatcher，不猜future contract、
不替换current binding、不推进input publication；readiness pure successor派生的full contract必须先capture为下述token才可交给commit。Consumer必须在commit前以
`claimManagedSurfacePreparedAuthenticatedActionRouteInternalV1<TAttempt, TResult>(prepared, consume):
ManagedSurfaceAuthenticatedActionRouteInternalV1<TAttempt, TResult>`preclaim；first exact claim独占，clone/foreign/aborted/second claim在router或
consumer调用前fail closed。Commit one-shot/no-throw，abort no-throw/idempotent；只有successful commit后get才返回exact installed binding，
commit前、abort或false/stale后均为`null`。

Raw full contract不能直接进入kernel transition fence。Action-route另增fresh frozen exact zero-key
`ManagedSurfacePreparedInputBindingContractInternalV1`与
`captureManagedSurfacePreparedInputBindingContractInternalV1(contract: ManagedSurfaceInputBindingContractV1)`；capture在transition外
descriptor-validate/freeze raw contract并以module-owned WeakMap认证token。Clone/foreign/borrowed token、raw contract、accessor或已被其他
prepared handle消费的token都必须在gate内只靠WeakMap/identity lookup返回false，不读caller descriptor、不触发trap且不throw。

每个exact `(InputRouterV1, inputContextId)`最多one dispatcher registration并冻结first creation取得的exact registrar identity；Narrative只占
one such context。Existing context上的later alternate registrar必须ignored/unread而非冲突或替换。该context record只保留one current与最多
**two** inert prepared records（root replacement与History并行时，各exact authority只留latest）；prepared record不另注册handler，也不保留
retired cleanup/unregister history。Third distinct authority必须在registrar/router read前以existing authority conflict拒绝；same-authority newer prepare
在transition fence外terminal-abort并替换old latest slot。Prepare只预留private scalar high-water revision而不发布current；abort/stale可burn gap，
只有commit发布reserved revision。Prepared commit只做one-shot/no-throw module-owned expected-current/pointer/revision CAS；两个并行prepared first
commit wins，loser因expected-current drift稳定false，只有其Surface candidate仍current时才可abort后fresh prepare/retry。Logical predecessor
立即stale且inactive dispatcher稳定ignored；10,000轮prepare/abort/commit/competitor/retry不得积累handler、retired cleanup/unregister、revision/
candidate/tombstone或array history。

Shared `ManagedSurfaceStableReadinessCommitGuardInternalV1`仍为frozen exact one-method own-data object，但唯一method精确为
`commitInternalV1(contract: ManagedSurfacePreparedInputBindingContractInternalV1 | null): boolean`。Root Host路径新增
`settleStableReadinessReadyWithCommitGuardInternalV1(envelope: ManagedSurfaceStableReadinessEnvelopeInternalV1, commitGuard:
ManagedSurfaceStableReadinessCommitGuardInternalV1): ManagedSurfaceStableReadinessResultInternalV1`与同参同result的
`settleStableReadinessFailedWithCommitGuardInternalV1`；existing unguarded failed不作为Host路径。History不得扩
S4.2.1 one-method preparation authority，而由same exact claimant领取独立
`ManagedSurfaceStableExactParentTransientChildReadinessAuthorityInternalV1`与closed
`ManagedSurfaceStableExactParentTransientChildReadinessResultInternalV1 = Readonly<{ kind: "applied" }> |
Readonly<{ kind: "stale" }> | Readonly<{ kind: "faulted" }>`。Authority的
`settleExactParentTransientChildReadinessReadyInternalV1`与
`settleExactParentTransientChildReadinessFailedInternalV1`都接收
`(candidate: unknown, commitGuard: ManagedSurfaceStableReadinessCommitGuardInternalV1)`并返回该result；claim exact仍为
`claimManagedSurfaceStableExactParentTransientChildReadinessAuthorityInternalV1(kernel, exactClaimant)`。Same claimant复用，missing/
different claimant、foreign kernel或borrowed receiver在candidate/state read前以既有readiness-claim invalid拒绝；无release/transfer/
second claimant。

Cross-axis action另增closed
...
Readonly<{ kind: "faulted"; code: "surface.stable_reconcile_faulted" }>`与
`ManagedSurfaceStableExactParentTransientChildActionRouteAuthorityInternalV1`。该authority extends
`ManagedSurfaceContractBoundActionRouteAuthorityInternalV1`，新增exact
...
`claimManagedSurfaceStableExactParentTransientChildActionRouteAuthorityInternalV1(kernel, exactClaimant):
ManagedSurfaceStableExactParentTransientChildActionRouteAuthorityInternalV1`，并必须命中S4.2.1 same kernel/same exact claimant。Only
authenticated current ready History可captured；preparing/failed/retired、parent/source/slot drift或ABA successor均unavailable；它只拥有
`surface.action_routed` continuation gate，不取得close/dismiss authority；same claimant重复claim复用same retained authority，且无release/
transfer/second claimant。

Root/History ready与failed统一先pure-plan whole-composite successor、从该successor派生future full input contract，并在transition fence外
capture为authenticated zero-key token或exact `null`，再进入prepared install gate。Root initial failure传`null`；root replacement failure与
History failure必须传恢复后的retained-root token，
由Host在同一gate commit预备的retained-parent binding并abort candidate binding。Ready/fail guard在Surface assignment前只做prepared
binding、candidate phase、Host generation与Host-local focus-registration plain pointer/CAS；不得调用router/registrar/unregister、DOM/React/
Story/History observation/bridge/Coordinator/semantic consumer或任意caller callable。同步listener首见ready或failed/resumed state时必须已见
exact new binding contract；ready/fail first winner独占，late opposite、stale evidence、guard false/throw/nonboolean、plan/prepared drift均不
assignment、不notification、不terminalize。Listener同步replace/release/dispose后outer仍只返回historical applied/settled。

Generic bypass按stale evidence first、applied-only fence second：stale epoch/instance/readiness/routing/owner evidence保持既有exact code；只有
ordinary generic transition原本会applied到authenticated current cross-axis History时才回滚exact current composite state并统一返回
`rejected / surface.invalid_transition`。Fence覆盖ordinary readiness ready/failed、close expected/top、dismiss、两个History action、selective
owner cancel/dispose与direct child mutation；topology、input publication、cursor、binding与notification均zero。Claimed readiness、
S4.2.1 retain/cutover/empty/publisher或bridge disposal/Coordinator terminal structural cascade继续applied，不能误拦；S4.2.3.1只能新增
separate same-claimant exact current History-child lifecycle authority，不能删除该generic fence。

`.2.2.2.2`冻结raw
`NarrativeStableHistoryObservationPortInternalV1` exact methods为
`getSnapshotInternalV1(): DeepReadonly<NarrativeHistoryV1>`与`subscribeInternalV1(listener: () => void): () => void`；preflight只把它
descriptor-capture为zero-key `NarrativeStableCapturedHistoryObservationPortInternalV1`。Malformed present port、custom prototype、accessor或proxy
trap可在source-private boundary以`ui.narrative_stable_history_observation_invalid` TypeError分类，但observable result必须getter-zero地collapse为
existing `{ kind: "faulted"; code: "narrative.candidate_preflight_faulted" }` exact zero；不能新增public/family rejection code或让raw exception越界。Host
renderer只取得
`NarrativeStableHistoryRenderObservationInternalV1`，其exact methods同为`getSnapshotInternalV1`/`subscribeInternalV1`，不得取得raw port、
mutable Story state、Coordinator/evidence或semantic writer。Render wrapper按bounded canonical `NarrativeHistoryV1` bytes缓存frozen snapshot，
不按raw object identity：equal canonical bytes复用same exact snapshot，different bytes才mint fresh；same raw identity被原地改成different bytes也必须
fresh canonical copy。每个captured raw port最多one underlying subscription，且只在canonical bytes变化时通知Host subscriber；raw identity与
mutable raw value永不暴露。

Renderer contract exact names为`NarrativeStableDialogueRendererPropsInternalV1`、
`NarrativeStableHistoryRendererPropsInternalV1`、`NarrativeStableRendererPropsInternalV1`与
`NarrativeStableRendererComponentInternalV1`。Dialogue props exact fields为`kind: "dialogue"`、`pending`、`visualConfig`、
`playerProfile`、`textResolver`、`quickMenuContribution`；History props exact fields为`kind: "history"`、`history`、`visualConfig`、
`playerProfile`与`textResolver`。同一个candidate renderer接收该closed discriminated union；不得把raw reveal/controller/action authority交给
renderer。DOM-free Reveal、Auto/Skip clock与suspension归S4.2.4.2，Host player-view integration归S4.2.4.3。

Host render contract exact names为`NarrativeStableHostRenderPhaseInternalV1 = "preparing" | "active" | "suspended"`、root/History
closed `NarrativeStableHostRenderEntryInternalV1`、`NarrativeStableHostRenderSnapshotInternalV1`与
`NarrativeStableHostRenderSourceInternalV1`。`NarrativeStableHostRenderKeyInternalV1`是engine-minted opaque branded string type，不是runtime
zero-key handle；它由family-private monotonic allocator生成，不能编码/解析instance/occurrence或接受caller string重建。Entry以exact
`renderKey: NarrativeStableHostRenderKeyInternalV1`保持identity；
closed entry rows以`kind: "dialogue" | "history"`区分；两行都有`phase`、`preparation`、`initialFocusTargetId:
ManagedSurfaceFocusTargetIdV1`、`rendererComponent: NarrativeStableRendererComponentInternalV1`与`rendererProps`。Dialogue row持有full
`NarrativeStableDialogueRendererPropsInternalV1`；History row另有same render-key type的exact parent `parentRenderKey`、
`rendererProps: Omit<NarrativeStableHistoryRendererPropsInternalV1, "history">`及
`historyObservation: NarrativeStableHistoryRenderObservationInternalV1`。Same candidate的preparing → active/suspended必须复用same exact render
identity而不remount；`preparation`只在preparing时为exact root/History token，active/suspended时为`null`。Snapshot exact frozen `{ entries }`、
cached、parent-before-child、
retained aggregate before successor、cardinality `0..3`；max-three精确容纳retained root + History + preparing replacement。只有entry identity/order/
phase/frozen candidate identity变化才fresh；History canonical bytes只更新independent observation snapshot，不重建Host entry。Ready cutover同一
notification后只保留successor。Render source exact methods为
`getSnapshotInternalV1(): NarrativeStableHostRenderSnapshotInternalV1`与`subscribeInternalV1(listener: () => void): () => void`；它与
S4.2.2.1只枚举preparing token的readiness snapshot是两个不同projection，不能互相冒充。

Host runtime exact names为`CreateNarrativeStableHostRuntimeInputInternalV1`、`NarrativeStableHostRuntimeInternalV1`与
`createNarrativeStableHostRuntimeInternalV1(input)`。Input exact fields为`session`、`hostIdentity`、
`portalContainer: HTMLDivElement`、`inputRouter`与`isGestureCurrent`；runtime exact frozen shape只有`attachment`与`renderSource`。
Factory必须在调用`session.attachHostInternalV1`前完成own-data/portal/router/callback与existing Host/portal conflict validation；若取得fresh lease后
任何later runtime/render-source construction仍失败，必须立即release该lease而不留下logical Host占用。
Closed `NarrativeStableReadinessSettlementResultInternalV1`精确为
`Readonly<{ kind: "settled"; completion: null }> | Readonly<{ kind: "stale"; completion: null }> |
Readonly<{ kind: "faulted"; completion: null }>`。`NarrativeStableHostAttachmentInternalV1`继续exact only
`settleRootReadinessReadyInternalV1(rootPreparation, readyCommit)`、
`settleRootReadinessFailedInternalV1(rootPreparation)`、
`settleHistoryReadinessReadyInternalV1(historyPreparation, readyCommit)`、
`settleHistoryReadinessFailedInternalV1(historyPreparation)`与`releaseInternalV1()`；四个settlement method都返回该result，failed public method仍只接
preparation，但private runtime必须预备上述retained-parent binding或null handoff。Invalid input沿用
`ui.narrative_stable_host_attachment_invalid`，distinct Host沿用`ui.narrative_stable_host_lease_conflict`；same host不同portal在current或
released grace内以`ui.narrative_stable_host_portal_conflict`于任何kernel/render/router read前拒绝。

`NarrativeStableHostReadyCommitInternalV1`为fresh frozen exact zero-key token。Exact mint为
`prepareNarrativeStableHostReadyCommitInternalV1(input: PrepareNarrativeStableHostReadyCommitInputInternalV1):
NarrativeStableHostReadyCommitPreparationResultInternalV1`；input exact own-data fields为
`{ hostRuntime, renderEntry, portalShell: HTMLDivElement, initialFocusTarget: HTMLElement }`，closed result精确为
`Readonly<{ kind: "prepared"; readyCommit: NarrativeStableHostReadyCommitInternalV1; completion: null }> |
Readonly<{ kind: "reattached"; completion: null }> | Readonly<{ kind: "stale"; completion: null }> |
Readonly<{ kind: "faulted"; completion: null }>`。Prepared token private record绑定exact runtime/attachment generation/render entry/portal/
shell/initial target/prepared binding与Host-local focus-registration；shared guard是其唯一activation CAS。Actual `.focus()`、trap、restore与
History opener仍归S4.2.3.2，不能在kernel fence内执行。Engine-owned shell必须以`tabIndex={-1}`注册entry exact
`initialFocusTargetId`；本slice只原子切换registration ownership，不能调用`.focus()`。

Family不得新增Narrative-parallel child action authority name；`.2`只用per-kernel same claimant private claim `.1`的generic child action authority，
从authenticated ready History candidate取得future contract并preclaim family consumer。Preparing candidate永不取得input；在`.3` close authority尚未
存在时，History的`ui.cancel`与`player.toggle_history`必须stable consumed且topology exact zero，Host generation fenced/terminal late route也不得
fall through到root/lower context。

React exact names为`NarrativeSurfaceHostPropsInternalV1`与`NarrativeSurfaceHostInternalV1`；props exact fields只有`session`、
...
diagnostics owner，不能倒写failure；event handler、passive effect、Promise/timer/async/Persistence fault不在本boundary内。Visual phase必须精确：
`preparing`为`visibility: hidden`的hidden-with-layout且inert/`aria-hidden`/pointer-disabled，`suspended`仍mounted并在blocking History后视觉保留但
inert/`aria-hidden`/pointer-disabled，且不得`visibility: hidden`；`active`才visible/interactive。三者都不得用`display: none`或HTML `hidden`
移除layout。

Host release/real detach立即fenceold generation attachment ack、ready commit/mint、action consumer/focus generation token、new intent/terminal
acknowledgment及late callback，但logical dispatcher/current binding/focus ownership slot继续保留到合法successor接管或final cleanup，不能在grace
起点提前撤销；无Host grace内该logical consumer必须经generation fence stable-consumed/no-op，不能fall through。Same host + same portal只可在
one coalesced microtask grace内以fresh generation续接并返回`reattached`，不得settle或重建render identity；distinct Host或same host + different
portal均不能夺取grace slot。Grace
到期仍无successor时先terminal-fence old runtime的factory/attach/ack/render-subscriber全部external ingress，再按root-first、History-second
顺序对仍current pending preparation逐个terminal-once failed，并在每次同步notification后重验session/generation/portal/preparation/
terminal ownership，因为listener可replace或terminalize。某次failure若因cascade成为stale或faulted，只要runtime仍ownerless就不能阻止最终
cleanup；若重验发现合法successor或已由其他terminal owner接管才停止旧cleanup。Pending drain后先terminalize session/attachment/render
source/subscriptions、current binding/action consumer/focus tokens与全部late callback，再调用factory早已captured的exact bridge dispose callable，
使ready/retained subtree通过existing structural cascade退役而不能成为invisible live subtree；不得用generic close模拟detach。Cleanup/dispose
throw或fail-closed result必须contain且session保持永久terminal。Already terminal bridge/Coordinator只silent fence且不double dispose/notify。
Grace expiry已经terminalize old session/bridge；之后若需更换portal，只能由fresh bridge/session successor重新attach，不能复活old runtime或
沿用old render/binding identity。

`.2.2.2.1` RED至少覆盖exact shapes/negative guards、raw-contract capture与zero-key token clone/foreign/accessor/borrowed rejection、root/History
ready+fail nullable/token handoff、per-context single dispatcher/first registrar freeze与later alternate unread、最多two concurrent prepared records、
private revision reserve/burn/publish、first-commit-wins/fresh retry、listener first observation/reentry、generic bypass stale precedence、same-claimant
action/readiness claims、retain/cascade及无retired cleanup history的10k bounded churn。其implementation scope exact仅：
...
successor max-three render order、pre-attach validation与post-lease construction failure release、portal/shell/focus-target drift、named ready-mint input/
result四行`completion: null`、StrictMode release/reattach、same-host portal conflict、History two-action consumed/zero、real detach pending drain +
per-notification recheck + structural ready/retained cascade及pre/post-ready error boundary，并锁root-first/History-second drain、failure stale/fault仍final
dispose及terminal pre-fence all ingress；另覆盖public/runtime negatives及10k bounded Host churn。其implementation scope exact仅：
...
两批全部source-relative/package-internal。每个new top-level type/value（含`NarrativeStableHostRenderKeyInternalV1`与ready-mint input/result）、
prepared-contract token/capture factory、guard/authority/capture/prepared/attachment/render-source/runtime/Host
member spelling都必须有UI root与`./internal`双`@ts-expect-error`及package runtime/type inventory negative guard；不得修改UI barrel/package
exports、S4.2.1 one-method authority、public/generic Coordinator/runtime-kernel/Surface contract、reducer/input-router/stable-contract source、
Base/Web/CSS/GameStage/composer/`DefaultGameRootV1`、Engine Lab/template/examples、legacy `DialoguePanelV1`/`VnLayerV1`或live
architecture/features/development/website。

Hard stop：`.1`若必须在guard内调用external callable、读取caller descriptor/trap、修改generic operation/result/code或越过five-file scope，
若nullable token不能从pure successor的raw full contract在transition外capture，若two concurrent prepared记录无法保持per-context one
dispatcher与scalar O(1)，
或generic fence会拦截claimed/structural cascade，停止并修订。
`.2`若必须暴露raw/mutable History/Story/Coordinator authority、用readiness snapshot渲染ready subtree、在guard内调用DOM/React/router、
以generic close处理detach、需要async renderer promise/stream、parallel Host或越过seven-file scope，也停止并修订。History exact lifecycle
substrate归S4.2.3.1，Narrative close/input与root + History actual DOM focus/opener归S4.2.3.2，generic prepared
state-install participant substrate归S4.2.4.1，DOM-free controller/reveal/timer/suspension归S4.2.4.2，dormant Host
player-view integration归S4.2.4.3，Engine Lab归S4.2.5.1c，GameStage/Story/live cutover归S4.3。

本entry在三份owning design/planning docs中同步exact contract；本cross-plan change本身只修改本文件，没有source/test/runtime/architecture/
React Host/public/live diff，也没有新runtime delivery evidence。只要求本文件`deno fmt --check`与`git diff --check`；S4.2.2.1的focused/UI/
full/check、browser/examples/prebuilt prior evidence均不能冒充本docs-only checkpoint验证。原S4.2.2.2 broad checkpoint与本`.0`现均为
...
primary RED严格落在action-route pair、stable-composite pair与`public-api.test.ts`五文件；本entry明确授权的五份test-only
characterization也同步迁移，没有修改Coordinator、composition runtime、Narrative production source、router、generic operation/result/code、
barrel/package或live graph。

Action route新增descriptor-captured raw contract到frozen zero-key token、inert prepared handle与pre-commit authenticated consumer claim。
每个exact router/context只保留one stable dispatcher、one logical current与最多two latest-per-authority prepared records；first registrar
identity冻结，later registrar连descriptor都不读，可能已产生副作用的initial registrar失败后context永久poison而不重注册。One scalar revision
high-water在prepare时reserve，abort/stale burn gap，commit才以plain token/slot/expected-current CAS发布；成功或terminal失败都清除predecessor
strong link。Same-event router reentry在authority、gesture与consumer gate前exact-once，binding replacement/dispose只fence logical current，
不把physical dispatcher unregister callback误作lifetime owner。

Stable composite新增root guarded ready/failed、独立same-claimant History readiness authority及ready History action capture authority。Pure successor
先派生future contract并在transition外capture token；guard只允许prevalidated commit callable与module-owned CAS，只有成功才在state assignment和
同步notification前完成binding/candidate provenance。Ordinary generic readiness、close、dismiss、History actions与owner mutation命中current
authenticated cross-axis child时保留stale evidence precedence，其余统一`rejected / surface.invalid_transition` exact zero；claimed readiness与
replacement/empty/publisher-disposal/Coordinator-terminal structural cascade保持applied。Live candidate WeakMap只保留origin与bounded provenance，
不反向持有predecessor state或第二topology。

UI-wide characterization现锁定stable dispatcher在replacement/terminal后仍one registration/zero unregistration，同时old logical binding、gesture与
Coordinator ingress同步fail closed；Narrative旧cleanup改走合法root cutover structural cascade而没有重开generic `readiness_failed` bypass。
UI root与`./internal`对全部new top-level/member spelling的type/runtime negatives保持green；无public/package/live扩张。

验证通过focused **3 files / 119 tests**、UI package **80 files / 1244 tests**、full
**254 files / 4174 tests**与完整`deno task check`（含format、lint、styles、typecheck、determinism、assets、all Story checks及e2e build）。
本source-relative delivery未重跑browser/examples/prebuilt；先前Engine browser `101 / 101`、examples browser
`45 passed / 2 skipped`与prebuilt Player `38 / 38`只作prior evidence。`.2.2.2.0/.1/.2`现均为completed historical
checkpoints；原S4.2.3 exact-entry gate现也由下述completed `.3.0`关闭并重切，`.3.1`又已由下述delivery关闭。
...
slice与direct RED/implementation gate现为 **PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**；唯一有效顺序为
**S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。
...
package-internal与dormant。UI barrel/package export、generic Coordinator/runtime-kernel/Surface contract、Base/Web/CSS、GameStage/
composer/`DefaultGameRootV1`、Engine Lab/template/examples、legacy `DialoguePanelV1`/`VnLayerV1`及live application wiring均无扩张。

History observation现在descriptor-capture exact raw port，只发布canonical-byte比较后的frozen copy与identity-stable snapshot；每个active
observation最多one raw subscription，只在canonical bytes变化时通知，retire先exact-once unsubscribe并清除raw binding/listener holder，late raw
callback为zero ingress，caller-retained observation只返回最后cached canonical identity。Immutable render source按parent-before-child、
retained-before-successor发布cardinality `0..3`的initial/replacement root与History entry，同vector/phase复用exact entry/snapshot identity；removed/
terminal entry同步删除preparation、History observation与render-record强链接，terminal source固定返回one cached empty snapshot且late subscribe为
frozen idempotent no-op。

Narrative Host runtime与React Host只面向explicit `HTMLDivElement` portal；preparing、suspended与active shell保持各自hidden-with-layout/
inert/visible语义。Root与History都经zero-key ready token、prepared action binding及guarded readiness完成atomic ready/fail handoff；每个render
entry的focus target、portal shell、runtime generation与action generation一起重验并在same commit安装。Same logical Host + same exact portal可在one
coalesced microtask内以fresh runtime generation reattach exact render identity与per-entry focus，old attachment/ready mint/action/focus callback立即
stale；real detach按root-first、History-second drain pending，并在captured bridge dispose前terminal-fence session、render source、subscriber、
binding/focus及全部late callback。History的two exact actions在preparing、ready与Host-fenced窗口保持stable consumed/no-op，不重开generic bypass。

Production strong state只保留one current Host runtime/attachment/render source、one active subscriber set、one cleanup microtask、max-three render
entries、scalar render-key high-water与per-current-candidate action/focus/ready records；release、successor、partial construction failure与external
terminal共享scrub/delete路径，不积累generation、renderer、preparation、timer或tombstone history。Mutation-sensitive bounded row以真实
`createNarrativeStableHostRuntimeInternalV1`完成`10,000`轮release → same-host/same-portal create → active-entry `reattached`，同时锁old runtime
ready-mint `stale`、old attachment settlement no-op、shared render source/snapshot identity与final detach terminal empty。UI root、`./internal`及package
runtime/type inventory对全部new top-level与member spelling保持negative guard，未形成public API或live wiring。

验证通过focused **4 files / 238 tests**、UI package **81 files / 1273 tests**、full **255 files / 4203 tests**，以及完整
`deno task check`、`deno task typecheck`、`deno task lint`、相关文件`deno fmt --check`与`git diff --check`。本HEAD未重跑
browser/examples/prebuilt；先前Engine browser `101 / 101`、examples browser `45 passed / 2 skipped`与prebuilt Player `38 / 38`只作prior
evidence。S4.2.2.2.2据此转为completed historical checkpoint；其原S4.2.3 exact-entry pointer现由下述completed `.3.0`
关闭并重切，`.3.1`也已由下述delivery关闭。linear core/current/next与implementation gate现统一推进到
下述completed **S4.2.3.2 dormant Narrative close/input/root + History focus Host lifecycle**；该delivery现又把live
...
broad implementation checkpoint也已由`.5.1a`–`.5.1c`细分并转为historical，`.5.1a`、`.5.1b`与`.5.1c`现也已由上述delivery关闭。唯一current/next、core
slice与direct RED/implementation gate为 **PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**，后续顺序为
**S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。
...
physical dismiss混在一个checkpoint；它现只作superseded broad checkpoint，不再直接进入RED或implementation。唯一有效线性顺序冻结为：

1. **S4.2.3.0（本entry，completed）**：只冻结下述exact names/shapes、atomic protocol、RED、file scope与stop；
2. **S4.2.3.1 DOM-free generic exact History-child lifecycle substrate（completed）**：只交付separate same-claimant
   exact-child close/dismiss authority与whole-composite atomic handoff，不接Narrative/React/DOM/Host；
3. **S4.2.3.2 dormant Narrative close/input/root + History focus Host lifecycle（completed）**：一次性交付candidate-bound
...
`.3.1`新增独立、source-relative、package-internal
`ManagedSurfaceStableExactParentTransientChildLifecycleCommitGuardInternalV1`，其frozen exact sole member为
`commitInternalV1(contract: ManagedSurfacePreparedInputBindingContractInternalV1): boolean`。这里的contract必须是transition外从
pure successor capture的authenticated **nonnull** token：History close/dismiss必然恢复surviving exact Dialogue parent的full input
contract，不能用`null`冒充closed composite，也不能在kernel transition fence内读取raw contract、调用router或重建binding。

Closed generic result精确命名为
`ManagedSurfaceStableExactParentTransientChildLifecycleResultInternalV1`，且只有以下frozen exact rows：
...
Lifecycle authority精确命名为
`ManagedSurfaceStableExactParentTransientChildLifecycleAuthorityInternalV1`，其frozen exact methods只有：

- `closeExactParentTransientChildInternalV1(candidate: unknown, guard:
...
- `dismissExactParentTransientChildInternalV1(candidate: unknown, dismissKind: ManagedSurfaceDismissKindV1,
  guard: ManagedSurfaceStableExactParentTransientChildLifecycleCommitGuardInternalV1):
  ManagedSurfaceStableExactParentTransientChildLifecycleResultInternalV1`。

Exact claim为
`claimManagedSurfaceStableExactParentTransientChildLifecycleAuthorityInternalV1(kernel, exactClaimant)`。它是与
S4.2.1 preparation、S4.2.2.2.1 readiness/action claims分离的fourth authority record，但必须先命中same kernel的existing
preparation claim及**same exact claimant**；same claimant repeat返回same retained authority，different/missing claimant、foreign kernel、
borrowed receiver或spoof在candidate/state read前以
`ui.managed_surface_stable_exact_parent_transient_child_lifecycle_claim_invalid`抛TypeError。它没有release、transfer、
second claimant或Narrative-parallel claim。

Generic lifecycle algorithm固定为authenticated candidate first、pure whole-composite successor second、nonnull contract capture与prepared
install third、guard CAS与state assignment last。Explicit close先认证candidate仍是current exact top History child，再对preparing与ready
两相都使用existing **`close_top`** pure operation；不得使用`close_expected`、`readiness_failed`、generic Coordinator handle或任何
`*_with_owner_preparation_cancel` variant。Dismiss在candidate preparing时使用existing
`route_fallback_dismiss_exact_candidate`，ready-active时使用existing `route_dismiss`；invalid dismiss kind必须在policy lookup与guard前
faulted/zero，locked policy返回`locked / surface.dismiss_locked`且不调用guard。Successful close精确映射
`applied / surface.closed`，successful dismiss精确映射`applied / surface.dismissed`。

Pure successor必须只退休exact History child、恢复同一Dialogue parent为active并一次性重建whole-composite input/publication；same-owner
root replacement preparation、retained root DOM/session identity、semantic frame、History content与allocation high-water保持不变。Stale
candidate/currentness必须先于policy与guard capture/invocation返回canonical `stale`。Guard只在prepared state仍installable且
candidate/provenance/policy仍exact时接收nonnull token并调用一次；只有exact `true`才允许assignment，guard `false`返回canonical
`stale`，throw或nonboolean返回canonical `faulted`；token/candidate/lineage drift同样不得产生partial binding、topology、input
publication、notification或owner-preparation cancel。State assignment完成后才解锁并同步通知；listener首见child absent时必须已经看见
parent resumed及restored binding，listener立即开
successor History时outer close不得命中successor。

S4.2.2.2.1 ordinary generic protected-child fence继续原样保留：ordinary readiness、close expected/top、dismiss、History actions与owner
mutation对authenticated current cross-axis History仍按stale evidence first、applied-only fence second返回existing
`rejected / surface.invalid_transition` exact zero。只有本separate lifecycle authority可越过该applied-only fence；claimed readiness及
root replacement retain/failure、ready cutover、empty、publisher/bridge disposal和Coordinator terminal structural cascade继续保持既有
applied语义。本slice不修改Narrative consumer，因此ready History的`ui.cancel`与`player.toggle_history`仍stable-consumed/no-op，
preparing History仍无ordinary input；不能形成“已close但尚无opener restore”的中间delivery。

`.3.1` mutation-sensitive RED至少覆盖：

1. authority/guard/claim/result的exact keys、frozen rows、descriptor/accessor/extra-key/borrowed receiver、same-claimant reuse、
   foreign claimant/kernel及claim error，并对全部top-level/member spelling保留UI root与`./internal` type/runtime negative inventory；
2. preparing与ready各自explicit close，及两phase × Back/Escape/backdrop/routed-cancel；success精确为
   `+1 topology / +1 input / 0 allocation / 1 notification`，child消失、same parent active；
3. invalid dismiss kind before policy/guard、locked policy exact zero、stale candidate/evidence precedence、guard false/throw/nonboolean与
   prepared install drift exact zero；
4. retained root + History + same-owner root replacement preparation/failure保持replacement与exact parent；close/dismiss不得触发owner
   preparation cancel，root ready cutover/empty/terminal与user close first-winner，loser stale；
5. generic closeExpected/closeTop/dismiss/readiness/actions/owner mutation继续`surface.invalid_transition`，claimed lifecycle成功及claimed
   readiness/structural cascade作为positive controls；
6. synchronous listener即时route、open successor、replace/empty/dispose与subscriber throw containment，不产生split binding、double close或
   outer-callback successor retirement；
7. 10,000轮prepare/ready/close/dismiss/stale/replacement churn只保留one retained lifecycle authority、existing bounded candidate records、
   scalar revision/high-water与current claimant，没有per-close token、tombstone或append-only history；Narrative topology/action/focus保持zero diff。

`.3.1` implementation scope严格只有以下**3 files**：

- `engine/packages/ui/src/managed-surfaces/managed-surface-stable-composite-state.ts`；
- `engine/packages/ui/src/managed-surfaces/managed-surface-stable-composite-state.test.ts`；
- `engine/packages/ui/src/public-api.test.ts` negative guards。

`.3.1` hard stop：若close/dismiss不能只靠existing pure `close_top`、fallback dismiss、ready dismiss、whole-composite reflow与prepared install
表达，若必须新增`ManagedSurfaceOperationV1`、修改reducer/input-router/runtime-kernel/Coordinator/managed-surface public contract或generic
receipt/code，若nonnull resumed-parent token不能在transition外capture，或generic fence/structural cascade必须被删除，立即停止并修订本entry。
不得把Narrative family/session/Host、DOM/focus/opener、GameStage/React/Web、Base/Save/wire、Engine Lab/Story/live writer、timer/controller或
legacy deletion并入该three-file substrate。

`.3.2`在Narrative boundary新增frozen source-relative
`NarrativeStableHistoryChildControllerInternalV1`，exact methods只有
`closeInternalV1(): NarrativeStableHistoryChildLifecycleResultInternalV1`与
`dismissInternalV1(dismissKind: ManagedSurfaceDismissKindV1): NarrativeStableHistoryChildLifecycleResultInternalV1`。Closed
`NarrativeStableHistoryChildLifecycleResultInternalV1`只有五个frozen exact rows：
...
每个History candidate取得one exact controller，preparing → ready与same-host/same-portal reattach保留controller identity，retired child、root
cutover、empty、terminal、real detach及fresh successor child永久fence old controller。Controller只作为History Host render entry的exact
`controller` field存在；它不进入`NarrativeStableHistoryRendererPropsInternalV1`、renderer component、History observation、Story
props或任何barrel/package export，也不暴露candidate、parent、Coordinator、evidence、opener或focus ledger。Family只把`.3.1` generic
`applied / surface.closed`映射`closed`、`applied / surface.dismissed`映射`dismissed`，并把locked/stale/faulted逐行映射；不新增第二topology
writer或新的generic receipt/code。Borrowed controller receiver必须抛
`ui.narrative_stable_history_child_controller_invalid`。

Active History的existing ready action route必须先通过application/topology/instance/input-owner/routing/publication/catalog/gesture gates，且
`attempt`必须exact `null`并保持unread。`player.toggle_history`随后调用exact controller explicit close，`ui.cancel`调用
`dismissInternalV1("routed_cancel")`；两者都保持managed Input consumed、outer receipt为historical
`unchanged / surface.action_routed`、close result只进入`consumerResult`，且不向retained root/lower context fall through。Preparing
History继续不commit ordinary candidate binding，而由one Host-generation-bound **fallback-only managed input registration**先于retained root
dispatcher处理：`player.toggle_history` explicit close，`ui.cancel` routed-cancel dismiss，其他action与viewport_point被blocking fallback handled，
`pointer_cancel`与`focus_loss`保持ignored/reset。Ready/fail/close/release必须在同一generation handoff撤销fallback registration；StrictMode
不得double register或留下ready-then-effect窗口。Dialogue root四类dismiss继续locked；root `ui.cancel`与background input必须被Narrative
managed isolation消费且topology exact zero。

`.3.2`同时独占Dialogue root与History的**actual DOM focus/trap/restore**。Initial Dialogue blocking fallback在第一次root focus前Host-local
capture exact previous owner；它必须是connected、same-ownerDocument、位于current Host scope外且不是`body`的`HTMLElement`。Fallback取得
Narrative stage isolation、focus scope与closed Tab trap；ready root聚焦`.2.2.2.2`已注册的exact
shell target。Root replacement preparation/failure继承same previous-owner ledger且不抢retained subtree focus；replacement cutover聚焦successor，
...
connected exact opener；opener断开才退回same parent registered initial target。Root + History同commit退休、root replacement cutover、
successor child已存在、application/Host terminal或external focus已取得ownership时不得intermediate restore。Ledger只保留current root owner与
current History opener，retirement/terminal同步scrub，不进入caller-retained snapshot/controller。

Physical dismiss只消费existing contracts：Back在本dormant slice只通过controller/internal route覆盖，不新增Web `CloseWatcher`或browser
navigation ingress；Escape由active/fallback Host key route，backdrop只接受primary matching pointer gesture。Host只复用existing
`useStageInputIsolationV1("narrative", blocking)`与`useStagePointerGestureFenceV1("narrative")`；backdrop必须先arm persistent stage pointer
fence再同步dismiss，Escape/Back/routed cancel不得arm。不得修改GameStage或从GameStage读取portal/composer/live Story authority。

`.3.2` mutation-sensitive RED至少覆盖：

1. controller/result/render-entry `controller` exact keys、frozen/identity/member negatives；renderer props、History observation、UI root、
   `./internal`与package exports均看不到controller/opener/parent/Coordinator；
2. preparing toggle/cancel/Back/Escape/backdrop close exact candidate且保留root replacement；fallback managed registration抢先、blocking input
   no-fallthrough、ready/fail/close/release atomic unregister与StrictMode exact counts；
3. active toggle explicit `closed`、active/routed/physical dismiss `dismissed`、locked/stale/faulted mapping及double/late/foreign/grace callback；
   action被consumed且success精确one topology/input commit/notification；
4. initial root fallback/ready shell focus、Dialogue四类locked、Tab正反向closed loop、无tabbable时shell fallback、focusin escape recapture、
   suspended/preparing successor不抢focus；
5. History Host-local opener在首次History focus前capture，preparing fallback→ready target，close/fail四dismiss restore connected opener，
   disconnected opener→same parent target，same-commit parent retirement/successor child/terminal/external owner suppress intermediate restore；
6. backdrop primary matching、pointer fence before close、residual click不穿透，Escape/Back/routed cancel zero fence；keyboard/gamepad/pointer使用同一
   candidate/controller/currentness gate；
7. same-host/same-portal reattach、real detach、root/History race、listener reentry、focus throw与subscriber throw containment；DOM focus fault不得
   rollback已提交topology或关闭successor；
8. 10,000轮open/focus/dismiss/reattach只保留one Host runtime、bounded current controller/fallback registration/root-owner/History-opener slots与
   existing render/action/focus records，无DOM ref、generation、controller或tombstone history。

`.3.2` implementation scope严格只有以下**7 files**：

- `engine/packages/ui/src/narrative/narrative-managed-surface-family.ts`及同名`.test.ts`；
- `engine/packages/ui/src/narrative/narrative-managed-surface-session.ts`及同名`.test.ts`；
- `engine/packages/ui/src/narrative/narrative-surface-host.tsx`及`narrative-surface-host.test.tsx`；
- `engine/packages/ui/src/public-api.test.ts` negative guards。

`.3.2` hard stop：若controller必须进入renderer/Story props或暴露raw candidate/parent/Coordinator/evidence，若close与opener restore不能作为
one candidate-bound Host lifecycle交付，若需要修改`.3.1` generic authority、input-router source、reducer、GameStage、CSS、composer/Web/Base/
Save/wire、Engine Lab/template/examples/live Story或legacy writer，立即停止并修订。不得用React local `showHistory`、第二input/focus writer、
async check-then-close/rollback、owner-preparation-cancel、ready后passive-effect binding、caller-retained DOM ledger或unbounded generation map绕过。
Reveal/Auto/Skip timer与History suspension remaining留S4.2.4.2，Host player-view integration留S4.2.4.3，Engine Lab留S4.2.5.1c，
Narrative portal/composer/tracked Story迁移、legacy
writer/export删除与browser/prebuilt promotion留S4.3。

本`.3.0` amendment在三份owning design/planning docs中同步exact contract；本cross-plan change本身严格只修改本文件，没有source、test、
runtime、architecture、Host/React/GameStage/live claimant、public API或delivery evidence diff。验证只要求本文件
`deno fmt --check`与scoped `git diff --check`；既有focused/UI/full/check/browser/examples/prebuilt只作prior evidence，不能冒充本docs-only
checkpoint的新runtime验证。S4.2.3 broad checkpoint与本`.3.0`据此转为completed historical entry；该entry交付时唯一current/next、core
slice与implementation gate为 **S4.2.3.1 DOM-free generic exact History-child lifecycle substrate**，现由下述delivery关闭。

**PF4/S4.2.3.1 DOM-free generic exact History-child lifecycle substrate delivery（completed）：** implementation与primary RED严格只有以下
三文件，全部保持source-relative与package-internal：

- `engine/packages/ui/src/managed-surfaces/managed-surface-stable-composite-state.ts`；
- `engine/packages/ui/src/managed-surfaces/managed-surface-stable-composite-state.test.ts`；
- `engine/packages/ui/src/public-api.test.ts`。

没有reducer、input-router、runtime-kernel、Coordinator、generic/public contract、barrel/package export、Narrative/React/DOM/Host、GameStage/Web、
Base/Save/wire、Engine Lab/Story/live writer、timer/controller或legacy deletion diff。

Stable composite交付独立same-claimant exact-child lifecycle authority、nonnull prepared-input-contract commit guard与canonical
`applied(surface.closed | surface.dismissed) | locked(surface.dismiss_locked) | stale | faulted`结果。Preparing与ready explicit close都只使用
existing pure `close_top`，preparing dismiss使用exact fallback route，ready dismiss使用existing routed dismiss；invalid kind、locked policy、
stale candidate以及guard false/throw/nonboolean按冻结precedence exact zero，successful assignment前先提交restored-parent binding，listener首见
child absent时same parent已active。Same-owner root replacement、allocation high-water、claimed readiness与structural cascade保持，ordinary
generic protected-child fence仍返回existing `surface.invalid_transition`，没有owner-preparation cancel或新的generic operation/receipt/code。

Production strong state只新增one per-kernel retained lifecycle authority/current exact claimant与canonical result identities；candidate继续使用
existing bounded WeakMap provenance，10,000轮prepare/ready/close/dismiss churn后只推进scalar identity high-water，不保留per-close token、
second claimant、tombstone或append-only history。UI root与`./internal`对四个new top-level及两个unique lifecycle member spelling的type/runtime
negative inventory均保持green，没有public/package expansion。

验证通过focused **2 files / 78 tests**、UI package **81 files / 1281 tests**、full **255 files / 4211 tests**，以及完整
`deno task check`、`deno task typecheck`、`deno task lint`、相关文件`deno fmt --check`与`git diff --check`。本source-relative delivery
未重跑browser/examples/prebuilt；先前Engine browser `101 / 101`、examples browser `45 passed / 2 skipped`与prebuilt Player
`38 / 38`只作prior evidence，不冒充本delivery的HEAD验证。S4.2.3.1据此转为completed historical checkpoint；其当时唯一
live/current/next、core slice与implementation gate曾推进到
**S4.2.3.2 dormant Narrative close/input/root + History focus Host lifecycle**，现由下述delivery关闭。

**2026-08-11 PF4/S4.2.3.2 dormant Narrative close/input/root + History focus Host lifecycle delivery（completed）：** implementation与primary RED
严格只改family source/test、session source/test、Host source/test与`public-api.test.ts`七文件；全部保持source-relative、package-internal与
dormant。UI root、`./internal`、package exports、generic Coordinator/runtime-kernel/Surface contract、input-router/reducer、Base/Web/CSS、
GameStage/composer、Engine Lab/template/examples、legacy writer及live Story graph均没有扩张。

每个History candidate现拥有one exact frozen controller；preparing到ready及same-host/same-portal reattach保持identity，retirement、root
cutover、fresh child、empty、terminal或detach后永久stale。Explicit close与四类dismiss复用`.3.1` single lifecycle authority，并在generic
assignment前以prepared nonnull contract原子commit fresh surviving-root action/focus binding、fence child与fallback；locked/stale/faulted路径
abort/reconcile而不留下input gap。Preparing History及initial Dialogue各只使用one Host-generation fallback managed registration；toggle close、
cancel `routed_cancel` dismiss与blocking background input保持exact precedence，ready/fail/close/release都先logical inert再physical cleanup。

React Host把每个entry细分为one focusable outer focus scope与one inner renderer shell：preparing owner的outer scope可真实focus/trap，inner renderer
仍hidden/inert/aria-hidden/pointer-disabled，ready后原位启用；suspended/nonowner继续fenced。Host-local O(1) ledger完成root previous-owner与History
opener capture、closed Tab trap及coalesced restore，并通过source-relative纯查询
`isNarrativeStableHostRuntimeCurrentInternalV1`在排队前与microtask执行前双重拒绝released、superseded、detached或terminal runtime；mounted
bridge/Coordinator terminal empty不会产生stale restore。Escape由Host处理并排除DevDock owner；History backdrop只接受primary exact-target
matching pointer，且在同步dismiss前先arm existing Narrative Stage pointer fence。Stage isolation、focus throw containment与external owner/successor
抑制均不形成第二topology、input或focus authority。

Controller/runtime/DOM ledger与fallback registration的production strong state保持O(1)：one current Host runtime/attachment、one current root owner、
one current History opener/controller、bounded action/readiness/focus records、max-three render entries与scalar high-water；terminal/release/cutover同步
scrub，不保留caller DOM、generation或tombstone history。UI root与`./internal`的type/runtime negative inventory覆盖全部new top-level与member
spelling，controller/query/DOM ledger均未进入public boundary。

验证通过focused **4 files / 263 tests**、UI package **81 files / 1306 tests**、full **255 files / 4236 tests**与完整canonical
`deno task check`，并单独通过`deno task typecheck`、`deno task lint`、相关文件`deno fmt --check`及`git diff --check`。本dormant
delivery未重跑browser/examples/prebuilt；先前Engine browser `101 / 101`、examples browser `45 passed / 2 skipped`与prebuilt Player
`38 / 38`只作prior evidence，不冒充本delivery的HEAD验证。S4.2.3.2据此转为completed historical checkpoint；原S4.2.4 broad
...
**PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**；唯一有效顺序为 **S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。
...
2. **S4.2.4.1 generic prepared state-install participant substrate（completed）**：先覆盖runtime kernel全部state assignment路径，提供
   composition-local、same-claimant、pre-assignment two-phase participant；
3. **S4.2.4.2 DOM-free Narrative DialoguePlayerController core（completed）**：再交付captured clock/profile/text ports、reveal/Pause/Auto/Skip scheduling、
...
**S4.2.4.1 generic prepared state-install participant substrate delivery（completed）：** implementation与mutation-sensitive RED精确落在
`engine/packages/ui/src/managed-surfaces/managed-surface-runtime-kernel.ts`、同目录`managed-surface-runtime-kernel.test.ts`、
`engine/packages/ui/src/managed-surfaces/managed-surface-stable-composite-state.ts`、同目录
`managed-surface-stable-composite-state.test.ts`及`engine/packages/ui/src/public-api.test.ts`共五文件。
Generic runtime kernel交付frozen exact participant/prepared shapes与唯一weak claim seam；stable-composite specialization只解析既有private
composite→runtime alias并委托generic helper，没有建立第二registry或扩张kernel/input/adapter shape。

Ordinary transient transition、`transitionStateInternalV1`与prepared-state commit三个assignment path现统一在lock内pure plan、lock外prepare、
reacquire后先核expected state/install generation/current participant再validate、operation guard、logical commit与唯一assignment。Opaque install
generation封闭pre-callback及post-prepare ABA；gate exact false/throw保持legacy result与original-error precedence；terminal assignment先永久fence claim，
完整listener vector返回后才best-effort completion，reentry successor使old completion稳定stale，authenticated failure只abort exact receiver once。
Participant ownership由per-kernel O(1) owner token绑定；terminal weak tombstone不反向保活kernel/participant，却永久拒绝跨kernel release/transfer；
production不保留state/controller/timestamp或append-only transaction history。

UI root、`./internal`与package inventory的type/runtime public negatives保持green；没有public/package barrel、generic operation/result/receipt、Base/Web、
Narrative controller/React Host、Engine Lab/Story或live wiring扩张。验证通过focused **3 files / 129 tests**、UI package
**82 files / 1357 tests**、full **256 files / 4287 tests**，以及完整`deno task check`、`deno task typecheck`、`deno task lint`、target
`deno fmt --check`与`git diff --check`。本delivery未重跑browser/examples/prebuilt；先前Engine browser **101 / 101**、examples browser
**45 passed / 2 skipped**与prebuilt Player **38 / 38**只作prior evidence，不冒充本HEAD验证。`.4.1`据此转为completed historical
checkpoint；其交付时的live/current/next、core slice与implementation gate曾推进到
**S4.2.4.2 DOM-free Narrative DialoguePlayerController core**，
该gate现已由下述delivery关闭；随后曾推进到 **S4.2.4.3 dormant Host player-view integration**，该gate现也由下述delivery关闭。
...
**S4.2.4.2 DOM-free Narrative DialoguePlayerController core delivery（completed）：** commit `21f700f`的implementation与
mutation-sensitive tests精确落在上述七文件：new `dialogue-player-controller.ts` pair、existing Narrative family pair、session与Host两份
test-only fixture/oracle以及`public-api.test.ts`。Session/Host production source、UI root与`./internal` barrels、package exports、generic
Managed Surface/Base/Save、React/DOM renderer、Engine Lab/Story及live graph均保持zero diff。

Candidate preflight现把exact plain own-data raw clock/profile/text ports descriptor-capture为private-brand、frozen zero-own-key handles；raw receiver、
callback、cancel与store从不进入candidate snapshot或renderer。Source-relative controller module交付上述exact policy/snapshot/input/controller、three
attempt与closed result shapes；factory以exact bridge/target/frame provenance及currentness claim封闭foreign/retired/value-equal/retry/reentry，profile
initial capture与post-subscribe handshake关闭lost update，而frame-local timing policy、speaker/text与UTF-16 reveal length保持首次capture不漂移。

Controller core现统一承载UTF-16 cursor与sub-character remainder、instant/manual/content-auto/player-auto/Skip/Pause、seen logical-once、semantic
first-win及Promise settlement fencing。Family-owned `.4.1` two-phase participant在assignment前capture并validate exact generation/frame/mode，logical
commit保存History/higher-blocker remaining并撤销old attempt，完整notification后才resume fresh generation/next-tick；ready-active late factory、instant
baseline、delayed Auto/Skip toggle、synchronous/duplicate/stale clock callback与source replacement均在raw side effect前后重验，current fault只retire exact
controller，stale fault对successor为zero。Production strong state保持per target one controller、one raw profile subscription、one tick/cancel、one
generation与one current automatic attempt，listener/holder集合有界；WeakMap provenance与10,000 controller/tick churn row共同锁住O(1) owner retention。

UI root与`./internal` type/runtime negative inventory覆盖全部17个new top-level names及其exact member spellings，没有形成public/package export。
Session与Host两份test只把旧opaque fixtures迁移为raw ports并核captured handles，未取得controller/timer authority。验证通过focused
**5 files / 334 tests**、UI package **83 files / 1428 tests**、full **257 files / 4358 tests**与完整canonical `deno task check`；本delivery未重跑
browser/examples/prebuilt，先前Engine browser **101 / 101**、examples browser **45 passed / 2 skipped**与prebuilt Player **38 / 38**只作prior
evidence，不冒充commit `21f700f`的HEAD验证。`.4.2`据此转为completed historical checkpoint；该delivery当时把live/current/next、core slice与
implementation gate推进到 **S4.2.4.3 dormant Host player-view integration**，该gate现由下述delivery关闭。
...
重建generation，或需要Engine Lab/GameStage/composer/DialoguePanel/VnLayer/Web/CSS/Base/Save/live wiring，`.4.3`立即停止；Engine Lab只归`.5.1`，
tracked consumer migration、legacy writer/export删除与browser/examples/prebuilt promotion只归S4.3。

**S4.2.4.3 dormant Host player-view integration delivery（completed）：** commit `1438b32`的implementation与
mutation-sensitive RED精确落在Narrative family pair、session pair、Host pair与`engine/packages/ui/src/public-api.test.ts`七文件；UI root、
`./internal`与package barrels/exports、Engine Lab/Story、GameStage/composer、DialoguePanel/VnLayer、Web/CSS/Base/Save及live graph均保持zero diff。

Family现为每个exact same-frame controller建立one safe cached observation与descriptor-captured bound text resolver；preparing→active→suspended→resume及
same-host/same-portal StrictMode reattach保持observation/controller/renderKey exact identity，fresh frame/replacement/failure/empty/terminal则永久fence old
proxy。Snapshot equal复用identity并zero notify；profile callback只在exact current generation上先刷新cached immutable player view，再通知bounded
listeners。Controller factory任一raw profile/reduced-motion/text/subscribe seam fault均发布frame-bound fault proxy：pre-ready observation/render fault沿
existing readiness-failed terminal-once，accepted-ready late fault由outer diagnostics owner接管；retired proxy只保留safe final passive snapshot与late
frozen no-op subscription，resolver拒绝raw re-entry。

Session Dialogue entry新增exact `playerObservation`，Dialogue renderer props新增exact `playerView`并把Dialogue/History profile与resolver收窄为current
immutable profile与safe callable。History profile变化只mint same-renderKey fresh entry/props，保持controller/history observation/DOM identity；Host的
module-level keyed Dialogue child以`useSyncExternalStore` materialize exact frozen `playerView`并覆盖current `playerProfile`，不取得controller、raw
profile store/clock、deadline/mode/semantic或Surface authority。History focus effect只依赖kind/renderKey/parentRenderKey等stable focus lifecycle identity，
因此current-profile churn更新renderer而不抢走descendant focus，也不重建readiness/controller/tick。

Terminal与replacement participant先logical fence并scrub controller/raw bindings，再在完整kernel listener vector之后deferred publish final
observation；delivery前unsubscribe可取消自身pending callback，standalone controller fault则同步且exact-once发布final view。Production strong state保持
per target one controller/materialization/raw subscription/tick与bounded listener holders，10,000 Host/controller/snapshot churn不累积retired proxy或raw
owner。UI root与`./internal` type/runtime negative inventory覆盖两个new source-relative top-level contract及`playerObservation | playerView` exact member
spellings，没有形成public/package export。

验证通过focused **4 files / 296 tests**、UI package **83 files / 1442 tests**、full **257 files / 4372 tests**与完整canonical
`deno task check`；format、lint、styles、typecheck、determinism、assets、Story checks与Engine Lab production build均为green。本delivery未重跑
browser/examples/prebuilt；先前Engine browser **101 / 101**、examples browser **45 passed / 2 skipped**与prebuilt Player **38 / 38**只作prior
...
13. **S4b.1a whole-canvas family + routed-input substrate（已完成；historical）**：交付family + routed-input substrate；
14. **S4b.1b dormant production composition/Host/GameStage substrate（已完成；historical）**：保持独立merge boundary；
15. **S4b.1c tracked-consumer atomic cutover and promotion（已完成；historical）**：完成后才允许PF5。

#### S4.2.5.1a managed InputRouter facade corrective（已完成）
...
**PF4/S4.2.5.1a managed InputRouter facade corrective（已完成，2026-08-11，commit `0f41e41`）：** 本delivery严格只修改上述
exact five files。`input-router.ts`由direct `createInputRouterV1()` identity私有保存唯一managed registrar，并以source-relative
`bindManagedInputRouterFacadeInternalV1()`把composition public facade绑定到该exact raw managed vector；facade、target与gate都在任何registrar、handler或
callback读取前完成frozen plain exact-own-data capture。Gate的non-thenable认证同时执行contained actual `Get("then")`与cycle-safe own/prototype descriptor
walk，inherited thenable、Proxy-synthesized then及throwing/revoked/trapping Proxy都统一fail closed为exact invalid TypeError，不能泄漏raw trap。

Same active tuple保留同一个cleanup identity；native router作为facade、facade chain/foreign target、active conflict与terminal rebind都被拒绝。Closed gate返回
shared frozen noop且registration unread；gate throw/nonboolean统一invalid；gate或raw registration getter reentry会在current-token复核失败后回滚刚安装的physical
handler。Composition在public facade freeze之后、publication之前exact-once bind，hosted terminal与ordinary dispose均先关闭ingress并release link，再继续既有
physical cleanup；managed handler继续先于ordinary vector，public facade identity及three-method shape不变。Cleanup按logical fence → scrub target/registrar/gate
strong refs → delete facade registrar → shared weak terminal sentinel排序，late register恢复existing `ui.managed_input_router_required`，old unregister/cleanup不能
命中successor。10,000行使用fresh target/facade generation完整执行bind/repeat/register/route/unregister/cleanup，module-owned state保持bounded O(1)，没有第二router、
registrar history或strong tombstone。

UI root与`./internal`只增加该helper的compile/type/runtime negative inventory；package/input barrels、raw target/registrar/public member、Narrative family/session/Host、
conformance与live writer在commit中均保持zero expansion。验证通过focused **3 files / 80 tests**、UI package **83 files / 1458 tests**、full
**257 files / 4388 tests**与完整canonical `deno task check`；format、lint、styles、typecheck、determinism、assets、全部Story checks及Engine Lab production
...
**PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**，唯一后续顺序为
**S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。
...
**PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**，后续唯一顺序为 **S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。
...
7. **S4b.1a whole-canvas family + routed-input substrate（已完成；historical）**：现在进入DOM-free family与routed-input substrate；
8. **S4b.1b dormant production composition/Host/GameStage substrate（已完成；historical）**：保持独立merge boundary；
9. **S4b.1c tracked-consumer atomic cutover and promotion（已完成；historical）**：完成后才允许PF5。
...
**PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**；冻结线性顺序为
**S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。
...
RED gate为**PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。
...
不因closeout改写。唯一live current/next、core slice与direct RED/implementation gate现为**PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**；冻结线性顺序为
**S4.3 broad checkpoint（historical） → S4.3.0（docs-only，已完成） → S4.3.1a（已完成；historical） → S4.3.1b（已完成；historical） → S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。
...
6. **PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**：`.1c`已经完成并closeout；S4b不提前修改Save migration语义。
...
`.1c`现已完成并转为historical checkpoint。唯一live current/next、core slice与direct RED/implementation gate现为**PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**；冻结线性顺序为**S4b broad checkpoint（historical） → S4b.0（docs-only，已完成） → S4b.1a（已完成；historical） → S4b.1b（已完成；historical） → S4b.1c（已完成；historical） → PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。
...
completed/historical；唯一live current/next、core slice与direct RED/implementation gate为**PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。
...
**PF5/M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。
...
S4b.1c tracked-consumer atomic cutover/promotion与closeout均已完成，PF5 已激活并完成。原 M3
broad checkpoint 由 **M3.0 exact entry（docs-only，已完成）**重切；M3.1 single-slot
read-only inspection、M3.2 adoption declaration set、M3.3 same-namespace bounded backup
repository substrate、M3.4 semantic operations 与 M3.5 player-readable recovery UI 现均已完成
并转为 historical；M3.6.0 docs-only exact inventory corrective、M3.6a maintained corpus /
Story lifecycle、M3.6b four-runtime parity 与 M3.6c Browser repeated-download evidence 也已
关闭。M3.6d exact-twelve live docs/PF5 promotion 也已完成并转为 historical；当前唯一 live
current/next、core slice与direct gate是 PF6/S5 docs-only exact-entry/adjudication，尚未授权
broad S5 implementation。
完整 contract、exact allowlist、mutation-sensitive RED、gate与 stop 由
[Save migration plan](2026-07-30-save-migration.md) 第 6 节拥有；本文只拥有跨计划顺序。

PF5 内部冻结线性顺序为：

1. **M3.0（docs-only，已完成）**：冻结 single-slot inspection taxonomy、adoption set边界、
   same-namespace bounded backup/restore、lineage 16/re-anchor、支持 floor与后续 slices；只改
   Save design与两份 owning plans，不交付 runtime/public capability；
2. **M3.1（已完成；historical）**：只实现 single-slot read-only `inspectSave`；exact kinds 为
   `direct | migration_required | adoption_required | migration_and_adoption_required |
   inspect_only | rejected | faulted`。不增加 `inspectSaves()`，不交付 adoption set/backup/UI；
3. **M3.2（已完成；historical）**：把现有单 adoption declaration 提升为上限 256 的 exact-match
   immutable set，并对第 257 项、duplicate/ambiguous match 在 Host I/O 前 fail closed；
4. **M3.3（已完成；historical）**：只建 `save` namespace 内每 Story/slot 一条的 bounded backup
   repository/CAS substrate；existing backup 返回 `backup_pending`，不得 overwrite或被普通
   Save/Auto/clear 删除；
5. **M3.4（已完成；historical）**：交付 single-slot read-only backup status 与
   upgrade/re-anchor/restore/export-backup/discard-backup语义 operation；backup status不返回
   bytes/key/revision、不取得 lease且零写入/Session mutation；只有
   restore/discard消费 backup，export始终保留；
6. **M3.5（已完成；historical）**：交付玩家可读、可访问且不泄漏内部 stack/raw authority 的
   Save recovery UI；
7. **M3.6.0（docs-only，已完成）**：修正原 13-path promotion inventory，并把后续 execution
   拆为四个独立切片；不交付 runtime/test/fixture/live product capability；
8. **M3.6a（已完成；historical）**：冻结并验收 maintained corpus 与两个 Story 的完整 Save
   lifecycle；
9. **M3.6b（已完成；historical）**：把 maintained corpus 接入 Deno/Chromium/Firefox/WebKit parity；
10. **M3.6c（已完成；historical）**：关闭 Browser 同名连续 download 的 distinct-path exact-bytes evidence；
11. **M3.6d（已完成；historical）**：只更新 live product docs并关闭 PF5。Cat Cafe revision 1
    是首个支持 floor，不虚构旧 fixture；Engine Lab
    维护revision 3/4到current 5的compatibility fixtures与 `3 -> 4 -> 5` chain，不伪称历史release
    capture。

**2026-08-12 M3.5 docs-only scope corrective（已完成；historical）：** owning Save
plan 的 exact allowlist 从十五项修正为二十七项：补入既有 managed System exact-parent
confirmation contract/session/Host 与 tests、`system-dialog-host` copy mapping source/test，以及
Engine Lab/Cat Cafe 既有 browser specs 和两份 Playwright config。confirmation closed union 只
追加 slot-bound re-anchor/restore/discard；它们必须复用同一 managed child，既有
load/clear/import 不变，upgrade 与 export-backup 不确认。新增 Firefox project 只以
`grep: /@save/` 跑 Save-tagged specs，Chromium/WebKit 普通矩阵与 Chromium prebuilt smoke 不
扩大。本 corrective 只恢复 M3.5 已冻结 gate 的可实现边界，未交付 source/test/config；其
implementation gate 已由下述 delivery 关闭。

**2026-08-12 M3.1 single-slot read-only inspection delivery（已完成；historical）：**
实现严格命中 owning plan 冻结的八路径 allowlist。真实 mutation RED 在实现前产生 `7` 个
失败，随后同一 suite 转 GREEN。Final gates 为 focused `17 / 17`、current-load baseline
`38 / 38`、Base `81 files / 1156 tests`、两个 type-test files、determinism Deno `3 / 3` 与
Chromium/Firefox/WebKit `6 / 6`；scoped oxlint/fmt/diff/typecheck 全绿。Canonical
`deno task check` 覆盖 format `955 files`、full lint/style/typecheck/determinism、
`266 files / 4533 tests`、assets、five registered Story checks 与 Engine Lab build
`415 modules`，全部 green。该 delivery 只交付 single-slot inspection；其当时推进到 M3.2 的
historical pointer 已由下述 M3.2 delivery 关闭。

**2026-08-12 M3.2 adoption declaration set delivery（已完成；historical）：** 真实 RED 证明
旧 classifier 仍读取 singular `adoptionDeclaration` property，focused suite 因而失败。实现把
application/Core、Persistence service/classifier 与 Engine Lab 统一到 maximum-256 immutable
exact seven-field set，并冻结 descriptor-safe pre-I/O admission、`0 / 1 / >1` match、stable
ambiguous rejection、permutation independence与 10,000-attempt boundedness；未交付 M3.3 backup
substrate或后续 operation/UI/corpus。

Final gates 全绿：focused `6 files / 274 tests`、Base `81 files / 1163 tests`、两个 type-test
files、typecheck与scoped fmt/oxlint/diff、determinism Deno `3 / 3` 与
...
与 Engine Lab build `415 modules`。M3.2 据此转为 completed/historical；唯一 live/current/next、
core slice与 direct RED/implementation gate 当时推进到 M3.3；该 historical pointer 已由下述
M3.3 delivery 关闭。

**2026-08-12 M3.3 same-namespace bounded backup repository substrate delivery（已完成；
historical）：** package-internal exact backup key、bounded shell/identity read、one-generation
raw-byte backup 与 backup/target/lease 单批 CAS primitive 已交付；普通 Save、Auto、clear、
annotation、list/inspection/load/import/export 均保留 pending backup。实现未增加 Host
namespace、public transaction、第二份 backup、player operation 或 Desktop durability 声明。
...
Final gates 全绿：focused repository/property/Persistence/IndexedDB `4 files / 121 tests`、Base
`81 files / 1177 tests`、typecheck 与 scoped fmt/oxlint/diff。Canonical `deno task check` 覆盖
format `955 files`、`266 files / 4557 tests`、assets、five registered Story checks 与 Engine Lab
production build `415 modules`。M3.3 据此转为 completed/historical；唯一 live current/next、
core slice与 direct RED/implementation gate当时推进到 M3.4；该 historical pointer 已由下述
M3.4 delivery 关闭。

**2026-08-12 M3.4 upgrade, re-anchor and backup-resolution operations delivery（已完成；
historical）：** single-slot `inspectBackup` 与 `upgradeSave`、`reanchorSave`、`restoreBackup`、
`exportBackup`、`discardBackup` 已按 owning Save plan 的 fresh-read/no-token、lease-fenced 与
same-namespace bounded backup 合同交付。upgrade/re-anchor 的 backup/target/lease commit 全有或
全无；restore/discard 是唯一消费 backup 的 operation，export 始终保留 exact raw bytes；成功或
失败都不安装 Session、不替换 replay anchor、不改变 Persistence status。恰好 16 条 lineage 的
exact Save 仍可加载，只有 otherwise-valid unique adoption 能 re-anchor，`>16` 与 zero/multi
adoption 均 fail closed。

Final gates 全绿：M3.4 specific `4 files / 298 tests`、focused integration
`6 files / 356 tests`、Base `81 files / 1221 tests`、IndexedDB `13 / 13`、determinism Deno
`3 / 3` 与 Chromium/Firefox/WebKit `6 / 6`；canonical `deno task check` 覆盖 format
`955 files`、`266 files / 4604 tests`、lint/style/typecheck/determinism/assets、five registered
Story checks 与 Engine Lab production build `415 modules`。首次 canonical 的唯一失败是保留的
10,000-attempt stress row 在并行负载下约 `47s` 撞到默认 `30s` test budget；保持 attempt count
与产品断言不变、把显式 budget 调整为 `120s` 后，focused 用时 `15.11s` 且 canonical 全绿。
这是 test-budget corrective，不是产品 failure。M3.4 据此转为 completed/historical；其当时
推进到 M3.5 的 historical pointer 已由下述 delivery 关闭。

**2026-08-12 M3.5 player-readable Save recovery UI delivery（已完成；historical）：** UI/Web
只经 optional atomic recovery group 消费 single-slot inspection、backup status 与 semantic
operation，并把 disposition、backup status、operation outcome 与 stable rejection code 映射为
本地化、可访问的 player copy；没有向 UI 暴露 stack、digest、raw bytes、Host key/revision/fence
或 exported file authority。re-anchor、restore、discard 复用同一个 managed System exact-parent
confirmation child；upgrade 与 export-backup 不确认。explicit inspection 保持 slot-local，pending
read/write mutually exclusive，stale/mismatched result fail closed。
...
**2026-08-12 M3.6.0 exact promotion inventory corrective（docs-only，已完成）：** M3.6 的
product contract不变；owning Save plan 只把无法承载既有 four-runtime driver/matrix与 Browser
no-clobber evidence 的原 13-path inventory 修正为 23 路径，并按 M3.6a corpus/Story lifecycle、
M3.6b four-runtime parity、M3.6c Browser repeated-download evidence、M3.6d live docs/PF5
promotion 四批冻结 exact scope。M3.6a formatter corrective 随后加入 `deno.json`，使 aggregate
成为 24 路径；M3.6b implementation pre-audit 再证明 immutable fixture file bytes 会让 driver
corpus path 变为 async，原同步 direct test 必须显式 `await`，worker protocol test 也必须成为
exact owner（允许 zero-diff），因此只把既有
`e2e/src/test/save-state-migration-vector.test.ts` 与
`e2e/src/test/save-state-migration-worker.test.ts` 加入 M3.6b，把该 slice 从 7 修正为 9、aggregate
从 24 修正为 26。该 pre-audit 当时认为既有 Base facade 足够；随后 canonical authority-map
gate 反证 broad `@sillymaker/base/testkit` 会把 `runtime/application` 与
`persistence-service` 拉入 migration Worker closure。第二个 M3.6b docs-only scope corrective
因此只把既有 `engine/packages/base/src/testkit/save-state-migration-determinism.ts` 加入 exact
scope，并限定它重导出 release-corpus admission、inventory 与 descriptor types，使 M3.6b 从 9
修正为 10、aggregate 从 26 修正为 27。这些 corrective 均未修改 source/test/runtime/fixture/
live product docs，也不声明任何 M3.6 runtime delivery；这些 corrective 完成后的后续 gate 当时为
M3.6b，该 historical pointer 现已由下述 delivery 关闭。
...
live/current 的有效指针；原 M3.6d 五份文档不能合法更新这两个 owner，直接 closeout 会留下
错误的当前执行入口。owning Save docs 因此只把这两份既有文档加入 M3.6d/aggregate exact
allowlist，使 M3.6d 从 5 修正为 7、aggregate 从 27 修正为 29。本 corrective 不修改新增的
两份文档，不交付 runtime/test/fixture/live product capability，也不声明 M3.6c 或 M3.6d
...
candidate 落盘后的 active-pointer scan 又证明
...
corrective 只覆盖 roadmap 与 focused Surface plan，仍不足以关闭 PF5。owning Save docs 因此只
把这两份既有 design owner 加入 M3.6d/aggregate exact allowlist，使 M3.6d 从 7 修正为 9、
aggregate 从 29 修正为 31。本 corrective 只修改三份 owning Save docs，不修改两份新增文档，
不交付 runtime/test/fixture/live product capability，不声明 M3.6d delivery/promotion，也不推进
production-floor pointer；随后恢复的 exact-nine candidate 已完成两份新增 owner，但又由下述
exhaustive-scan corrective supersede。
...
额外冲突声明。owning Save docs 因此只把这三份既有 live owner 加入 M3.6d/aggregate exact
allowlist，使 M3.6d 从 9 修正为 12、aggregate 从 31 修正为 34。本 corrective 只修改三份
owning Save docs，不修改三份新增文档，不交付 runtime/test/fixture/live product capability，
不声明 M3.6d delivery/promotion，也不推进 production-floor pointer。M3.6d 仍为当前 pending
exact-twelve docs-only closeout。

**2026-08-12 M3.6a maintained corpus and Story lifecycle delivery（已完成；historical）：**
实现严格命中 owning Save plan 冻结的 exact `10` paths（含只精确排除四份 maintained Save
bytes 的 `deno.json`），没有修改 four-runtime driver、Browser spec 或 live product docs。真实
RED 先让三份新增 corpus suites 因 release-corpus helper/barrel 尚不存在而在 module resolution
失败；随后同一 suites 以 checked-in immutable bytes 转 GREEN。两个 Story 从 physical fixture
执行完整 Save lifecycle，并覆盖 failure no-mutation、backup/restore、adoption、lineage `15/16`、
...
下述 delivery 关闭，唯一 current gate 推进到
**M3.6c Browser repeated-download no-clobber evidence（当前；M3.0–M3.6b 已完成）**。

**2026-08-12 M3.6b four-runtime corpus parity delivery（已完成；historical）：** 首个真实 RED
把 expected vector 增加 ordered four-case release corpus，而 actual collector 仍为空；实现以
literal `?no-inline` URL 异步读取 immutable fixtures，Engine Lab revision 3/4 执行真实
`3 -> 4 -> 5` callbacks，revision 5 与 Cat Cafe revision 1 执行 current exact admission，并保持
synthetic M2 vector 独立。Canonical 随后产生 authority-map `1 / 14` RED并报告四条 forbidden
transitive paths；narrow migration testkit facade corrective 移除 broad Base testkit import后，
同一 authority-map 转为 `14 / 14` GREEN。

Frozen exact `10` paths 中实际 `4` paths 有 diff、`6` paths zero-diff。Final gates 为 focused
`3 files / 6 tests`、Deno `1 file / 3 tests`、Chromium/Firefox/WebKit repeat-each-two `6 / 6`
（real `10.41s`），以及 canonical format `959 files`、`269 files / 4701 tests`、assets、five
registered Stories与 Engine Lab build `415 modules`（real `41.83s`），全部 green。M3.6b 据此
转为 completed/historical；唯一 live current/next、core slice与 direct RED/implementation gate
推进到 M3.6c Browser repeated-download no-clobber evidence。

**2026-08-12 M3.6c repeated-download delivery（已完成；historical）：** 新 exact-byte
assertions 消费旧 `void` seed helper，先在 Engine Lab/Cat Cafe 两处产生 TS2769 typecheck RED；
随后两份 `@save` flow 在导航前冻结 metadata clock，从同一 IndexedDB transaction 取得
defensive backup byte oracle，捕获两个同名 download event，保存到 distinct test-owned paths，
逐 byte 验证并在 export 后证明 backup 仍 available，再继续既有 discard/restore。Engine/Cat ×
Chromium/Firefox/WebKit `6 / 6`；prebuilt build `415 modules` + Chromium `44 / 44`（real
`54.95s`）；canonical format `959 files`、`269 files / 4701 tests`、assets、five Stories 与
build `415 modules`（real `42.96s`），全部 green。该 evidence 不声明 Browser suffix authority
或 Desktop process-crash atomicity。

**2026-08-12 M3.6d exact-seven candidate（未交付）：** 第一轮候选同步了 live capability、
supported corpus/release workflow 与七份文档的 current pointers，但 whole-doc scan 随后证明它
遗漏上述两份 active design owner，因此不构成 M3.6d delivery、PF5 promotion 或 pointer
advancement。最终 closeout 必须命中修正后的 exact nine-doc scope，并在九份 docs 的
fmt/diff/stale-pointer audit 与 fresh canonical gate 全绿后，才可把 PF5/M3 转为 historical并推进
到 PF6/S5 docs-only exact-entry/adjudication；随后的 exact-nine candidate 又由上述
exhaustive-scan corrective 证明遗漏三份 active live owner，同样不构成 delivery/promotion。最终
closeout 必须命中 exact twelve-doc scope，并在十二份 docs 的 fresh gate 全绿后才能推进 pointer。

**2026-08-12 M3.6d final exact-twelve delivery（已完成；historical）：** final overlay 严格命中
十二份 live/owning docs，更新 implemented capability、supported floor、release workflow与所有
有效 execution pointer；未修改 runtime/test/fixture/config，也未扩大 Browser suffix 或 Desktop
crash-atomicity 声明。全量 tracked Markdown、website 与 config scan 没有发现 scope 外 live
contradiction 或 navigation/link 更新。Scoped format、diff、stale-pointer 与 exact inventory gate
全绿；fresh final `deno task check` 覆盖 format `959 files`、`269 files / 4701 tests`、assets、
five registered Story checks 与 Engine Lab production build `415 modules`；`/usr/bin/time -p` 为
real `46.59s`、user `224.14s`、sys `47.21s`，全部 green。M3.6d 与 PF5/M3 据此
completed/historical，current/next、core slice与direct
gate推进到 PF6/S5 docs-only exact-entry/adjudication；broad S5 runtime implementation 尚未授权。

已交付的 M3.1 显式 inspection 在需要时同步执行pure State migration callback，却必须保持Host record
commit/delete、lease、Session/replay/Persistence mutation为 `0`；`listSlots()`只保持State
migrator count `0`，不移除既有stored/Story/reference/invariant validation。M3 backup的多记录
原子承诺只覆盖已promotion的Memory/IndexedDB Host store；Desktop preview的process-crash
durability仍由PF-D拥有。Browser同名导出只证明两次download event与Playwright不同临时路径
bytes正确，不宣称JavaScript决定最终filesystem suffix。

fixture 是长期兼容承诺，不是一次性 plan fixture。只有显式声明支持的 revision必须有真实
maintained bytes；若发现必须支持的 Engine Lab same-revision/different-digest revision 5，
PF5立即停止并先接受显式 recovery/migration contract，不得以 `5 -> 5` edge、adoption或倒造
fixture绕过。

### PF6 — Surface contract harness and authoring promotion

所有 live surface family 与 PF5/M3 均已完成。PF6 的第一步不是直接实施本节 broad S5 清单，
而是 **S5 docs-only
exact-entry/adjudication**：先冻结 exact allowlist、mergeable slices、RED 与 gates，随后才建设：
