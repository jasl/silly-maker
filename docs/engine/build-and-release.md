...
Artifact names start from a release-friendly stem: the configured `web.desktop.name` plus the application's `package.json` version and short git commit when they are known (`ImoutoApp-0_1_0-abc1234`; dots become underscores so the stem never reads as a file extension, and either part is silently omitted when unavailable — packaging never requires git or a version field). Without `--target` the output is the host-platform preview (`<Stem>.app`). Each explicit `--target <triple>` adds one cross-compiled package named `<Stem>-<triple>.<ext>` with a per-OS copy-and-run format: macOS `.app`, Windows `.msi`, Linux `.AppImage` (the accepted triples are `deno desktop`'s six `x86_64`/`aarch64` × darwin/windows/linux values). `--compress[=xz|lzma|zstd]` makes the payload self-extracting — useful for asset-heavy applications. The configured `.png`/`.icns` icon is macOS-format and is forwarded only to darwin outputs.
