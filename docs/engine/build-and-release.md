...
- **Cloudflare Workers** — the Cat Cafe example carries an app-local `wrangler.jsonc` (assets-only Worker named `silly-maker-cat-cafe` serving `./dist-web`). From `examples/cat-cafe/`: `deno task deploy:cf` (builds, then deploys; authenticate once with `deno run -A npm:wrangler login`). The Player lands at `https://silly-maker-cat-cafe.<account>.workers.dev/`. Any application can copy the same two pieces (`wrangler.jsonc` + the `deploy:cf` script) with its own Worker name; each application deploys as its own Worker, independent of the composed site.
