...
- **2026-07-31（公开仓库卫生）** — 将 publication-sensitive 商业参考的公开登记收敛为 opaque ID，删除项目名、发行目录、精确规模和孤立的兼容文件名；外部未发布验证工作区继续只提供中性需求证据，不能成为正式源码、测试、构建或 Artifact 依赖。内容数据库、命中区域和 Host 元进度的 maintained evidence 明确归原创 `examples/cat-cafe` 与正式测试所有。
