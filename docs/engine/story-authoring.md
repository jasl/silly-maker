...
`viewId` identifies that view's current admitted props and named-slot contract;
together with `nodeId` it is the React lifecycle identity. A compatible
implementation change keeps the ID and local React state. An incompatible props
or slot contract receives a new `viewId`, so normal React replacement remounts
the node and its local fault boundary. Do not add a parallel contract-revision
axis without a maintained released-source compatibility requirement.
