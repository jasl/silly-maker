...
A layout document may also declare a `widgets` section — declarative chrome controls the generic `ChromeWidgetSurfaceV1` host (`@sillymaker/ui`) renders at their referenced boxes when mounted inside the Story's HUD-slot chrome. Two kinds exist: `intent` (button: `intentId` + `labelTextId` + optional icon `assetId`) and `hold_progress` (read-only meter: `labelTextId`). The Story supplies the whole behavioral side as props: `stateOf(intentId)` projects availability (`enabled` / `disabled` with reason text ids / `hidden`), `onActivate(intentId)` maps the activation onto a semantic command (an ordinary invoke, an occurrence-fenced `hold_write` while a hold runs, a choice resolution), and `holdProgress` passes the committed pending-hold view (`remainingMs`/`totalMs`) or null. Widgets never route — declaring one grants no gameplay authority, and a timed menu over a running hold stays the fenced-write + hold-`when`-arm composition, just with document-declared buttons instead of hand-placed ones.
...
takes the member). The hint is presentation-only — authoritative
arithmetic, `when` arms, digests, and replay never read it — and it does
