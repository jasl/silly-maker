...
export type { TemplateApplicationInstanceV1 } from "./core-definition.ts";
export { templateCoreApplicationDefinitionV1 } from "./core-definition.ts";

const ownerIdV1 = "owner.sillymaker.template" as SessionLeaseOwnerId;
...
export interface CreateTemplateApplicationInstanceOptionsV1 {
 ...
 * `deno task app simulate template` target. Deterministic by default.
 */
export async function createTemplateApplicationInstanceV1(
  options: CreateTemplateApplicationInstanceOptionsV1 = {},
): Promise<TemplateApplicationInstanceV1> {
  const resolved = resolveCoreGameApplicationV1(templateCoreApplicationDefinitionV1);
  if (resolved.kind === "failed") {
    throw new TypeError(`template Story failed to resolve: ${resolved.failure.code}`);
  }
  ...
  return createCoreGameApplicationInstanceV1(resolved.application, {
    host: {
      ...
      ownerId: ownerIdV1,
      nextHandoffRequestId: () => "handoff.sillymaker.template",
    },
  });
}
