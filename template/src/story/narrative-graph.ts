 ...
 */
function graphNodeForV1(node: TemplateNarrativeNodeV1): unknown {
  ...
  switch (node.kind) {
    ...
    default: {
      ...
      throw new TypeError(`unknown template narrative node ${String(exhaustive)}`);
    }
  }
}

export function projectTemplateNarrativeGraphV1(): NarrativeGraph {
  const entry = templateScriptV1[0];
  if (entry === undefined) throw new TypeError("template.narrative_script_empty");
  return parseNarrativeGraph({
    entryNodeId: entry.nodeId,
    nodes: templateScriptV1.map((node) => graphNodeForV1(node)),
  });
}
