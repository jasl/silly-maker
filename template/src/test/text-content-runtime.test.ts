...
describe("Template runtime text-content gate", () => {
  ...
  it("resolves text after the selected runtime pack is admitted", async () => {
    const textContent = createTextContentSessionV1({
      ...
      loadPackBytes: (descriptor) => runtimePackBytesV1(descriptor.runtimePath),
    });
    await textContent.ensure(templateEndingTextPackIdV1);
    expect(textContent.resolveText(null, "text.template.line.cat" as TextId)).toBe(
      "看，檐角下躲着一只小猫，毛都淋湿了。",
    );
  });
});
