...
本目录下的每个包是一个 Story（游戏）。改动前先认清你在哪个包里工作：

| 目录          | 角色                                                                                                | 改动纪律                                                              |
| ------------- | --------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `template/`   | 新游戏起点。开新故事 = 复制本目录 + 全局改名 + 在 `game/project.config.ts` 注册                     | 保持最小可玩；占位剧本可整体替换，结构不轻易加东西                    |
| `examples/*/` | 收编的示例：`bookshop`（剧本写法）、`cat-cafe`（内容数据库/命中区域/回合制/元进度，配 `DESIGN.md`） | 只修不扩；新玩法别加在示例上                                          |
| `e2e/`        | Engine Lab 引擎一致性 Story。引擎行为的验收面                                                       | 只应引擎工作而动；改状态契约必须按版本表 bump revision 并同步测试断言 |
