...
照料循环均已实现；M3 已完成视觉/宽窄屏、`trusting`/梳理、腹部边界与首次叼球归还切片，完整产品仍是 WIP。** 它从当前
...
关系纵深，10/16 个自主行为、脸/颈/背/腹部 4/8 类直接互动、背部梳理、逗猫棒与投球 2/3 种玩具，以及
`accept | tolerate | warn | refuse` 四类结果表现。露腹与触摸邀请保持分离：`supine_relaxed` 本身不授权
腹部触摸，只有后续新的当前 `belly_offer` 才授权一次短暂顺毛互动。bonded 且心情合适时，小猫会叼来作者场景
中的真实小球；玩家可用鼠标或触控抓取、拖动和投掷，并在追逐归还后留下可恢复的语义结果。Save/reopen、
...
rule；它明确 defer，不作为已实现互动。益智喂食器、其余自主行为/反应、完整后期内容与产品打磨仍由
M3–M5 承担。
