...
- **多结局**：第七周周日夜结算（巷口传奇/踏实的日子/平凡的幸福）。

## 运行

```bash
deno task story dev example-cat-cafe          # 浏览器开发（Vite）
deno task story build example-cat-cafe        # 生产构建 → dist/example-cat-cafe
...
软件 PolyForm Noncommercial 1.0.0；文本、设计与美术 CC BY-NC-SA 4.0（见仓库根 `LICENSE.md`）。工程背景：设计规格 [DESIGN.md](DESIGN.md)，引擎缺口分析 [研究笔记](../../docs/research/2026-07-28-imouto-rpgmv-gap-analysis.md)。
