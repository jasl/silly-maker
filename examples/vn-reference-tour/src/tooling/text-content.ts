...
import { vnReferenceTourTextContentManifestV1 } from "../content/text-content.ts";
...
for (const pack of vnReferenceTourTextContentManifestV1.packs) {
  for (const variant of pack.variants) {
    const document = documentByRuntimePathV1.get(variant.runtimePath);
    if (document === undefined) {
      throw new TypeError(
        `vn-reference-tour.authoring_text_variant_missing:${variant.runtimePath}`,
      );
    }
    ...
    for (const [textId, text] of admitted.entries) {
      if (localeEntries.has(textId)) {
        throw new TypeError(`vn-reference-tour.authoring_text_duplicate:${textId}`);
      }
...
const fallbackByLocaleV1 = new Map(
  vnReferenceTourTextContentManifestV1.locales.map((locale) =>
    [
      locale.locale,
...
export function vnReferenceTourAuthoringTextForLocaleV1(
  locale: string | null,
  textId: string,
): string | null {
  ...
  try {
    cursor = locale === null
      ? vnReferenceTourTextContentManifestV1.defaultLocale
