...
| `silly-os/` | 复古桌面 shell：窗口管理器（重叠/焦点/最小化/最大化/拖拽，UI 瞬态）、应用注册表、确定性扫雷（事务 RNG 布雷、雷位不上发布面）、记事本（存档即硬盘）、iframe 浏览器、浏览器语言自动 i18n、全自绘系统外观（自动保存/开机恢复、控制面板窗口、任务栏音量托盘，引擎预设 UI 零暴露） | 代码与文案 MIT；像素图标 CC0 |
