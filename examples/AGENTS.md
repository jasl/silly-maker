...
| `vn-reference-tour/` | WIP VN Reference Product candidate. M0–M3 delivered its independent package, then-complete two-route author/headless data, engine-maintained default VN Player, final media/audio, Back/Forward, responsive/accessibility matrix, system menu and Save surfaces, persistent settings/live locale, and layered recovery evidence. Browser forced close restores the last durable autosave rather than promising a last-moment async flush. M4's ambient-binding Agent task and owner-authorized participant handoff are delivered; independent review corrected the historical 59/44 script to the current 110/82 denominator. Engine review and Starter classification are complete with no additional Template change, and independent product re-review passed without a product-integrity blocker. Representative real current-low-end qualification is the only open gate; M4 is not closed and M5 has not started. | MIT               |
