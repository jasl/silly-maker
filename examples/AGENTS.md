...
| `vn-reference-tour/` | WIP VN Reference Product candidate. M0–M3 delivered its independent package, complete two-route author/headless data, engine-maintained default VN Player, final media/audio, Back/Forward, responsive/accessibility matrix, system menu and Save surfaces, persistent settings/live locale, and layered recovery evidence. Browser forced close restores the last durable autosave rather than promising a last-moment async flush. M4's ambient-binding Agent author task is delivered; human handoff and remaining product evidence stay open. | MIT               |
