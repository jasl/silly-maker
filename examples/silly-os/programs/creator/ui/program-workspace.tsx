...
import { CreatorChatSupplementV1 } from "./creator-chat-supplement.tsx";
...
} from "../../../src/ui/workspace-chrome.tsx";
import "./creator-program-workspace.css";
...
export interface ProgramWorkspacePropsV1 {
  ...
  readonly initialViewState?: ProgramWorkspaceSessionViewStateV1 | undefined;
...
function unavailableWorkspaceReviewV1(
  review: CreatorWorkspaceReviewProjectionV1,
...
function ProgramWorkspaceReadyV1({
  ...
  const [chatWidth, setChatWidth] = useState(initialState.chatWidth);
  const [chatMaximumWidth, setChatMaximumWidth] = useState(chatDefaultWidthV1);
  const [workpieceOpen, setWorkpieceOpen] = useState(initialState.workpieceOpen);
  const [fullscreen, setFullscreen] = useState(false);
  const [activeTab, setActiveTab] = useState<WorkpieceTabV1>(initialState.activeTab);
  const [mobilePane, setMobilePane] = useState<WorkspaceMobilePaneV1>(
    initialState.mobilePane,
  );
      ...
      <div className="program-workspace__body" ref={splitRef}>
        ...
        >
          ...
          <ChatPaneV1
            ...
            initialConversationViewState={initialState.conversation}
            onDraftChange={(draft) => {
              const current = viewStateRef.current;
            ...
            statusNotice={programAgentReadiness === undefined
              ...
              : (
                <CreatorReadinessNoticeV1
                  ...
                  surface="workspace"
                  {...(onOpenCreatorSettings === undefined
                    ? {}
                    : { onRecover: onOpenCreatorSettings })}
                />
              )}
            ...
            {...(piAgentRun === undefined ? {} : {
              piAgentRun: {
                ...piAgentRun,
                presentation: {
                  title: piAgentRun.runtime === "pi_provider"
                    ? copy.programAgentName
                    : copy.piTestTitle,
                  ready: copy.piTestReady,
                  unavailable: piAgentRun.runtime === "pi_provider"
                    ? copy.piLiveFailed
                    : copy.piTestFailed,
