...
import {
  createBrowserCreatorProgramWorkspaceAuthorityV1,
} from "../persistence/browser-creator-program-workspace-authority.ts";
import { createIndexedDbProgramDataRepositoryV1 } from "../../../src/application/persistence/indexeddb-program-data-repository.ts";
...
import type { WorkspaceImmutableSnapshotReceiptV1 } from "../../../src/workspace/contracts.ts";

interface MemoryProgramDataV1 {
  readonly repository: ReturnType<typeof createCreatorProgramDataRepositoryV1>;
}

function createMemoryProgramDataV1(): MemoryProgramDataV1 {
  return {
    repository: createCreatorProgramDataRepositoryV1(
      createIndexedDbProgramDataRepositoryV1({
        indexedDB: new IDBFactory(),
        keyRange: IDBKeyRange,
        facets: [indexedDbCreatorPersistenceFacetV1],
      }),
    ),
  };
}
...
interface FakeVolumeV1 {
  ...
  checkpointId: string;
  generation: number;
  candidate: WorkspaceImmutableSnapshotReceiptV1 | null;
  readonly retained: Map<string, WorkspaceImmutableSnapshotReceiptV1>;
...
interface FakeHostV1 {
  ...
  readonly prepared: WorkspaceImmutableSnapshotReceiptV1[];
  readonly adopted: WorkspaceImmutableSnapshotReceiptV1[];
  readonly imports: {
...
function fakeHostV1(): FakeHostV1 {
  ...
  const prepared: WorkspaceImmutableSnapshotReceiptV1[] = [];
  const adopted: WorkspaceImmutableSnapshotReceiptV1[] = [];
  ...
  const port: BrowserWorkspaceHostPagePortV1 = {
    ...
    async captureStableHead(workspaceSessionId) {
...
interface AuthorityHarnessV1 {
  readonly authority: ReturnType<typeof createBrowserCreatorProgramWorkspaceAuthorityV1>;
  readonly repository: CreatorProgramDataRepositoryV1;
  readonly host: FakeHostV1;
}

async function authorityHarnessV1(
  repositoryOverride?: CreatorProgramDataRepositoryV1,
  ...
  const authority = createBrowserCreatorProgramWorkspaceAuthorityV1({
    repository,
    host: host.port,
...
async function createIndependentProcessWorkspaceV1(input: {
  readonly harness: AuthorityHarnessV1;
...
async function acquireImportLeaseV1(input: {
  readonly harness: AuthorityHarnessV1;
  readonly processId: string;
  readonly observedAt?: number;
}) {
  const repository = input.harness.repository;
  const process = await repository.loadProcess(input.processId);
  const checkpoint = process?.checkpoint ?? null;
  if (process === null || checkpoint === null) {
    throw new Error("missing Process predecessor");
  }
  ...
  if (acquired.kind === "conflict") throw new Error("Process import acquire conflict");
...
async function acquireExecutionV1(input: {
  ...
  const review = await input.harness.authority.inspectProcessWorkspace(input.processId, {
    hostAccess: "required",
  });
...
describe("Browser Program Workspace authority V1", () => {
  it("creates one Catalog/Process/Workspace unit with an exact Program package pin", async () => {
    ...
    const [record, workspace, process, transcript, inspection] = await Promise.all([
      harness.repository.load(identity.programId),
      harness.repository.loadProcessWorkspaceBinding(identity.processId),
      ...
      harness.authority.inspectProcessWorkspace(identity.processId, {
    ...
    expect(createComposite).toHaveBeenCalledWith({
      catalog: expect.objectContaining({
        reviewedWorkspace: expect.objectContaining({
          processId: identity.processId,
          workspaceId: identity.workspaceId,
          generation: 1,
        }),
      ...
      workspace: expect.objectContaining({ processId: identity.processId }),
  ...
  it("creates and inspects a Process Workspace without pre-acquiring its idle volume", async () => {
    ...
    expect(harness.host.events.filter((event) => event === "host:close")).toHaveLength(1);
    expect(harness.host.events).not.toContain("host:attach");

    const opensBeforeIdleInspection =
      harness.host.events.filter((event) => event === "host:open").length;
    ...
    expect(harness.host.events.filter((event) => event === "host:open"))
      .toHaveLength(opensBeforeIdleInspection);
    ...
    const programOpened = await harness.authority.openProcessWorkspace({
  ...
  it("probes an exact Process volume without retaining or attaching a Workspace", async () => {
    const harness = await authorityHarnessV1();
    const identity = await createProgramV1({ harness });
    const opensBeforeProbe = harness.host.events.filter((event) => event === "host:open").length;
    const closesBeforeProbe = harness.host.events.filter((event) => event === "host:close").length;
    const attachesBeforeProbe =
      harness.host.events.filter((event) => event === "host:attach").length;

    await expect(harness.authority.probeProcessWorkspace(identity.processId)).resolves.toBe(true);

    expect(harness.host.events.filter((event) => event === "host:open"))
      .toHaveLength(opensBeforeProbe + 1);
    expect(harness.host.events.filter((event) => event === "host:close"))
      .toHaveLength(closesBeforeProbe + 1);
    expect(harness.host.events.filter((event) => event === "host:attach"))
      .toHaveLength(attachesBeforeProbe);
    expect(harness.host.activeSessionId()).toBeNull();

    const binding = await harness.repository.loadProcessWorkspaceBinding(identity.processId);
    if (binding === null) throw new Error("expected Process Workspace binding");
    harness.host.volumes.delete(binding.volumeId);
    await expect(harness.authority.probeProcessWorkspace(identity.processId)).rejects.toThrow(
      "fake.volume_missing",
    );
    expect(harness.host.activeSessionId()).toBeNull();
    await harness.authority.dispose();
  ...
  it("replays one exact Process Workspace composite when its response is lost", async () => {
    ...
    let createCalls = 0;
    const repository: CreatorProgramDataRepositoryV1 = {
      ...memory.repository,
  ...
  it("imports one UI source file at the exact Process head and releases the idle session", async () => {
    const harness = await authorityHarnessV1();
    const { subject, processInput } = await createIndependentProcessWorkspaceV1({
      harness,
      suffix: "import",
    });
    const execution = await acquireImportLeaseV1({
      harness,
  ...
  it("rejects a stale Process generation before touching its Workspace", async () => {
    const harness = await authorityHarnessV1();
    const { processInput } = await createIndependentProcessWorkspaceV1({
      harness,
      suffix: "stale-import",
    });
    const execution = await acquireImportLeaseV1({
  ...
  it("recovers a successful Process file import after the temporary close fails", async () => {
    ...
    expect(harness.host.events.filter((event) => event === "host:open")).toHaveLength(2);
    expect(harness.host.events.filter((event) => event === "host:close")).toHaveLength(3);
  ...
  it("does not guess a lost Process import outcome and lets the same source recover idempotently", async () => {
    ...
    const closesBeforeLostImport =
      harness.host.events.filter((event) => event === "host:close").length;

    await expect(harness.authority.importProcessWorkspaceFile(request)).rejects.toMatchObject({
      code: "outcome_unknown",
    });
    expect(harness.host.events.filter((event) => event === "host:close"))
      .toHaveLength(closesBeforeLostImport);
    await expect(harness.authority.importProcessWorkspaceFile(request)).resolves.toMatchObject({
      changed: false,
      source: { generation: 2 },
    });
    expect(harness.host.events.filter((event) => event === "host:close"))
      .toHaveLength(closesBeforeLostImport + 1);
    expect(harness.host.activeSessionId()).toBeNull();
    await harness.authority.dispose();
  });
  ...
  it("captures a successor review head and accepts it with the same Process transaction", async () => {
    ...
    const applied = await harness.authority.applyRevision({
      ...
      transcript: appendV1({
        ...
        updatedAt: 2,
        checkpoint: {
          checkpointId: `checkpoint.${identity.processId}.revision.2`,
          throughSequence: 2,
          workspaceId: identity.workspaceId,
    ...
    const inspection = await harness.authority.inspectProcessWorkspace(identity.processId);
    expect(inspection).toMatchObject({
      review: {
        latestAccepted: {
  ...
  it("rebinds an Agent terminal checkpoint to the exact captured review head", async () => {
    ...
    const applyComposite = vi.fn(async (
      input: Parameters<
        CreatorProgramDataRepositoryV1["commitProgramRevisionWithProcessExecutionTerminal"]
      >[0],
    ) => ({
  ...
  it("queries one exact composite operation when its terminal response is lost", async () => {
    ...
    const repository: CreatorProgramDataRepositoryV1 = {
      ...
      async queryCreatorProcessOperation(input) {
        operationQueries += 1;
        return await memory.repository.queryCreatorProcessOperation(input);
      },
    };
    const harness = await authorityHarnessV1(repository);
    const identity = await createProgramV1({ harness });
  ...
  it("requires every accepted Workspace snapshot, not only the latest decision", async () => {
    ...
    await expect(
      harness.authority.inspectProcessWorkspace(identity.processId, { hostAccess: "required" }),
  ...
  it("admits an Agent submit only for the exact Process package, attempt, and Workspace", async () => {
    ...
    const access = await harness.authority.setProcessNetworkAccess({
      processId: identity.processId,
      enabled: true,
    });
    expect(access).toMatchObject({ kind: "committed", value: { enabled: true } });
    const acquired = await acquireExecutionV1({
      harness,
      ...identity,
      attemptId: "attempt.agent-submit.one",
    });
    const attempt = acquired.process.activeAttempt;
    if (attempt === null) throw new Error("missing Agent submit attempt");
    ...
    const admitted = await harness.authority.withAgentSubmitAdmission({
      ...
      processId: identity.processId,
      programId: identity.programId,
      programPackage: acquired.process.programPackage,
      ...
      operation: async (value) => value?.enabled ?? false,
    });
    ...
    await expect(harness.authority.withAgentSubmitAdmission({
      ...
      workspaceSessionId: opened.snapshot.descriptor.workspaceSessionId,
      expectedCheckpointId: attempt.startingCheckpoint.workspaceCheckpointId,
      expectedGeneration: attempt.startingCheckpoint.workspaceGeneration + 1,
      operation: async () => true,
    })).rejects.toMatchObject({ code: "agent_submit_stale" });

    await expect(harness.authority.withAgentSubmitAdmission({
      agentRunId: "attempt.agent-submit.other",
      processAttemptGeneration: attempt.generation,
      processId: identity.processId,
      programId: identity.programId,
      programPackage: acquired.process.programPackage,
  ...
  it("resets Catalog/Process before purging detached Workspaces and disposes once", async () => {
    const memory = createMemoryProgramDataV1();
    const dispose = vi.fn(() => memory.repository.dispose());
    const repository: CreatorProgramDataRepositoryV1 = { ...memory.repository, dispose };
    const harness = await authorityHarnessV1(repository);
    const identity = await createProgramV1({ harness });
    const opened = await harness.authority.openProcessWorkspace({
      processId: identity.processId,
  ...
  it("joins an in-flight serialized operation before disposing its Repository owner", async () => {
    ...
    const repository: CreatorProgramDataRepositoryV1 = {
      ...memory.repository,
      async loadProcessNetworkAccess(processId) {
        started.resolve();
        await loadGate.promise;
        return await memory.repository.loadProcessNetworkAccess(processId);
      },
      dispose,
    };
    const harness = await authorityHarnessV1(repository);

    const pendingLoad = harness.authority.loadProcessNetworkAccess("process.test.missing");
