...
import type { BrowserPiReasoningEffortV1 } from "../../../src/agent/browser-pi-worker-protocol.ts";
import type { CreatorWorkspaceReviewProjectionV1 } from "../runtime/creator-workspace-review.ts";
import { ChatPaneV1, type ChatPanePropsV1 } from "../../../src/ui/chat-pane.tsx";
import type { CreatorActiveProcessProjectionV1 } from "../runtime/creator-controller.ts";
import type { PreviewProgramV1, ProgramProposalV1 } from "../runtime/contracts.ts";
import {
  CreatorChatSupplementV1,
  ProgramWorkspaceReviewV1,
} from "../ui/creator-chat-supplement.tsx";
import {
...
const changedReviewV1: CreatorWorkspaceReviewProjectionV1 = {
  revision: 1,
  latestAccepted: {
...
function activeProcessV1(
  workspaceReview: CreatorWorkspaceReviewProjectionV1 | null,
...
function renderChatV1(
  review: CreatorWorkspaceReviewProjectionV1 | null,
  proposal: ProgramProposalV1 = proposalV1,
) {
  return render(
    <ChatPaneV1
      copy={getCreatorCopyV1("en")}
      agentName="Program Agent"
      transcript={emptyTranscriptV1}
      feedSupplement={
        <CreatorChatSupplementV1
          copy={getCreatorCopyV1("en")}
...
function renderAgentChatV1(
  ...
  const { onSend = vi.fn(), decisionPending = false, piAgentRun, ...optionalProps } = props;
  const copy = getCreatorCopyV1("en");
  const presentedRun = piAgentRun === undefined ? undefined : {
    ...piAgentRun,
    presentation: {
      title: piAgentRun.runtime === "pi_provider" ? copy.programAgentName : copy.piTestTitle,
      ready: copy.piTestReady,
      unavailable: piAgentRun.runtime === "pi_provider" ? copy.piLiveFailed : copy.piTestFailed,
      draft: copy.piTestDraft,
      cancel: copy.piTestCancel,
      forgetCredential: copy.piTestForget,
    },
  };
  return render(
    <ChatPaneV1
      copy={copy}
      agentName={copy.programAgentName}
      transcript={emptyTranscriptV1}
      feedSupplement={
        <CreatorChatSupplementV1
          copy={copy}
      ...
      onSend={onSend}
      {...(presentedRun === undefined ? {} : { piAgentRun: presentedRun })}
      {...optionalProps}
    />,
  );
}
...
describe("SillyOS Workspace composer model selection", () => {
  ...
  it("uses the shared picker and hides the persistent live Provider card when ready or completed", () => {
    ...
    const picker = screen.getByRole("combobox", { name: "Program Agent model" });
    ...
    const reasoningPicker = screen.getByRole("combobox", {
      name: "Program Agent reasoning effort",
    });
  ...
  it("disables the model picker during a live run while retaining its Cancel control", () => {
    ...
    expect(screen.getByRole("combobox", { name: "Program Agent model" })).toBeDisabled();
    expect(screen.getByRole("combobox", { name: "Program Agent reasoning effort" }))
      .toBeDisabled();
    expect(screen.queryByText("Model Provider")).toBeNull();
    expect(screen.queryByRole("button", { name: "Forget Provider key" })).toBeNull();
...
describe("SillyOS Workspace review presentation", () => {
  ...
  it("renders the same review contract with Chinese copy", () => {
    render(
      <ProgramWorkspaceReviewV1 copy={getCreatorCopyV1("zh-CN")} review={changedReviewV1} />,
    );
  ...
  it("stops presenting an old Authority head as current when live execution advances or fails", () => {
    const exactReview: CreatorWorkspaceReviewProjectionV1 = {
      ...changedReviewV1,
      mutableHead: {
