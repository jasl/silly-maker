...
import {
  createCreatorControllerV1 as createCreatorControllerImplementationV1,
...
import type { CreatorAgentTerminalRunV1, PreviewProgramV1 } from "../runtime/contracts.ts";
import type { CreatorWorkspaceReviewProjectionV1 } from "../runtime/creator-workspace-review.ts";
import {
  createCreatorPersistenceTestAdapterV1,
  type CreatorPersistenceTestAdapterV1,
} from "./creator-persistence-test-adapter.ts";

type TestProgramDataRepositoryV1 = CreatorPersistenceTestAdapterV1;
...
function createCreatorControllerV1(
  input:
    & Omit<
      Parameters<typeof createCreatorControllerImplementationV1>[0],
      "programPackage"
    >
    & { readonly programPackage?: InstalledProgramPackageReferenceV1 },
) {
  return createCreatorControllerImplementationV1({
    ...input,
    programPackage: input.programPackage ?? creatorProgramPackageV1,
  });
}

function createMemoryProgramDataRepositoryV1(): TestProgramDataRepositoryV1 {
  return createCreatorPersistenceTestAdapterV1({
...
function workspaceReviewV1(
  record: Awaited<ReturnType<CreatorProgramDataRepositoryV1["load"]>>,
  process: ProcessHeadV1,
): CreatorWorkspaceReviewProjectionV1 | null {
  if (record === null) return null;
  const pending = record.head.pendingReviewBinding?.processId === process.processId
    ? record.head.pendingReviewBinding
    : null;
  const checkpoint = process.checkpoint;
  const mutableHead = checkpoint === null ? null : {
    checkpointId: checkpoint.workspaceCheckpointId,
    generation: checkpoint.workspaceGeneration,
  };
  const accepted = record.latestDecision?.status === "accepted"
    ? record.latestDecision.snapshot
    : null;
  const statusV1 = (
    expected: { readonly checkpointId: string; readonly generation: number } | null,
  ): CreatorWorkspaceReviewProjectionV1["pendingStatus"] =>
    expected === null || mutableHead === null
      ? expected === null ? null : "unavailable"
      : expected.checkpointId === mutableHead.checkpointId &&
          expected.generation === mutableHead.generation
      ? "matches"
      : "changed";
  return {
    revision: 1,
...
function processWorkspaceCheckpointV1(input: {
  readonly process: ProcessHeadV1;
  readonly workspace: ProcessWorkspaceBindingV1;
  readonly semanticCheckpoint: Pick<ProcessCheckpointV1, "checkpointId" | "throughSequence">;
  readonly workspaceHead?: {
    readonly checkpointId: string;
    readonly generation: number;
  };
}): ProcessCheckpointV1 {
  const workspaceHead = input.workspaceHead ?? (input.process.checkpoint === null ? null : {
    checkpointId: input.process.checkpoint.workspaceCheckpointId,
    generation: input.process.checkpoint.workspaceGeneration,
  });
  if (workspaceHead === null) throw new Error("expected Process Workspace checkpoint");
  return {
    ...input.semanticCheckpoint,
    workspaceId: input.workspace.workspaceId,
    workspaceCheckpointId: workspaceHead.checkpointId,
    workspaceGeneration: workspaceHead.generation,
  };
}

async function loadProcessWorkspaceV1(
...
> {
  ...
  return process === null || workspace === null ? null : { process, workspace };
}
...
function createControllerWorkspaceV1(
  ...
  return {
    async inspectProcessWorkspace(processId) {
      const pair = await loadProcessWorkspaceV1(repository, processId);
      if (pair === null) return null;
      const subject = pair.process.subjectProgramId === null
        ? null
        : await repository.load(pair.process.subjectProgramId);
      return {
        process: pair.process,
        workspace: pair.workspace,
        review: subject === null ? null : workspaceReviewV1(subject, pair.process),
        mutableHead: pair.process.checkpoint === null ? null : {
          checkpointId: pair.process.checkpoint.workspaceCheckpointId,
          generation: pair.process.checkpoint.workspaceGeneration,
        },
      };
    },
    ...
    async create(input) {
      ...
      const workspaceHead = {
        checkpointId: `workspace-checkpoint.${input.process.processId}.1`,
        generation: 1,
      };
      return await repository.createProgramWithProcess({
        catalog: {
          ...
          reviewedWorkspace: {
            ...workspace,
            checkpointId: workspaceHead.checkpointId,
            generation: workspaceHead.generation,
          },
        },
    ...
    async applyRevision(input) {
      ...
      const pair = await loadProcessWorkspaceV1(repository, input.transcript.processId);
      const binding = current?.head.pendingReviewBinding ?? null;
      if (current === null || pair === null || binding === null) {
        return {
          kind: "conflict",
      ...
      return await repository.applyProgramRevisionWithProcessTranscript({
        ...
        transcript: {
          ...input.transcript,
          checkpoint: processWorkspaceCheckpointV1({
            process: pair.process,
            workspace: pair.workspace,
            semanticCheckpoint: input.transcript.checkpoint ?? {
              checkpointId: `${input.transcript.commitId}.checkpoint`,
              throughSequence: input.transcript.entries.at(-1)!.sequence,
            },
            workspaceHead,
          }),
        },
...
function createPassiveControllerWorkspaceV1(): CreatorControllerWorkspacePortV1 {
  const unusedMutationV1 = (): Promise<never> =>
    Promise.reject(new Error("passive Creator controller test does not mutate a Workspace"));
  return {
    inspectProcessWorkspace: () => Promise.resolve(null),
    closeActiveWorkspace: () => Promise.resolve(),
    create: unusedMutationV1,
    applyRevision: unusedMutationV1,
    applyAgentRevision: unusedMutationV1,
    decide: unusedMutationV1,
  };
}
...
async function seedProgramV1(input: {
  ...
  const created = await input.repository.create({
    ...
    reviewedWorkspace: {
      processId: `review.${input.programId}`,
      workspaceId: `workspace.${input.programId}`,
      volumeId: `volume.${input.programId}`,
      workspaceFormat: 1,
      checkpointId: `checkpoint.${input.programId}.1`,
      generation: 1,
    },
...
describe("Creator Controller Program/Process projection", () => {
  it("pages Program summaries without a total count cap", async () => {
  ...
  it("fences a late catalog page after a newer catalog initialization wins", async () => {
    ...
    const fencedRepository: CreatorProgramDataRepositoryV1 = {
      ...repository,
  ...
  it("keeps an active Creator controller stable when another compatibility marker is opened", async () => {
    ...
    const predecessor = createCreatorControllerV1({
      ...
      ownerInstanceId: "creator.predecessor",
    ...
    await Promise.all([predecessor.initialize(), successor.initialize()]);

    expect(await predecessor.openProgram("program.subject")).toEqual({
      kind: "completed",
      value: true,
    });
    expect(predecessor.getSnapshot()).toMatchObject({
      route: "process",
      activeProcess: {
        process: { processId: "process.creator" },
      },
    });
    expect(await successor.openProgram("program.subject")).toEqual({
      kind: "completed",
      value: true,
    });
    expect(successor.getSnapshot()).toMatchObject({
      route: "process",
      activeProcess: {
        process: { processId: "process.creator.successor" },
      },
    });
    expect(await successor.openProcess("process.creator")).toEqual({
      kind: "failed",
      code: "program_package_mismatch",
    });
    expect(await predecessor.openProcess("process.creator.successor")).toEqual({
      kind: "failed",
      code: "program_package_mismatch",
    });
    expect(await predecessor.openProcess("process.creator")).toEqual({
      kind: "completed",
      value: true,
    });
    expect(predecessor.getSnapshot().activeProcess?.process.processId).toBe("process.creator");
  });
  ...
  it("terminalizes an exact active attempt as replaced when Program currentness advances", async () => {
    ...
    const pair = await loadProcessWorkspaceV1(repository, run.processId);
    if (current === null || pair === null) throw new Error("expected current Program Workspace");
    expect(
  ...
  it("retries passive expired-attempt recovery when the Workspace owner remains busy", async () => {
    ...
    const passive = createCreatorControllerV1({
      ...
      workspace: {
        ...baseWorkspace,
        async inspectProcessWorkspace(processId, options) {
          ...
          return await baseWorkspace.inspectProcessWorkspace(processId, options);
        },
        async closeActiveWorkspace() {
  ...
  it("declines retry after Workspace evidence drifts without rewriting the settled terminal", async () => {
    ...
    const successor = createCreatorControllerV1({
      ...
      workspace: {
        ...baseWorkspace,
        async inspectProcessWorkspace(processId) {
          const inspection = await baseWorkspace.inspectProcessWorkspace(processId);
          if (!workspaceDrifted || inspection === null || inspection.review === null) {
            return inspection;
          }
          const mutableHead = {
      ...
      const successor = createCreatorControllerV1({
        ...
        workspace: {
          ...
          async inspectProcessWorkspace(processId) {
            ...
            return inspection === null || inspection.review === null ? inspection : {
              ...inspection,
              mutableHead,
              review: { ...inspection.review, mutableHead },
            };
          },
        },
        budgets: ordinaryBudgetsV1,
