...
} from "../runtime/memory-program-catalog-repository.ts";
import type {
...
import { cloneProgramCatalogHeadV1 } from "../runtime/program-catalog-repository.ts";
import type { PreviewProgramV1 } from "../runtime/contracts.ts";
import type { CreatorWorkspaceSnapshotReceiptV1 } from "../runtime/creator-workspace-review.ts";
...
describe("Memory Program catalog repository", () => {
  it("stores normalized heads, immutable revisions, decisions, and Process review bindings", async () => {
    ...
    expect((await reopened.load("program.alpha"))?.latestDecision).toMatchObject({
      repositoryRevision: 2,
      programRevision: 1,
    });
  });
  ...
  it("treats reordered but semantically identical mutation inputs as exact replay", async () => {
    ...
    const reorderedCreate = {
      ...
      reviewedWorkspace: {
        generation: 1,
        checkpointId: "checkpoint.program.canonical.1",
        workspaceFormat: 1,
        volumeId: `volume.${programId}`,
        workspaceId: `workspace.${programId}`,
        processId: `process.${programId}`,
    ...
    const applyInput: ProgramCatalogApplyRevisionInputV1 = {
      ...
      reviewedWorkspace: reviewedWorkspaceV1(programId, 2),
      updatedAt: 2,
    };
    ...
    const reorderedApply = {
      updatedAt: 2,
      reviewedWorkspace: {
        ...reorderedCreate.reviewedWorkspace,
        generation: 2,
        checkpointId: "checkpoint.program.canonical.2",
      },
      proposalId: "proposal.canonical.2",
      program: {
  ...
  it("preserves every immutable revision without a revision-history cap", async () => {
    ...
    for (let revision = 2; revision <= 40; revision += 1) {
      ...
      const applied = await repository.applyRevision({
        ...
        reviewedWorkspace: reviewedWorkspaceV1("program.revisions", revision),
        updatedAt: revision,
      });
  ...
  it("uses exact CAS and commit idempotency without changing a predecessor on conflict", async () => {
    ...
    expect(
      await repository.create({
        ...
        reviewedWorkspace: reviewedWorkspaceV1("program.cas"),
        updatedAt: 1,
      }),
    ...
    const mutation = {
      ...
      reviewedWorkspace: reviewedWorkspaceV1("program.cas", 2),
      updatedAt: 2,
    } as const;
  ...
  it("rejects apply and decision timestamps older than the current head", async () => {
    ...
    await repository.create({
      ...
      reviewedWorkspace: reviewedWorkspaceV1("program.time"),
      updatedAt: 10,
    });
    ...
    expect(
      await repository.applyRevision({
        ...
        reviewedWorkspace: reviewedWorkspaceV1("program.time", 2),
        updatedAt: 9,
      }),
    ).toMatchObject({ kind: "conflict" });
    expect(
  ...
  it("pages accepted lineage while rejected rows consume budget and advance the cursor", async () => {
    ...
    for (let revision = 2; revision <= 40; revision += 1) {
      ...
      await repository.applyRevision({
        ...
        reviewedWorkspace: reviewedWorkspaceV1(programId, revision),
