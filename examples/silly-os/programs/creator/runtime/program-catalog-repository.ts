...
import {
  admitCreatorWorkspaceSnapshotReceiptV1,
  type CreatorWorkspaceSnapshotReceiptV1,
} from "./creator-workspace-review.ts";
...
export interface ProgramCatalogAcceptedDecisionV1 {
  ...
  readonly repositoryRevision: number;
  readonly snapshot: CreatorWorkspaceSnapshotReceiptV1;
}
...
/** Exact Process-owned Workspace head reviewed for one Program proposal. */
export interface ProgramCatalogReviewedWorkspaceV1 {
  readonly processId: string;
  readonly workspaceId: string;
  readonly volumeId: string;
  readonly workspaceFormat: 1;
...
export interface ProgramCatalogCreateInputV1 {
  ...
  readonly proposalId: string;
  readonly reviewedWorkspace: ProgramCatalogReviewedWorkspaceV1;
  readonly updatedAt: number;
}
...
export type ProgramCatalogDecideInputV1 =
  | (ProgramCatalogDecideInputBaseV1 & {
    readonly status: "accepted";
    readonly snapshotReceipt: CreatorWorkspaceSnapshotReceiptV1;
  })
  | (ProgramCatalogDecideInputBaseV1 & {
    readonly status: "rejected";
  });
...
/**
 * Explicit Host-shared library of Creator-produced Programs. This is not a
 * Process domain-data namespace; Process Conversation, Workspace, settings,
 * lease, and facet state remain isolated by Process identity.
 */
...
export function cloneProgramCatalogReviewedWorkspaceV1(
  value: ProgramCatalogReviewedWorkspaceV1,
): ProgramCatalogReviewedWorkspaceV1 {
  if (
    !identifierV1(value.processId) || !identifierV1(value.workspaceId) ||
    !identifierV1(value.volumeId) || value.workspaceFormat !== 1 ||
    !identifierV1(value.checkpointId) || !positiveIntegerV1(value.generation)
  ) throw new TypeError("invalid reviewed Process Workspace");
  return {
...
export function cloneProgramCatalogReviewBindingV1(
  ...
  if (
    ...
    !positiveIntegerV1(value.repositoryRevision) || !identifierV1(value.processId) ||
...
export function cloneProgramCatalogDecisionV1(
  ...
  const snapshot = admitCreatorWorkspaceSnapshotReceiptV1(value.snapshot);
...
export function normalizeProgramCatalogCreateInputV1(
  ...
  const reviewedWorkspace = cloneProgramCatalogReviewedWorkspaceV1(input.reviewedWorkspace);
  if (
    !identifierV1(input.commitId) || program.revision !== 1 ||
    !identifierV1(input.proposalId) || !timestampV1(input.updatedAt)
...
export function normalizeProgramCatalogApplyRevisionInputV1(
  ...
  if (
    ...
    program.programId !== input.programId || !timestampV1(input.updatedAt)
...
export function normalizeProgramCatalogDecideInputV1(
  ...
  if (
    ...
    !identifierV1(input.expectedProposal.proposalId) ||
    !positiveIntegerV1(input.expectedProposal.programRevision) ||
    !identifierV1(input.commitId) || !timestampV1(input.updatedAt)
  ) throw new TypeError("invalid Program catalog decision");
  if (input.status === "accepted") {
    const snapshotReceipt = admitCreatorWorkspaceSnapshotReceiptV1(input.snapshotReceipt);
