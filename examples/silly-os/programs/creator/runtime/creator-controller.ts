...
import type { CreatorWorkspaceReviewProjectionV1 } from "./creator-workspace-review.ts";
import type {
  ...
  CreatorBrowserProcessWorkspaceInspectionV1,
...
export function createCreatorControllerV1(input: {
  ...
  const programPackage = admitInstalledProgramPackageReferenceV1(input.programPackage);
  ...
  const openProcessAtEpochV1 = async (
    ...
    try {
      ...
      if (!sameProgramPackageV1(process.programPackage, programPackage)) {
        return fail("open", "program_package_mismatch", null, null);
      }
      const [subject, executionLease, passiveWorkspaceReview] = await Promise.all([
        ...
        input.workspace.inspectProcessWorkspace(process.processId, {
      ...
      let workspaceReview = passiveWorkspaceReview?.review ?? null;
      ...
      if (
        ...
        try {
          const recovered = await withTemporaryWorkspaceReleaseV1(async () => {
            const inspection = await input.workspace.inspectProcessWorkspace(
              expiredProcess.processId,
              { hostAccess: "required" },
            );
            return {
              process: await settleExpiredAttemptV1(
                expiredProcess,
                executionLease,
    ...
    try {
      ...
      do {
        ...
        process = page.summaries.find((summary) =>
          sameProgramPackageV1(summary.programPackage, programPackage)
        );
    ...
    try {
      const result = await input.workspace.create({
        ...
        transcript: {
          ...
          checkpoint: {
            checkpointId: `${processId}.created`,
            throughSequence: 2,
          },
  ...
  const sendFollowUpV1 = async (
    ...
    const creatorReply = creator.followUp({
      workspace: {
        workspaceId: active.process.checkpoint!.workspaceId,
  ...
  const controller: CreatorControllerV1 = {
    ...
    async prepareAgentRun(rawText) {
      ...
      try {
        ...
        const mutableHead = inspection?.mutableHead ?? null;
        if (mutableHead === null) {
          publish({
            route: snapshot.route,
            catalog: snapshot.catalog,
    ...
    async retryInterruptedAgentRun() {
      ...
      try {
        ...
        const review = inspection?.review ?? null;
        if (
