...
describe("SillyOS translation batch protocol", () => {
  ...
  it("admits syntactically valid unit identifiers longer than the former arbitrary limit", () => {
    const unitId = `unit.${"segment".repeat(24)}`;
    expect(admitTranslationBatchRequestV1({
      ...requestV1,
      glossary: [],
      units: [{ ...requestV1.units[0]!, unitId }],
    })).toMatchObject({ kind: "admitted", request: { units: [{ unitId }] } });
  });
