...
- one ordinary `sillyos.translation@1.0.0` package-backed Process route. A
...
artifacts. It pins the complete immutable package reference
`{ programId, packageVersion, contentDigest }`; refresh, a bundled successor, or
another imported package never substitutes newer content into that Process.
Removing a package from the current library does not erase its Conversation.
When the exact package, Workspace, or optional domain storage cannot be opened,
SillyOS degrades to the package-independent pageable Conversation rather than
claiming that another package can resume the work.

Creator and Translation are ordinary bundled distribution sources. Their
archives use the same admission, IndexedDB installation repository, exact
loader, fixed runtime-profile selection, and Program UI Container as a
user-selected ZIP. Bundling grants no runtime capability or acquisition-origin
privilege. A package can select only a runtime profile already supplied by the
fixed SillyOS harness; it cannot add same-realm TypeScript, React, an
interpreter, or an IndexedDB schema from the archive.

The current implementation is intentionally smaller than a general Program
SDK or package manager. Build-known profile modules are acquired lazily, and
capability IDs remain descriptive package data rather than tool grants. There
is no dependency solver, arbitrary code loader, cross-Program service locator,
automatic VFS migration, or forward/backward compatibility guarantee. During
pre-stable development an incompatible product database is cleared instead of
retaining an obsolete reader or alias.
...
`programs/translation/package/`; they are not yet a general settings
...
binding, exact Host import, Browser Translation dispatch, V16 Process workset,
