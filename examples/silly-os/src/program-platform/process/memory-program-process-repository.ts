...
export interface MemoryProgramProcessRepositoryBackingV1 {
  ...
  readonly processSettingsOverrides: Map<string, ProcessSettingsOverrideV1>;
  ...
  readonly processSummaryOrder: string[];
...
export function createMemoryProgramProcessRepositoryBackingV1(): MemoryProgramProcessRepositoryBackingV1 {
  return {
    processes: new Map(),
    processSettingsOverrides: new Map(),
...
function removeProcessSummaryIndexV1(
  ...
  const globalIndex = backing.processSummaryOrder.indexOf(summary.processId);
  if (globalIndex >= 0) backing.processSummaryOrder.splice(globalIndex, 1);
  backing.processSummaries.delete(summary.processId);
}

function insertProcessSummaryIdV1(
  backing: MemoryProgramProcessRepositoryBackingV1,
  order: string[],
  summary: ProcessSummaryV1,
): void {
...
function writeProcessSummaryIndexV1(
  ...
  const order = backing.processSummaryOrderBySubject.get(summary.subjectProgramId) ?? [];
  insertProcessSummaryIdV1(backing, order, summary);
  backing.processSummaryOrderBySubject.set(summary.subjectProgramId, order);
  insertProcessSummaryIdV1(backing, backing.processSummaryOrder, summary);
}
  ...
  return {
    async createProcess(rawInput) {
      ...
      const candidate: ProcessHeadV1 = {
        ...
        revision: 1,
        programPackage: normalized.programPackage,
      ...
      try {
        ...
        backing.processSettingsOverrides.set(normalized.processId, {
          schemaVersion: 1,
          processId: normalized.processId,
          revision: 1,
          overrideJson: null,
        ...
        backing.processSettingsOverrides.delete(normalized.processId);
    ...
    async loadProcessSettingsOverride(rawProcessId) {
      assertAvailableV1("load_process_settings_override");
      const processId = normalizeProcessIdV1(rawProcessId);
      if (loadProcessV1(processId, "load_process_settings_override") === null) return null;
      const stored = backing.processSettingsOverrides.get(processId);
      if (stored === undefined) return null;
      try {
        return cloneProcessSettingsOverrideV1(stored);
      } catch {
        // Settings are optional. A bad settings row cannot make the Process unreadable.
        return null;
      }
    },

    async setProcessSettingsOverride(rawInput) {
      assertAvailableV1("set_process_settings_override");
      const input = normalizeProcessSettingsOverrideMutationInputV1(rawInput);
      const process = loadProcessV1(input.processId, "set_process_settings_override");
      if (process === null) return { kind: "conflict", current: null };
      const stored = backing.processSettingsOverrides.get(input.processId);
      let current: ProcessSettingsOverrideV1 | null = null;
      try {
        current = stored === undefined ? null : cloneProcessSettingsOverrideV1(stored);
      } catch {
        // A malformed optional row degrades to package defaults and is not overwritten blindly.
      }
      if (
      ...
      const next = cloneProcessSettingsOverrideV1({
        ...current,
        revision: current.revision + 1,
        overrideJson: input.admittedOverrideJson,
        updatedAt: input.updatedAt,
      });
      backing.processSettingsOverrides.set(input.processId, next);
      return { kind: "committed", settings: cloneProcessSettingsOverrideV1(next) };
    },
    ...
    async listRecentProcessSummaries(rawInput) {
      assertAvailableV1("list_recent_process_summaries");
      const normalized = normalizeRecentProcessSummaryListInputV1(rawInput);
      const order = backing.processSummaryOrder;
      const loadSummaryAtV1 = (index: number): ProcessSummaryV1 => {
        const processId = order[index];
        const stored = processId === undefined
          ? undefined
        ...
        try {
          return cloneProcessSummaryV1(stored);
        } catch {
          throw createProgramProcessRepositoryFailureV1(
            "schema_invalid",
            "list_recent_process_summaries",
    ...
    async beginProcessAttempt(rawInput) {
      ...
      const activeAttempt = {
        ...
        settingsOverrideJson: (() => {
          const stored = backing.processSettingsOverrides.get(normalized.processId);
          if (stored === undefined) return null;
          try {
            return cloneProcessSettingsOverrideV1(stored).overrideJson;
          } catch {
            return null;
          }
        })(),
    ...
    async reset(): Promise<void> {
      ...
      backing.processSettingsOverrides.clear();
      backing.processSummaries.clear();
      backing.processSummaryOrder.length = 0;
