...
import { workspaceArchiveFileNameV1 } from "../ui/silly-os-app.tsx";
...
describe("SillyOS workspace archive filename", () => {
  it("truncates long ASCII names at the exact admitted UTF-8 boundary", () => {
    const fileName = workspaceArchiveFileNameV1("A".repeat(256));
  ...
  it("truncates multibyte names without splitting a Unicode code point", () => {
    const fileName = workspaceArchiveFileNameV1("界".repeat(100));
  ...
  it("preserves the exact legal maximum and removes dangerous filename characters", () => {
    const exact = workspaceArchiveFileNameV1("a".repeat(243));
    const sanitized = workspaceArchiveFileNameV1("../Foo\\Bar/\0<script>..");
    const fallback = workspaceArchiveFileNameV1("../\\\0");
