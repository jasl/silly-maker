...
import {
  ...
  piNetworkDisabledErrorCodeV1,
} from "./pi-network-tool-binder.ts";
import {
...
export function createBrowserPiWorkerRuntimeV1(input: {
  ...
  const requireNetworkAccessV1 = (
    run: ActivePiRunV1,
    operation: "fetch_url" | "download",
    rawUrl: string,
  ): string => {
    if (
  ...
  const executeFetchUrlV1 = async (
    ...
    const url = requireNetworkAccessV1(run, "fetch_url", rawUrl);
  ...
  const executeDownloadV1 = async (
    ...
    const url = requireNetworkAccessV1(run, "download", downloadInput.url);
    const client = networkClient;
    if (client === null) throw new Error("Browser Network Broker is unavailable");
    const result = await withRunNetworkSignalV1(
      run,
  ...
  const createRun = async (
    ...
    const createWorkspaceToolV1 = (toolId: BrowserBundledProgramHarnessToolIdV1) => {
      switch (toolId) {
        ...
        case "write":
          return bindPiWorkspaceWriteToolV1(createWriteTool(), workspaceRun);
        case "edit":
          return bindPiWorkspaceEditToolV1(createEditTool(), workspaceRun);
        case "bash":
          return bindPiWorkspaceBashToolV1(createBashTool(), workspaceRun);
        case "grep":
          return createPiWorkspaceGrepToolV1(workspaceRun);
        case "fetch_url":
          return createPiFetchUrlToolV1({
            execute: (toolCallId, url, signal) => executeFetchUrlV1(run, toolCallId, url, signal),
          });
        case "download":
          return createPiDownloadToolV1({
