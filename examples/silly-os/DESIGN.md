...
| Live Provider workspace tools     | The current Creator runtime profile receives Pi's native `read`/`write`/`edit`/`bash` plus the fixed structured `grep` capability over the same Process-bound independent-origin Authority. Translation grants no Workspace tools. The public `a17c3490` Chromium Anthropic journey proves one bounded `write`/`write`/`bash-qjs` loop, exact Sandbox bytes/generation/receipts, cancel/currentness, no key in the inspected control-origin durable projection, Forget, and retained output afterward                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Real-model `read`, `edit`, `grep`, `fetch_url`, and `download` use are not proved on the public origin; the one qualified loop does not promote general tool capability                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
candidate publish atomically with the Process terminal; an expired foreground
Run is retryable only when the exact Workspace checkpoint/source binding is
still current and no candidate exists. Every terminal publication is fenced by
...
declares `agent.text` and `translation.batch`; its runtime profile selects no
Workspace tools, `bash` or `qjs`, and its formal package has `scripts: []`.
