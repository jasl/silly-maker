...
| Product/Workspace storage     | Product metadata and preferences remain on the control origin. Physical Program Data Repository V11 retains the normalized Program definitions/heads/revisions/decisions, Process/transcript rows, Process execution leases, and single `process_commits` operation-receipt authority. It also owns compact Translation Project heads, exact import-operation receipts, and separately pageable unit/glossary rows. Its `process_workspace_bindings` store gives one non-Creator Process an exact unique Workspace/volume identity, distinct from Program-scoped Workspace continuation and network-access rows. Process creation, its initial transcript checkpoint, and that binding share one IndexedDB transaction; Sandbox-origin OPFS remains the physical byte authority and is not part of that transaction. Translation source import acquires the existing Process execution lease after parsing while atomically verifying that its Project is still absent or at the caller's exact staging revision; the Workspace write and every later Project mutation verify that attempt/generation. Ready Project publication and the completed Process terminal/checkpoint share one IndexedDB transaction. An expired unfinished import is retained as unrecoverable for direct review, while ordinary start skips it and creates a fresh Process. Missing network rows mean disabled | V4–V8 preview database rows and control-origin OPFS bytes are not migrated; the latter may remain browser-managed but are unreachable. Exact V9/V10 databases upgrade additively to V11. The ordinary Translation controller uses the same Workspace Authority for exact original-file import and binds its resulting checkpoint to the Project. This is fenced sequencing rather than an atomic IDB/OPFS transaction, general import/restore framework, complete import-to-export journey, or fixed cross-device durability claim |
...
presentation on desktop and mobile. Durable target save/mutation is disabled,
and committed counters remain zero until the future candidate-commit loop. A
10,000-unit test proves DOM windowing rather than a 10,000-row render. Repository V11 now backs that
projection with one compact Translation Project head and separately pageable
unit/glossary rows; the UI source requests a bounded row window instead of a
complete aggregate. The Project has no arbitrary semantic unit ceiling. Batch
...
cold-reopens the durable Project without attaching the idle Workspace. Existing
...
Authority before publishing an exact source/checkpoint binding and ready Project
...
This foundation does not yet include the real Agent batch/target-commit/resume
loop, structured Review or QA, final exporter, OpenUI, Python, or CodeAct. The
guided workbench consumes the controller import and pageable Project source,
but remains an import-and-browse foundation. It must not be described as a
...
| Translation        | A usable translation Program                                   | P5-A research plus the ordinary `sillyos.builtin.translation` revision 1 Process route, independent Process Workspace, exact original-file import, V11 Project head/pageable rows, cold reopen, four deterministic text formats, born-digital PDF text-only projection, 10,000-unit DOM virtualization, and connected guided import/page source. The Definition grants no Agent tools | Add the real Agent batch/candidate commit/resume, structured Review/QA, final format-preserving export, OpenUI, and target qualification                            |
