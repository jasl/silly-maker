...
Installation Repository V1 plus Program Data Repository V16. Every Process pins an
exact `{ programId, packageVersion, contentDigest }` package reference and owns
its own Workspace binding, settings override, network preference and domain
state. V16 row-blind resets every earlier preview Program Data Repository schema; it
retains no old wire, method, type alias, migration reader or compatibility
fallback. Within an already-current V16 database, a removed optional facet's
...
repository, exact-package loader, runtime-profile selection and Program UI
Container. Installation records deliberately omit acquisition origin, so a
bundled package receives no runtime privilege.
...
that must always be optional. An installed Program package is immutable and may
be shared read-only; every Process created from it has an independent Workspace,
...
Program content. One durable Creator Process pins the exact Creator package, owns
...
itself is distributed as `sillyos.creator@1.0.0`, with an exact content digest
derived by ordinary package admission. A Creator Process pins that complete
reference at creation and may name the Program being created or edited as its
subject. Installing a successor package changes only the pointer for newly
created Processes; it does not rewrite this Process harness. The Process owns its user-visible pageable
...
```text
programs/<name>/
|-- package/          exact files admitted into bundled or imported archives
...
profile, optional Program storage or Process Workspace cannot be loaded, the
...
Agent or mutation actions and does not substitute a newer package. The complete
package contract lives in [PROGRAM-PACKAGE-DESIGN.md](./PROGRAM-PACKAGE-DESIGN.md).
Recent-Process restore resolves the exact package and runtime profile, then
...
1. **Program Package Installation Repository V1** stores admitted immutable
   package archives in `packages` and one `package_heads` current pointer per
   Program for newly created Processes. Bundled and ZIP-imported packages enter
   this same repository; stored records have no acquisition-origin privilege.
2. **SillyOS Program Data Repository V16** has a Program-neutral Conversation Core for
   exact-package Process heads, pageable rich Conversations, Process Workspace
...
| Package/Product/Workspace storage | Program Package Installation Repository V1 retains immutable admitted archives in `packages` and one `package_heads` pointer for new Processes; bundled and ZIP-imported packages enter the same repository with no acquisition-origin privilege. Program Data Repository V16 has a Program-neutral Conversation Core for exact-package Process heads, pageable Conversations, Process execution leases, `process_workspace_bindings`, `process_settings_overrides`, `process_network_access` and the single Core Process `process_commits` operation-receipt authority. Creator proposal/catalog state and Translation workset state live in their own build-known optional persistence facets; a facet may keep domain-local mutation idempotency records and join the exact Core Process transaction without becoming a second Process or Conversation authority. A Translation Process directly owns one compact workset head, exact receipts, and pageable unit/glossary/candidate rows; there is no independent Project identity, list, route, or lifecycle. Process creation, its initial transcript checkpoint and Workspace binding share one IndexedDB transaction; Sandbox-origin OPFS remains the physical byte authority and is not part of that transaction. Missing settings/network rows fall back to package defaults/default-off. | V16 row-blind resets every older preview Program Data Repository instead of carrying migration readers or compatibility aliases. In the current version, removed/unknown facet stores are ignored and a selected missing/damaged facet degrades to unavailable; explicit reset clears Core and all present facet stores. Old control-origin OPFS bytes may remain browser-managed but unreachable. Translation original-file import and later workset mutations are fenced against the exact Process lease/generation and checkpoint rather than pretending IndexedDB and OPFS share one transaction. This is not a general Workspace archive/restore framework, complete import-to-export journey, or fixed cross-device durability claim |
...
- A Creator Process pins its exact Creator package, owns the pageable user-visible
  Conversation and exact attempt plus starting/terminal checkpoint metadata,
  and coordinates review. The Program owns its selected Pi profile/capabilities
  and accepted artifacts, not that Process or Conversation. Pi's private
  session/continuation remains a separate opaque execution authority.
...
the exact installed packages available for a new Process, includes the ordinary
...
Starting a Program creates a Process pinned to that exact installed package.
Opening a recent Process first attempts its exact package/runtime profile and
independent Workspace. If any optional service is unavailable, the same route
opens the pageable package-independent read-only Conversation without settings,
Workspace, Agent or mutation actions; it never substitutes the package head for
new Processes.
...
1. a compact product bar containing Library, exact Program identity, and low-frequency
   Program actions;
...
10,000-unit test proves DOM windowing rather than a 10,000-row render. Program Data Repository V16 backs that
...
The ordinary Translation route owns a Process controller pinned to one exact
`sillyos.translation@1.0.0` package digest. Program Library lists installed
exact packages and recent Processes. Starting Translation creates one Process
with an independent Workspace, settings override, network preference, domain
namespace and initial transcript; cold reopen loads that exact package and its
...
| Capabilities       | Required Agent and UI abilities are understandable             | Exact package capability IDs select only fixed Host capabilities; Creator has Pi-native read/write/edit/bash, structured grep and bounded QJS evidence, while fixed `fetch_url`/`download` remain behind one default-off Process network switch. Translation additionally requires immutable Program-resource read plus bounded Workspace read/search/write for advisory working memory                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Broader real-model evidence, interpreter-backed workflows, and capability composition                                                                                                 |
