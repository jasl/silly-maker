...
| Product/Workspace storage     | Product metadata and preferences remain on the control origin. Physical Program Data Repository V9 normalizes Program definitions/heads/revisions/decisions, Creator Processes, transcript entries, one Process execution-lease store, and the single `process_commits` operation-receipt authority while retaining separate Program-scoped Workspace-continuation and network-access rows. Missing network rows mean disabled. The preview cutover is a row-blind reset and never translates an old aggregate, grant, or superseded receipt store. Workspace bytes remain Sandbox-origin OPFS                                                                                                                                                                                                                                                                    | Old preview database rows and control-origin OPFS bytes are not migrated; the latter may remain browser-managed but are unreachable. No import/restore or fixed cross-device durability claim exists                                                                                                                                                                                                                      |
