...
head. TXT, Markdown, SRT, and closed Translation JSON use deterministic
round-trip adapters. Born-digital PDF loads PDF.js/its Worker only when selected
and exposes text reflow only; the original remains authoritative and OCR,
layout-preserving PDF translation, password UI, rewriting, and PDF round-trip
export are not claimed.
...
This foundation does not yet include a format-preserving final exporter,
model-assisted semantic QA, Python, CodeAct, or real-Provider product
qualification. Package-authored OpenUI, Conversation-mode free follow-up and
non-blocking mechanical Review findings are delivered; Agent-generated OpenUI is
not. The delivered batch/review loop must therefore not be described as a
complete Translation Program or a closed P5-B/P5-C journey.
...
| Generated UI       | Agent-authored UI remains legible and controllable             | SillyOS-owned Program UI Container bounds guided/Conversation views, overlays, and Agent-run projection; complete package-authored OpenUI data maps through a closed renderer to Host-owned components and prompt actions                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Add currentness-fenced Agent-generated document publication without admitting arbitrary React nodes, portals, renderers, or action authority                         |
...
| Translation        | A usable translation Program                                   | Exact `sillyos.translation@1.0.0` package pinning, independent Process Workspace/settings/network/domain state, original-file import, V16 Process-owned workset/pageable rows, model-envelope batch planning, shared Agent execution, fenced terminal/candidate publication, editable exact-CAS accept/reject, non-blocking mechanical Review findings, package-authored OpenUI, Conversation free follow-up, cold/read-only reopen, four deterministic text formats, born-digital PDF text-only projection, 10,000-unit DOM virtualization, and Chromium/WebKit deterministic import-to-accepted-target evidence. The profile grants no Workspace tools and the formal package has `scripts: []` | Add model-assisted semantic QA, format-preserving export, Agent-generated OpenUI publication, repeated real-Provider product qualification, and target qualification |
