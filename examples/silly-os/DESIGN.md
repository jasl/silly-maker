...
large-project capability. The control, Workspace Sandbox, and Network Broker
artifacts are now deployed together from exact source commit
`329f8cc70a9b4a57d57c9653772dca519e3f9221`. The final Cloudflare versions are
control `1dc1a247-ed98-4063-931f-2dd4fa681bee`, Workspace Sandbox
`7e1310ba-86c4-421e-b284-9015f1a3323b`, and Network Broker
`b005f590-bea4-4a55-8c15-db1a6a22292b`. Their exact public HTML paths return
HTTP 200, all three embed the same source revision, the control composition
matches both expected child identities, and the control CSP admits only the two
named child origins. A public Creator/Settings smoke and a disposable
`?agent=pi-test` initialization observed one active Broker frame and one active
Sandbox frame without console warnings or errors. This is artifact and
bootstrap evidence, not a public-origin real-model tool qualification. That
tuple is the pre-S3 public baseline. Current source now contains the bounded S3
Credential Vault described below, but it has not yet been redeployed in this
round; the canonical origin must therefore be treated as not having the Vault
until a new exact build/deployment receipt replaces this paragraph. No
public-origin persistent-Vault or real custom-profile qualification is claimed.
...
session remains current. The later **Allow for this Program** choice is an
explicit opt-in checkbox in the current Program's network settings; unchecked
preserves **Allow once**, while checked may persist only the exact normalized
`(programId, origin, operation)` grant and can be revoked. It must never silently
promote a one-time decision. Exact placement and visual polish may follow the
broader UI rewrite without changing those semantics. **Deny** leaves the
operation unavailable. Approval UI shows the exact URL and warns that an Agent
can encode user or Workspace data into its path/query. CORS failure does not
mean the GET was never sent.
...
does not wait for a stuck Workspace close, export/network-approval settlement,
...
source graph. The Cloudflare route for that policy and selective hashed-asset
dispatch exists in current source; the previous public deployment does not
prove it and awaits this round's rebuild/redeployment.
...
| Credential persistence        | Current source implements an optional passphrase Vault in its own IndexedDB/database Worker: PBKDF2-HMAC-SHA-256 at 600,000 iterations, non-extractable AES-256-GCM, random Vault salt and per-record IV, encrypted verifier, immutable complete-endpoint AAD, create/unlock/Lock/list/upsert/Forget, and exact one-time MessagePort handoff without a full-key read API                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Session-only remains default. WebAuthn PRF/device verification, passphrase recovery, cross-device sync, public-origin deployment/evidence, and resistance to compromised unlocked control code remain absent                                                                                                                                                                                                                      |
...
| Browser            | Publishable local-first product with BYO Provider         | Compatible built-ins + request-free Save + optional diagnostics + strict S0 response floor + closed S1a independent-origin ordinary route + closed bounded Q1/N0 implementations + one build-matched public three-origin release                                                             | Durable network grants/download/search, public real-model tool evidence, and later execution profiles    |
...
The current Sandbox, live-tools, Q1, and N0 artifacts share one deployed build
identity; that release does not promote public-origin real-model tool behavior.
