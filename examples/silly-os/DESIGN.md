...
| Program network tools         | Closed N0 registers one fixed `{ url }` `fetch_url` Pi `AgentTool`. An exact session-only **Allow once** permit is consumed before one current request crosses the build-identified, keyless third-origin Broker. The Broker performs only bounded credential-free CORS HTTPS text `GET`; a 15 s total/5 s idle Broker deadline plus a 20 s Agent-side silent-peer deadline keeps the typed request bounded. Chromium and persistent WebKit prove the real frame/Worker route, denial, exact retry, and zero credential/request-body authority. The build-matched control/Broker/Sandbox release is deployed, and its exact public paths, CSP, identities, Creator/Settings route, and deterministic frame bootstrap have public evidence | Only CORS-readable declared UTF-8 text is active. Physical redirect/missing-CORS target evidence is not claimed; Playwright fulfillment bypasses those Browser checks. No durable Program grant, `download`, search, authenticated request, redirect following, arbitrary header/body, guest network, arbitrary-page scraping, or public-origin real-model `fetch_url` qualification is active |
