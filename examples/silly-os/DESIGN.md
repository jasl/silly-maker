...
Authority, the same-origin Host Worker is deleted, physical Product Repository
...
Bundling controls distribution and discovery only. It grants no runtime
privilege: bundled and later admitted Programs use the same Program UI
Container, Process/Workspace ownership, Agent execution, admission, and
currentness rules.
...
Program content. One durable Creator Process pins the Creator definition, owns
its pageable user-visible transcript, names the subject Program, and coordinates
that Program's current Workspace and review. The same Program may be reopened
from another Process without rewriting either transcript. Conversation does not
...
itself is the build-known `sillyos.builtin.creator@1` Program definition. A
Creator Process pins that definition revision at creation and may name the
Program being created or edited as its subject. The subject's later revisions
do not rewrite the Process harness. The Process owns its user-visible pageable
...
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SillyOS control plane           | Creator UI, Program manifest and lifecycle, ordinary preferences, product repository, persistent Agent-state projection, the Program network-access switch, review/currentness, closed `UiArtifact`/OpenUI-to-SillyMaker mapping, and truthful capability state | `eval`, `new Function`, user-controlled dynamic import, generated HTML injection, generated JavaScript/Python/shell execution, full-key readback, or a generic bridge to credentials, product storage, DOM, cookies, or browser APIs                    |
...
Workspace Sandbox. A trusted handler binds Program, run, tool-call, network-access, and
currentness identity; the model supplies only the admitted tool fields. The
current Program's one persisted network-access boolean gates both fixed network
tools and defaults to disabled.
...
dispatcher. SillyOS owns URL admission, the Program-level network switch, the
...
Network access is one ordinary non-secret product boolean keyed by `programId`.
...
the Product Repository; trusted control code synchronizes only the current
Program/workspace-bound boolean before submit and after a settings mutation.
...
continue; an unconfirmed disable or failed sync disposes only that Program's
Agent Worker. The UI warns once at the Program setting that an Agent can encode
...
is bounded and requires the Program switch to be enabled, and residual malicious-prompt/site, authorized
...
1. **SillyOS Product Repository** stores Program definitions/catalog, Creator
   Processes, pageable rich Conversations, ordinary preferences, human
   decisions, Process attempt/checkpoint and lease/fencing state, and exact
   Process operation receipts.
...
| Ordinary Workspace Authority  | The default Program Authority creates the exact-origin frame transport; its fixed Host Worker owns OPFS, snapshot/export, lifecycle and the Program-bound volume; the retired same-origin Host Worker and fallback are absent. Reverse-origin Chromium/WebKit tests prove that the Sandbox cannot read the control document or same-named control IndexedDB/OPFS sentinels and cannot emit a control-origin fetch. The exact build-matched Sandbox artifact and bootstrap path are deployed; one public Chromium QJS loop publishes exact mutations                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | This proves the named control/Sandbox boundary, one journey, and one exact Translation source-file import—not total XSS resistance, guest-code confinement within the Sandbox origin, general archive/package import or restore, public WebKit, or general public-origin Workspace behavior                                                                                                                                                                                                                                                                |
| Product/Workspace storage     | Product metadata and preferences remain on the control origin. Physical Program Data Repository V13 retains normalized Program metadata, Process/transcript rows, Process execution leases, and single `process_commits` operation-receipt authority. A Translation Process directly owns one compact workset head, exact receipts, and pageable unit/glossary/candidate rows; there is no independent Project identity, list, route, or lifecycle. `process_workspace_bindings` gives each non-Creator Process one exact Workspace/volume identity, distinct from Program continuation and network-access rows. Process creation, its initial transcript checkpoint, and that binding share one IndexedDB transaction; Sandbox-origin OPFS remains the physical byte authority and is not part of that transaction. Translation source import acquires the existing Process execution lease after parsing while atomically verifying that its workset is still absent or at the caller's exact staging revision; the Workspace write and every later workset mutation verify that attempt/generation. Ready workset publication and the completed Process terminal/checkpoint share one IndexedDB transaction. An expired unfinished import is retained as unrecoverable for direct review, while ordinary start skips it and creates a fresh Process. Missing network rows mean disabled | Before the first stable persistence contract, every older preview database resets row-blind to V13 instead of carrying migrations. Old control-origin OPFS bytes may remain browser-managed but unreachable. The ordinary Translation controller uses the same Workspace Authority for exact original-file import and binds its checkpoint to the Process-owned workset. This is fenced sequencing rather than an atomic IDB/OPFS transaction, general import/restore framework, complete import-to-export journey, or fixed cross-device durability claim |
...
staged and current Program files before the first write. It rejects delete,
unstaged overwrite, stale before-bytes, missing parent directories, invalid
paths, and over-limit output. Accepted changes then use the existing Program-
...
- A Creator Process pins its Creator definition, owns the pageable user-visible
...
- The independent Network Broker never owns or receives the Provider key. The
  ...
  Program's network-access boolean. The switch UI and persistence belong to
...
- Browser ultimately binds Pi's shipped `read`, `write`, `edit`, and `bash` tool factories
  ...
  the Program network switch, Broker execution, and `download` staging/receipts. They do not register
...
the complete target-neutral `ProgramWorkspaceSnapshotReceiptV1`; a rejected
...
10,000-unit test proves DOM windowing rather than a 10,000-row render. Repository V13 now backs that
...
The ordinary Translation route owns a `sillyos.builtin.translation` revision 1
Process controller. It
accepts only an accepted Translation subject, opens the newest exact Process or
creates one Process with an independent Workspace and initial transcript, and
cold-reopens its durable workset without attaching the idle Workspace. Existing
Processes remain explicitly reopenable by `processId` at the controller boundary
when their subject Program is no longer in the catalog; Home does not yet
discover orphan Processes. Import accepts File or bytes, derives raw SHA-256 and a canonical
...
harness qualification, not a Translation grant. The intended Translation
Definition currently has no capability IDs, and no Translation Agent profile
selects Workspace tools, `bash`, or `qjs`.
...
| Translation        | A usable translation Program                                   | P5-A research, plus the `sillyos.builtin.translation` revision 1 Process route, independent Process Workspace, original-file import, V13 Process-owned workset/pageable rows, model-envelope batch planning, shared Agent execution, fenced terminal/candidate publication, editable exact-CAS accept/reject, cold reopen, four deterministic text formats, born-digital PDF text-only projection, 10,000-unit DOM virtualization, and Chromium/WebKit deterministic import-to-accepted-target evidence. The Definition grants no Workspace tools | Add discovery-oriented QA, format-preserving export, OpenUI, Conversation free follow-up, real-Provider product qualification, and target qualification             |
