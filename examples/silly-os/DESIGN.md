...
| Ordinary Workspace Authority  | The default Program Authority creates the exact-origin frame transport; its fixed Host Worker owns OPFS, snapshot/export, lifecycle and the Program-bound volume; the retired same-origin Host Worker and fallback are absent. Reverse-origin Chromium/WebKit tests prove that the Sandbox cannot read the control document or same-named control IndexedDB/OPFS sentinels and cannot emit a control-origin fetch. The exact build-matched Sandbox artifact and bootstrap path are deployed; one public Chromium QJS loop publishes exact mutations                                                                                                                                                                                                                                                                                                               | This proves the named control/Sandbox boundary and one journey, not total XSS resistance, guest-code confinement within the Sandbox origin, import, public WebKit, or general public-origin Workspace behavior                                                                                                                                                                                                            |
| Product/Workspace storage     | Product metadata and preferences remain on the control origin. Physical Program Data Repository V10 retains the V9-normalized Program definitions/heads/revisions/decisions, Process/transcript rows, Process execution leases, and single `process_commits` operation-receipt authority. Its additive `process_workspace_bindings` store gives one non-Creator Process an exact unique Workspace/volume identity, distinct from Program-scoped Workspace continuation and network-access rows. Process creation, its initial transcript checkpoint, and that binding share one IndexedDB transaction; Sandbox-origin OPFS remains the physical byte authority and is not part of that transaction. Missing network rows mean disabled                                                                                                                            | Old preview database rows and control-origin OPFS bytes are not migrated; the latter may remain browser-managed but are unreachable. The Host protocol can import one fenced file into an already-open volume, but no ordinary Translation product journey, import/restore UI, atomic IDB/OPFS creation claim, or fixed cross-device durability claim exists                                                              |
...
suggestions, stage navigation, exact committed-unit progress, virtualized unit
rows, and target editing on desktop and mobile. A 10,000-unit test proves DOM
windowing rather than a 10,000-row render. It does not prove Repository/Worker
paging: the current admitted `TranslationProjectV1` is an in-memory aggregate
with a human title, immutable source structure, glossary, targets, committed
batch bindings, materialized O(1) progress, and a bounded row-window projection.
It has no arbitrary semantic unit ceiling. Batch subsets retain the Project's
global contiguous order, so later batches are valid when their first order is
non-zero.
...
Definition currently has no capability IDs, Creator bootstrap does not publish
it, and no Translation Process profile selects Workspace, `bash`, or `qjs`.

This foundation does not yet include an ordinary route or Translation-owned
Process controller, durable source-artifact/format-adapter/export bindings,
Worker or Repository project paging, the real Agent batch/commit/resume loop,
OpenUI, Python, or CodeAct. It must not be described as a complete Translation
Program or a closed P5-B/P5-C journey.
...
| Translation        | A usable translation Program                                   | P5-A research plus the first formal UI/domain foundation: admitted in-memory Translation Project, human title, global batch ordering, exact progress, 10,000-unit DOM virtualization, mobile target editing, and guided/Conversation container. The intended Definition remains unpublished and grants no tools                                               | Ordinary route and Process controller; durable source/format/export binding; Repository paging; real Agent batch/commit/resume and Review; OpenUI and target qualification |
