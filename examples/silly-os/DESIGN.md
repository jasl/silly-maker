...
The research candidate script's `project.json` remains a laboratory wire format;
it does not create a Project product layer or a second persistence authority
above the durable Process.
...
a Program extension API. A future OpenUI document enters only as admitted data
and is mapped by a closed SillyOS renderer into the guided surface; Program
content cannot provide a React node, portal, renderer, outer chrome, overlay, or
run strip.
...
The earlier deterministic fixed script remains a source-only research note.

This foundation does not yet include a format-preserving final exporter,
discovery-oriented QA beyond candidate/protected-structure admission, OpenUI,
Conversation-mode free follow-up, Python, CodeAct, or real-Provider product
qualification. The delivered batch/review loop must therefore not be described
as a complete Translation Program or a closed P5-B/P5-C journey.
...
| Generated UI       | Agent-authored UI remains legible and controllable             | SillyOS-owned Program UI Container bounds the guided/Conversation views, overlays, and Agent-run projection; Host-owned React surfaces prove the closed placement boundary, not generated UI                                                                                                                                                                                                                                                                                                                                                                                                                          | Admit OpenUI as data and map it through a closed renderer inside the guided slot; no arbitrary React node, portal, or action authority                              |
...
| Translation        | A usable translation Program                                   | P5-A research plus exact `sillyos.translation@1.0.0` package pinning, independent Process Workspace/settings/network/domain state, original-file import, V16 Process-owned workset/pageable rows, model-envelope batch planning, shared Agent execution, fenced terminal/candidate publication, editable exact-CAS accept/reject, cold/read-only reopen, four deterministic text formats, born-digital PDF text-only projection, 10,000-unit DOM virtualization, and Chromium/WebKit deterministic import-to-accepted-target evidence. The profile grants no Workspace tools and the formal package has `scripts: []` | Add discovery-oriented QA, format-preserving export, OpenUI, Conversation free follow-up, real-Provider product qualification, and target qualification             |
