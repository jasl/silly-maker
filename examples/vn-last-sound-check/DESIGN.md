...
[《最后一次试音》历史计划](../../docs/engine/plans/2026-08-27-vn-reference-tour.md) 拥有。
...
| ID prefix      | `vn-last-sound-check`；持久/作者 ID 一律 lower-case kebab-case     |
| 默认 locale    | `zh-CN`，完整 `en` variant，显式 fallback 到 `zh-CN`               |
| 目标时长       | 任一路线首次游玩约 10–14 分钟                                      |
| 目标平台       | Browser 独立可发布 release build；当前 Deno Desktop static preview |
...
隐藏的正确答案。本产品是原创、独立、内聚的小型 Visual Novel，不复制 Ren'Py Quickstart、已退役的 Bookshop、
...
| ------------------------------------ | ---------------- | -------------------------------- | --------------------- |
| `character.vn-last-sound-check.lin`  | 林澄 / Lin Cheng | 值班播音员，提出录制当下的新台呼 | `focused`, `relieved` |
| `character.vn-last-sound-check.zhou` | 周遥 / Zhou Yao  | 技术员与档案交接人，持有旧录音   | `neutral`, `soft`     |
...
| ------------------------------------------- | ---------- | ------------------------------------------------------ | ---------------------------------------- |
| `scene.vn-last-sound-check.control-room`    | 夜间控制室 | 话筒、磁带机、调音台、信号灯、挂钟、窗外微光、两名角色 | 开机、试听、说明一次窗口、选择与发送     |
| `scene.vn-last-sound-check.rooftop-antenna` | 清晨屋顶   | 天线、摆动电缆、总闸、状态灯、天色、两名角色           | 关闭发射机并确认 route-specific 入档结果 |
...
产品固定 **110 个唯一可见 text entries**。任一路线首次游玩可见 **82 个**；Title/Save/Settings 等系统 UI
文案不计入 110。除与两段既有语音逐字绑定的三句短台呼外，每个 `say` 页面保持一至两句可在最小桌面
视口完整呈现的文案；不能用空白、重复台词或不可达文本凑数。语音台呼同样必须通过最小视口行为验收。

| 章节                    | Scene        | 内容                               | entries |
| ----------------------- | ------------ | ---------------------------------- | ------: |
| `shared.power-on`       | control room | 开机、林澄入场、说明最后一班值守   |      18 |
| `shared.old-recording`  | control room | 试听旧台呼，确认它是周遥早年的录音 |      17 |
| `shared.one-window`     | control room | 档案窗口、两种方案及无法兼得的原因 |      16 |
| `decision.signal`       | control room | 1 条 prompt + 2 个 option labels   |       3 |
| `route.archive.prepare` | control room | 整理旧录音并发送                   |      11 |
| `route.archive.roof`    | rooftop      | 关闭发射机，确认旧声音入档         |      16 |
| `ending.archive`        | ending       | “旧声入档”结局标题                 |       1 |
| `route.present.prepare` | control room | 林澄录制并发送新台呼               |      11 |
| `route.present.roof`    | rooftop      | 关闭发射机，确认当下声音入档       |      16 |
...
28 个专属 entries、专属发送音频和专属结局；换一句尾声、换色或共享同一结果不能算双路线。
...
| BGM                       | `music.vn-last-sound-check.last-shift` | audio manifest + authoritative intent |
...
- `text.vn-last-sound-check.shared`：51 条 shared narrative + 3 条 choice 文案；
- `text.vn-last-sound-check.route.archive`：27 条旧台呼路线文案 + 1 条 ending title；
- `text.vn-last-sound-check.route.present`：27 条新台呼路线文案 + 1 条 ending title。
...
src/
  ...
  tooling/                        # Inspector + Narrative binding/source navigation、Flow、named simulations
assets/
...
`@sillymaker/vn` 是唯一 VN 品类所有者：静态产品选择 `preset`，动态产品组合 `ui/core` 与明确选择的
`history`。低层 UI primitive 不再公开第二个完整 preset。产品不复制 passive Narrative renderer，可以通过公开
CSS 变量换肤；确实需要结构性差异时整体 eject/replace，但不能并挂第二个 writer。
...
TypeScript symbols 使用 `vnLastSoundCheck` / `VnLastSoundCheck` camel/Pascal case。
...
的 Web Audio object。本产品建立自己的首个 Save floor；不接收 Cat Cafe/已退役的 Bookshop/Template Save，不建设跨产品
migration 或通用兼容框架。
...
已退役的 Bookshop、Template 或第三方 VN 的文案、角色、构图、录音、素材与品牌；参考产品只能用于行为层研究。
