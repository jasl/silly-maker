...
test("embedded authoring inherits the shared UI baseline and stays bounded", async ({ page }) => {
  ...
  for (const viewport of viewports) {
    ...
    if (singleColumn) {
      await panel.locator("[data-embedded-authoring-content='true']").evaluate((content) => {
        content.scrollTop = content.scrollHeight;
      });
      await expect(sourcePointer).toBeInViewport();
    } else if (viewport.width < 1_280) {
      await panel.locator("[data-inspector-properties='true']").evaluate((properties) => {
